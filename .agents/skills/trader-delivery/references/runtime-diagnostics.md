...
- `web`, `security-master`, `history`, `tencent`, `tushare`, `history-daily-capability`, `history-archive`, `browser`, `performance`: run exactly one owning diagnostic module through the unified CLI. `security-master` checks the atomic official SSE/SZSE listing snapshot without emitting stock identities; `history-daily-capability` audits the configured daily-history source boundary; `history-archive` reads every stable monthly SQLite header, samples bounded `dbstat` page classes, benchmarks the three latest-row workloads, and copies a bounded revision batch only into a disposable system-temporary database; it never modifies the active archive. Use these after a combined scan has isolated one boundary or when executing its dedicated gate.
...
- `runtime`: samples the running `/api/status`, Tomorrow/D25 current projections, and the Long current snapshot. Use for no-data, funnel, release/schema, projection identity, warmup-state, and restart symptoms.
