...
class StrategyOutcomeService:
    ...
    def update_outcomes(
        ...
        for signal in signals:
            ...
            label_status = str(outcome.get("label_status") or ("unfilled" if outcome.get("excluded") else "settled"))
            ...
            if label_status != "settled":
                ...
                self.repository.save_execution_record(execution_record)
                self.repository.delete_strategy_outcome(signal["id"])
            ...
            if primary_return is None:
                ...
                self.repository.save_execution_record(
                    _build_execution_record(signal, outcome, policy, scenarios, {})
                )
                self.repository.delete_strategy_outcome(signal["id"])
                skipped += 1
                unknown_count += 1
            ...
            if calculator_key not in benchmark_calculators:
                candidates = self.repository.candidate_snapshots_for_date(
                    calculator_key[2],
                    strategy_name=calculator_key[0],
                    strategy_version=calculator_key[1],
                )
                candidate_batches[calculator_key] = candidates
                benchmark_calculators[calculator_key] = CandidateBenchmarkCalculator(provider, candidates)
            candidate_rows = candidate_batches[calculator_key]
            selected_candidate = next(
                (item for item in candidate_rows if normalize_code(item.get("code")) == normalize_code(signal["code"])),
                None,
            )
            ...
            benchmark = benchmark_calculators[calculator_key].calculate(signal, outcome, primary_return_field)
