...
def score_tomorrow_candidates(
    ...
    rows.sort(key=lambda item: item["score"], reverse=True)
    rows = _attach_expected_return_prediction(
        "tomorrow_picks",
        rows,
    ...
    display_candidates = [row for row in rows if _ranking_gate_score(row) >= display_floor]
    ...
    strict_display_count = len([row for row in display_rows if _ranking_gate_score(row) >= min_score])
...
def score_swing_candidates(
    ...
    eligible_rows = [row for row in rows if _ranking_gate_score(row) >= min_score]
...
def _attach_expected_return_prediction(
    ...
    enriched.sort(
        key=lambda item: (
            coerce_number(item.get("rank_score"), coerce_number(item.get("score"))),
    ...
    for index, row in enumerate(enriched, start=1):
        ...
        row["ranking_source"] = "expected_return_rank_score"
    return enriched
...
def _expected_return_rank_active(row: Dict[str, object]) -> bool:
    return (
        ...
        and str(row.get("ranking_source") or "").strip() == "expected_return_rank_score"
    )


def _ranking_gate_score(row: Dict[str, object]) -> float:
    if _expected_return_rank_active(row):
...
def _tomorrow_risk_penalty_parts(row: pd.Series, provisional: bool = False) -> Dict[str, float]:
    ...
    if not provisional and has_close_range and close_location < 0.35:
        if bool(getattr(config, "USE_SMOOTH_PENALTY", True)):
            _set_positive_penalty(parts, "weak_tail_close", _penalty_value(close_location, 0.40, 10, 4.0, direction="below"))
        else:
...
# Compatibility exports: keep legacy ``stock_analyzer.scoring`` imports stable
# while the first low-risk chunks live in focused scoring_core modules.
from . import candidate_filters as _candidate_filters_impl
from . import market_regime as _market_regime_impl

HARD_FILTER_LABELS = _candidate_filters_impl.HARD_FILTER_LABELS
prepare_candidates = _candidate_filters_impl.prepare_candidates
candidate_filter_report = _candidate_filters_impl.candidate_filter_report
_candidate_base_frame = _candidate_filters_impl._candidate_base_frame
_candidate_filter_masks = _candidate_filters_impl._candidate_filter_masks
_combine_candidate_masks = _candidate_filters_impl._combine_candidate_masks
_is_buyable_gain = _candidate_filters_impl._is_buyable_gain

build_market_regime = _market_regime_impl.build_market_regime
_history_breadth_metrics = _market_regime_impl._history_breadth_metrics
_market_regime_breadth_frame = _market_regime_impl._market_regime_breadth_frame
_market_regime_with_history = _market_regime_impl._market_regime_with_history
