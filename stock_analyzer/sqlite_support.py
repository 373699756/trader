...
def open_sqlite(
    ...
    *,
    ...
    db_key = os.path.abspath(db_path) if str(db_path or "").strip() and db_path not in (":memory:",) else db_path
    ...
    if db_key not in _wal_initialized:
        connection.execute("PRAGMA journal_mode = WAL")
        _wal_initialized.add(db_key)
