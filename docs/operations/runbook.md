...
4. 如启用 Tushare 慢数据，执行 `.venv/bin/python -m pip install ".[tushare]"`，并设置 `TUSHARE_TOKEN`；或用 `TUSHARE_TOKEN_FILE` 指向只含一行 Token、POSIX 权限为 `600` 的普通文件。环境 Token 优先，Token 和文件都不得提交到仓库。
5. 确认目标端口只绑定回环地址，且 `.runtime/v2` 所在磁盘可写。
...
运行配置现使用 schema v5；`pipeline.cadence_seconds`、`market_data.cache_policy` 和 `performance_budgets` 分别是刷新、缓存和性能预算的唯一配置入口，缺键、未知键或固定值漂移都会在启动前失败。升级旧配置时必须整体切换仓库随附的 v5 文件，不能只手工追加部分字段。`/api/status` 的 `dependencies.cadence` 展示当前间隔、下一到期时间和运行中任务，动态计数使用 `cadence_<task>_planned/submitted/skipped_*`；同类任务仍运行时出现 skipped 属于预期背压，不会在随后补跑。
...
- 行情活动来源、失败次数和熔断状态。
- 五个来源 lane 的运行/待处理/latest-wins 状态，以及按数据集和来源统计的缓存条目、命中、负缓存、刷新、淘汰和估算字节数。
...
- 东方财富与新浪全市场请求并行采集并确定性合并；任一来源失败时使用另一来源，全部失败时保留最近有效统一快照。相同身份由 single-flight 合并，连续失败 3 次熔断 60 秒，超时后只放行一个恢复探针。腾讯候选报价失败、超时、返回空值或乱序旧版本时保留最近有效行情和 overlay，不清空推荐。
- Tushare 未安装、无 Token、额度不足、429 或超时：只降级证券主数据、交易日历、前复权日线、日度估值和财务指标；不会改用 Tushare 抓取高频报价，也不会阻塞东方财富、腾讯、新浪、AKShare、本地推荐或只读 Web。
