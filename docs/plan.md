...
完成记录：外部供应商适配器已迁移到 `infra/market_data/providers`，规范化、合并、字段质量和特征物化
已迁移到 `infra/market_data/normalization`；所有生产、诊断脚本和测试导入已切换，旧同级模块物理删除。
新增边界契约证明 normalization 不反向依赖 provider；来源回退、单位/复权、时区、缓存身份、merge epoch
和健康状态语义保持不变。定向测试、全量质量门禁、打包和仓库外 wheel 验收已通过。
...
完成记录：历史、参考数据和服务编排模块已分别迁移到 `history/`、`references/` 和 `service/` 唯一包；
服务门面改名为 `service/facade.py`，所有生产、测试和诊断导入切换到新路径，旧平铺模块物理删除。
历史预热、来源 lane、deadline、缓存身份、持久化和失败保留语义保持不变。定向测试、完整高风险门禁和
wheel 构建验证通过；真实 `sources` 供应商诊断未执行，现场供应商延迟和降级行为保留为未验证风险。
...
完成记录：过滤、板内评分、风险/融合和稳定选择模块已分别迁移到 `filtering/`、`scoring/`、
`risk_fusion/` 和 `selection/` 唯一包。跨阶段的 `ScoredDisposition`、`ScoredStockEvaluation` 和
`ScoredSelectionResult` 已归入推荐域共享 `models.py`，消除风险融合对选择实现的反向依赖；评分公式、
风险扣除、融合向量、动作门槛、TopK 和集中度约束保持不变。定向测试、性能检查、完整高风险门禁和
打包验证通过；本批未执行真实供应商诊断，因未改变供应商 I/O。
