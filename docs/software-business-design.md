...
- 本批将收益优化建议与评估研究显式签入本章：收益挑战者先按 `docs/score.md` 的 P0
...
- `docs/score.md` 只记录下一阶段评分研究的非权威实施计划。P0-P1 已把最多 60 日评价窗口、
  固定统计身份、硬过滤后数据裁剪目标和紧凑研究轨迹同步至两份权威文档；P2 及以后仍须逐批
  契约和测试。活动生产与收益晋级严格使用荐股策略文档第 15.1 节，不得由计划文件提前切换。

### 14.3 V2-only 直接替代与删除

迁移按 `docs/V2_plan.md` 的 V2-0 至 V2-11 逐批交付。每批先建立失败契约测试，再实现和
验证；V2 不得新增对旧 Pipeline、旧 snapshot、旧冻结、旧 Web、旧运行目录或旧状态对象的
依赖。开发阶段尚未替代的旧代码只能作为待删除实现存在，不能成为 V2 输入、回退或降级源。
...
`docs/V2.md` 是用户明确保留的 V2 唯一产品目标概览，`docs/V2_plan.md` 是对该概览的
现状评审和 P0-P13 可执行拆解；两者均固定标记为非生产契约。它们只能整理尚未实施的工程
顺序，不得覆盖本文、荐股策略文档或 `pyproject.toml`；其中任务落地前必须先把行为变化
同步到对应权威文档和契约测试，全部迁移完成后退役两份临时文档。
