...
| 11:20-13:00 | 暂停策略计算，以 10 秒全市场、10 秒候选和 10 秒 TopK 报价维持展示 |
| 13:00-14:20 | tomorrow、d25 预计算；long 继续刷新当前观察 |
...
规范 JSON 不超过 160 KiB。`runtime.json` 中 `published_recommendation_view.capacity=72`
...
- 生产配置使用 `versioned_dag`；候选行情、新闻和公司风险变化携带不可变版本进入有界
  事件链，本地评分完成后先发布 `projection_stage=local`，DeepSeek 完成事件只有通过
