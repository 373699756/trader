...
| 新浪 | 同周期全市场校验和字段回退 |
| 腾讯 | 候选和 TopK 定向实时报价 |
| Tushare | 120 积分档以 SDK `daily` 批量未复权日线作为历史主源；更高积分能力必须按配置显式启用 |
| AKShare | 行业、新闻、公告和候选级研究数据 |

Tushare SDK 是默认运行依赖，Token 缺失时按来源降级；transport timeout 固定 8 秒，
...
顺序交给 Tushare 慢数据通道分批预热；120 积分档以批量未复权日线提供 OHLC、成交量、
成交额和涨跌幅，明确标记 `unadjusted_daily`，不得伪称前复权。单只缺失不能回滚同批成功结果，下一周期只续跑
未覆盖代码。Tushare 不可用时可由东方财富有界补齐，但不得再次阻塞实时行情任务。
...
全市场截止 20 秒、候选报价 3 秒、研究数据 8 秒、DeepSeek 默认 12 秒。全市场成功后
