...
`youhua_contract_base_v1` 冻结三个公共接缝。Codex A 是 schema、版本、公共 port/event、
配置、publisher、bootstrap 和集成测试 owner；Codex B/C/D 只能在各自独占路径实现内部
算法，公共接缝变化必须向 A 提交接口申请。
...
阶段 1 的 `CONTRACT_BASE` 只有在 B1/C1/D1 标准报告均 `ready_for_gate=yes`、接口 owner
唯一、schema/version 唯一、B/C/D 均确认从同一 commit 开始后才能发布；未满足 G1 时，
B/C/D 等待，不进入生产实现。
...
- v15 五来源、v16 三板评分与 v17 P1-P6 性能、缓存、发布和 Web 热路径已经完成；下一
  未完成工程章节以 `docs/plan.md` 第 2.6 节为准。收益验证、调参或新策略不属于该章节。
...
`docs/plan.md` 和 `docs/plan_c.md` 只允许记录尚未实施的批次顺序、测量基线和退出条件；
`docs/plan_sudu.md` 只允许细化第 7.2 节 P1-P6 原子章节的列式、增量发布和性能门禁；
`docs/plan_youhua.md` 只允许记录 A/B/C/D 四方分阶段并行交接计划。四份计划必须显式标记
为非权威并链接本节规定的两份权威文档；发现冲突时以权威文档为准，实施引起契约变化时
先更新权威文档。除此之外不得在 `docs/` 新建并行需求、计划、问题单、运行手册或归档文件。
