...
说明本轮保持空仓；不能误报成数据异常、评分尚未完成或模型复核缺失，也不能把固定成本前的毛预测收益
...
后续历史评分优化路线的执行顺序、样本门槛和研究终态以荐股策略文档第 15.1.21–15.1.40 节为唯一
权威。路线先完成一级永久资格名单与二级动态硬过滤，核对当前 V1/V2、P2 报告、人工授权和状态投影的
身份一致性，再建立独立 H1 点时归档与只读覆盖审计，预注册 Today 11:20、Tomorrow/D25 14:50 的策略
标签、60%/20%/20% 时序切分、5 日 embargo 和最终至少 200 个交易日。随后建立全候选预测—实际残差
账本，依次完成过滤瀑布/候选召回消融、历史 DeepSeek 结构化增量、收益/成本/风险模型、净效用与组合
约束挑战者、嵌套 walk-forward 和多重检验，再分别运行三策略终端留出与严重亏损概率/市场状态稳健性。
评分热链只能在决策 hash 完全等价下优化脏集、缓存、single-flight 和持久化成本；最后才允许自动生成
挑战者、受控晋级和下一次启动激活。每次“继续”只交付下一个完整未完成章节，不得跨章恢复未来日采集、
盘中热切换或无门禁生产更新。

第 15.1.23 节基线审计只能由显式只读 CLI 组合类型化的生产身份与研究状态端口，不进入普通 HTTP、SSE
或 Web；第 15.1.24 至 15.1.36、15.1.38 节的 H1、残差、消融、DeepSeek facts、候选、验证与自动挑战者
均位于隔离 `domain/application/infra/research` 和独立历史目录，只能由显式研究 CLI 装配；
`bootstrap.py`、生产调度、HTTP、SSE、Web、活动数据库和冻结链不得持有这些研究服务。历史
DeepSeek 研究只允许在生产复核截止后通过独立低优先级原子桶运行，并与生产预算共同遵守每日 168 次
全局物理请求上限。既有 H0/R6/P2 工件保持不可变，既有 139 日窗口只能作为已观察历史审计，不能重新
标为独立盲测。线上 T+1 outcome 只保存正式推荐历史、运行监控和回退告警，不进入模型训练。

第 15.1.37 节若修改生产计算热链，必须遵守本文第 7、13 节运行、性能和发布门禁，只允许候选、分数、
风险、动作、排名和决策 hash 完全等价的工程优化；DeepSeek 信息价值调度仍属于隔离研究挑战者，不能
以性能名义接入。第 15.1.39 节只建立默认不接入组合根的内容寻址 Champion/Challenger 注册表；第
15.1.40 节必须在用户明确生产接入指令下另立高风险批次，才能让通过全部历史门禁的 `next_start` 工件
在下一次正常启动激活。
