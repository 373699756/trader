...
只更新 `CHANGELOG.md`。`docs/plan.md` 和 `docs/plan_c.md` 只允许记录尚未实施的批次顺序、
测量基线和退出条件；`docs/plan_sudu.md` 只允许细化第 7.2 节 P1-P6 原子章节的列式、
增量发布和性能门禁。三份计划必须显式标记为非权威并链接本节规定的两份权威文档；发现
冲突时以权威文档为准，实施引起契约变化时先更新权威文档。除此之外不得在 `docs/` 新建
并行需求、计划、问题单、运行手册或归档文件。
