...
同交易日 `today` 正式冻结且不是 `close_fallback` 时，当前单表切换为 11:20 锚点跟踪模式：
...
| 11:20-13:00 | today 保留既有冻结/检查点；tomorrow、d25、long 保留上午 current 并更新报价 | tomorrow、d25、long 缺失时各补一次本地 current | 午间重复评分、提前模型复核或补造迟到 today |
...
| 15:00 后 | 已有正式记录继续可查，报价只更新 overlay；long 继续展示当前行情 | 先读数据库/P6；同日正式记录缺失时才按完整收盘行情创建一次 `close_fallback` | 因程序启动时点而永久 `not_ready`，或用上一交易日伪装当前 |
