...
身份而对全市场触发候选级慢请求。免费证券身份持久化在独立 `reference` lane 异步执行，Tushare
增强继续使用 `tushare` lane；首次冷启动批次允许按缺失规则降级，完成后的数据必须进入后续批次
并持久化；调度失败不得阻塞本地决策或清空最近有效结果。
...
错误数以及 `tushare_required=false`，不得再把免费身份缺口归因为缺少 Tushare token。
...
超时后回退旧快照或相同研究版本只能更新健康状态，不得触发新评分。候选报价刷新产生的完整基础
候选特征必须作为共享 `ScoringInputEpoch` 直接复用，Today 与 D25 共用基础 epoch，Tomorrow 只按
同一基础 epoch 追加尾盘分钟差量；禁止刷新后丢弃特征再从缓存重复构建，也禁止以 `observed_at`
制造相同数据的新缓存身份。运行中评分在数据准备完成、纯本地评分开始和发布前都必须检查是否已被
更新输入替代；已替代任务不得继续发布、请求 DeepSeek 或形成冻结输入。
...
已有的候选行情缓存数、候选行情年龄及最新候选来源，行情覆盖显示真实样本，证券身份显示“待评分”，
漏斗显示“采集中/待计算”，冻结卡显示“采集中”；不得显示空白、`0 → 0 → 0` 或把全市场来源冒充
...
`/api/v2/status` 的公开 schema 为 `v2_status_v3`，并必须从当前进程已导入的常量加法返回
...
当前静态资源握手身份为 `release-contract-2026-08-26-v3`。
...
3. Web 推荐漏斗反复为 0 时，运行 `scripts/check_web_recommendation_health.py` 连续读取
   ...
   默认向标准输出写 `web_recommendation_health_v1` 聚合报告，并保留历史预热计数轨迹，不输出股票代码、
   逐股行情或外部载荷。供应商或历史持久化现场复测使用 `scripts/sample_history_sources.py`；默认只读，
   只有显式提供仓库外 `--persistence-runtime-dir` 时才在隔离 SQLite 目录对比逐条与批量事务。
...
下一项可执行评分工程 Gate 为“原生评分因子诊断层”，当前状态为未开始。该 Gate 只建立研究诊断能力，
不改变活动评分、阈值、融合、冻结、DeepSeek 请求或 Web；详细标签、诊断维度和不可越过的生产边界只以
荐股策略文档第 15.1.11 节为准。该 Gate 完成后仍须独立 Review、适用门禁、单提交和推送，不能因本文
记录了路线就视为已经实现。
