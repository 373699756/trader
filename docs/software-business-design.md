...
观察项顺序合并；普通 Web 的 today、tomorrow、d25 只渲染正式推荐，忽略观察项，long
则在主表渲染当前固定名单。
...
首页固定包含：Header 状态、日期和策略切换、当前策略说明、结果摘要卡、单一荐股表、
选中股票详情、系统状态/DeepSeek 预算与失败原因。普通页面不提供“当前推荐”、
...
核心状态。long 的 `not_ready` 明确显示“长期策略当前尚无可用数据”，不复用短线
“流水线尚未发布”的提示。
