...
要求立即启用已封存但历史拒绝的 Tomorrow P2 工件，并将人工 P1 日线代理与 P2 做成配置切换。两种人工
发布的身份、失败证据、代理差异、监控和禁止自动更新边界以荐股策略文档第 15.1.17–15.1.18 节为准，
不得把越权发布或日线 proxy 描述成原 P1/P2 研究门禁通过。
...
估算成本、预测成本后净超额和模型分歧；P1/P2 都没有逐股亏损概率输出，页面不得用历史总体严重亏损率
...
`/api/v2/status` 的公开 schema 为 `v2_status_v6`，并必须从当前进程已导入的常量加法返回
...
当前静态资源握手身份为 `release-contract-2026-08-30-v7`。
...
“P1/P2 生产评分配置切换”由策略配置 schema 14 的 `tomorrow_scoring_profile=p1|p2` 唯一控制，默认
`p2`，重启生效。`p1` 加载独立包内 `p1_manual_residual_momentum_v1` 线性 proxy 及规范化内容身份 SHA-256
`89f21552c2cd3f2addb16fa6db28f4a515991429ec287725e8c1434ee14cd1b4`；它绑定 H0 规范/manifest、
1,765,685 个训练行和 `h0_board_amount_residual_momentum_proxy_v1`，状态必须公开原 P1 唯一工件不存在，
不得登记为原 P1 研究通过。`p2` 继续加载既有 ridge/LightGBM 工件。组合根只装配所选单实例，创建 App
仍无线程、无网络、无数据库和无文件写入；资源缺失、篡改或输入宽度不符时启动失败关闭。
...
显式投影 `profile_id`、模型 ID/hash、真实历史状态和失败原因；Web 使用中性的“模型信号分”，避免 P1
运行时仍显示 P2 标签。包内 P1 参数可由 `scripts/package_tomorrow_p1_model.py --output <path>` 从 H0
...
生产；第 15.1.17–15.1.18 节的 P2 人工越权与 P1 日线 proxy 是用户明确授权的已记录例外，不反向改变
任何研究报告。
