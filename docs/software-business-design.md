...
系统允许返回 0 只推荐。产品不包含策略验证工作台、收益回填、自动调参、机器学习
训练、股票价格预测和模拟交易页面；离线点时验证属于发布质量门禁，不是产品功能。
...
默认线程和有界资源为：调度 1、数据采集 6、标准化/过滤 2、策略评分 3、DeepSeek 4、
合并 1、持久化 1。数据池含五个普通来源 worker、五个普通待处理槽位，以及腾讯候选
和 TopK 专用的一个紧急 worker、一个紧急槽位。每个来源最多一个运行任务和一个
...
| 11:20-13:00 | 暂停策略计算，以 60 秒行情维持展示 |
...
| 新浪 | 同周期全市场校验和字段回退 |
| 腾讯 | 候选和 TopK 定向实时报价 |
| Tushare | 证券主数据、交易日历、前复权日线、日度估值和财务指标 |
| AKShare | 行业、新闻、公告和候选级研究数据 |

Tushare 是可选 extra，SDK transport timeout 固定 8 秒。Token 读取优先级为
`TUSHARE_TOKEN`、`TUSHARE_TOKEN_FILE`、配置指向的单行文件；POSIX 下拒绝
group/other 可读文件。密钥、Token、完整请求或响应不得写入配置、日志、SQLite、
快照或 API。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 全市场行情 | 60秒 | 30秒 | 60秒 | 60秒 | 30秒 | 14:49:50一次 |
| 120只候选报价 | 10秒 | 5秒 | 10秒 | 10秒 | 3秒 | 2秒 |
| TopK展示报价 | 5秒 | 3秒 | 5秒 | 5秒 | 2秒 | 3秒 |
| 本地评分 | 30秒 | 10秒 | 20秒 | 30秒 | 10秒 | 14:50冻结 |
...
频率只能来自 `runtime.json.pipeline.cadence_seconds`。周期任务在途时跳过本周期，
从当前时刻重算下一次，不排队补跑。
...
目标为：TopK 关键阶段 P95 年龄不超过 10 秒、其他阶段 20 秒；候选主执行 15 秒、
其他阶段 30 秒；全市场主阶段 60 秒、其他阶段 120 秒；SSE 发布 2 秒；today 行情到
评分发布 15 秒。数据年龄超过 cadence 2 倍标记 stale，超过 3 倍标记 degraded；
today 行情超过 20 秒或尾盘冻结行情超过 30 秒只能观察。

全市场截止 20 秒、候选报价 3 秒、研究数据 8 秒、DeepSeek 默认 12 秒。来源连续失败
...
today 在 11:20 冻结，tomorrow/d25 在 14:50 冻结。冻结后迟到行情、风险、模型结果、
补审或重算均不得改变股票集合、分数、动作、排名、版本或 JSON 哈希。overlay 只叠加
当前价、涨跌和锚点至今，不修改冻结对象。
...
推荐响应至少提供股票代码和名称、策略、板块身份、观察/冻结时间、锚点价、当前价、
本地/模型/风险/最终分、动作、结构化理由与风险、数据新鲜度、来源、策略/引擎/融合/
schema 版本和快照 ID。缺失字段返回 `null`、状态和原因，禁止伪造 0。历史日期只允许
...
计算。页面切换策略/日期时必须取消或隔离旧请求，迟到响应不能覆盖当前选择。

## 10. 桌面界面

首页固定包含：Header 状态、日期和策略切换、策略摘要卡、TopK 表格、选中股票详情、
系统状态/DeepSeek 预算与失败原因。每行可见关键分数、动作、数据年龄、板块、锚点价、
当前价和风险；详情展示结构化证据、缺失原因、审计字段及版本。
...
缓存命中/淘汰/字节、single-flight、策略耗时、DeepSeek 物理调用和预算、冻结状态、
SQLite/JSON 写入分类、P6 发布版本、SSE 客户端和慢客户端丢弃数。

日志使用脱敏结构化摘要，不记录密钥、Token、完整模型载荷、完整外部响应或个人敏感
路径。所有外部 I/O 有 timeout、容量和失败策略。DeepSeek 密钥优先读取环境变量，
也可读权限安全的单行文件；绝不进入配置快照、推荐历史或 API。
...
只更新 `CHANGELOG.md`。两份活动文档之外不得在 `docs/` 新建并行需求、计划、问题单、
运行手册或归档文件。
