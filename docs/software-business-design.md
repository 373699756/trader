...
活动冻结只保留 today、tomorrow、d25 最近 20 个不同交易日；新策略身份单策略单日最多
12 条，旧策略冻结最多 18 条，兼容期推荐明细硬上限仍为 1080。第 21 日出现时，旧日冻结、manifest、closing overlay 和结果
...
原因和最多 6 个正式结果与 6 个观察结果。SSE 先发布完整决策身份，随后只允许匹配该 identity 的报价
...
受控动作阻断和集中度跳过原因计数、最高本地/最终分及空结果原因；历史旧快照缺少该元数据时
...
观察项顺序合并；新策略当前响应最多 6+6 共 12 项。普通 Web 的 today、tomorrow、d25
当前视图将其拆为正式推荐主表和独立观察池表；tomorrow/d25 的 `close_fallback` 和历史
视图必须渲染 API 已返回的全部 TopK 项，避免收盘补算只有观察项时被前端过滤成空表；
long 则在主表渲染当前固定研究池，并由页面按长期分组过滤。旧冻结记录不可改写，显式
历史查询为完整审计允许返回旧策略的最多 10+8 共 18 项。
...
集中度规则发布活动新快照最多 6 个无 veto 的本地候选为 `observe`，并追加
`close_fallback_observe_floor`，不得改写已冻结记录或提升为可执行。收盘补算和历史视图
显示 API 返回的全部项；普通当前短线快照把 `executable` 与 `observe` 分别显示在正式表
和观察池。15:00 后
...
同交易日 `today` 正式冻结时，当前正式表和观察池都切换为 11:20 锚点跟踪模式：
正式表只保留 `executable`，观察池只保留 `observe`；逐行显示排名、股票、11:20 锚点价、实际锚点报价时间、
...
最高最终分、观察门槛和执行门槛，不得统一误报为“未通过下行保护”。观察池无项时保留
自身精确空状态。历史视图保持
