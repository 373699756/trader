...
- `tomorrow`：交易日上午开始形成滚动本地草稿，13:00 后增加尾盘数据和模型复核，
  面向 T+1，14:50 冻结。
- `d25`：交易日上午开始形成滚动本地草稿，13:00 后增加模型复核，面向 T+2 至 T+5，
  14:50 冻结。
...
`data_version`，已经开始的评分允许完成。候选和评分事件的排队过期时间分别包含上游
...
| 09:30-09:36 | today 观察；tomorrow、d25 开始生成本地滚动草稿 |
| 09:36-10:30 | today 主执行；tomorrow、d25 继续本地滚动评分 |
| 10:30-11:20 | today 降级执行并提高动作门槛；tomorrow、d25 继续本地滚动评分 |
...
| 13:00-14:20 | tomorrow、d25 增加尾盘数据准备和 DeepSeek 增强复核；long 继续刷新当前观察 |
...
候选和板内总体，发布 `local_degraded` 滚动草稿；上午不消耗其 DeepSeek 策略桶，并标记
`deepseek_deferred_until_afternoon`。13:00 后刷新 tomorrow 尾盘分钟数据并启动
tomorrow/d25 DeepSeek 增强复核，复核完成可发布 hybrid；14:50 前均为可替换草稿。
任何时段确无达到门槛的候选仍允许返回真实空结果，但不得因阶段路由直接跳过两种策略。
若 11:20-13:00 冷启动且当日 tomorrow、d25 或 long 当前快照缺失，输入完成事件允许
各补算一次；已有同日快照时不得在午间重复评分，增强复核仍须等到 13:00 后。
...
- 当前策略和引擎身份为 `strategy_review28_2026_07` 与
