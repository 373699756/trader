...
提交一次最新评分，不能直接丢弃。已经开始的评分只允许运行到下一个安全检查点；发现更新输入后必须
结束且不得发布。三条板块评分通道各保持一个 worker，待处理
...
并显示活动问题数、最高优先级中文原因、影响策略、处理阶段、发生时间和恢复状态。主界面不得直接
...
Web 子报告契约为 `web_recommendation_health_v3`：每个策略除完整漏斗阶段外，还分别保留最多 32 项
人口过滤、候选过滤、候选瞬态、候选可选告警和供应原因聚合计数，以及当前策略/profile 的历史 session
要求；它会把“已有完整评分却被旧全局历史门槛阻断”报告为错误，不得输出股票身份或逐股数据。
