...
当前交付已完成第 2.8 节的原生输入并行链，并按第 2.9 节保存、恢复和离线校验工程影子
证据；仍不执行生产指针切换。历史 60 个交易日批量下载/回填同步暂停，不计入影子或切换
门禁；收益研究的 250 日回放和 20 日挑战者前向影子仍以荐股策略文档第 15.1 节为准，与
本次同版本工程一致性影子相互独立。只有后续独立交付批次取得并复核真实完整交易日证据，
才可另行实施组合根读写指针切换。
