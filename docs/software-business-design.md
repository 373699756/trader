...
- `long`：固定长期研究池，只展示当前行情状态，不运行荐股策略、不冻结、不生成交易动作、
  不写推荐历史。
...
overlay。全市场、候选和评分使用同级 FIFO，并按计划器固定顺序形成
“全市场→候选→评分”，避免下一轮全市场越过上一轮未完成的筛选或评分。候选和评分
...
`score` 仍作为漏通知恢复心跳。公司风险变化不得被普通行情合并或丢弃。策略计算仍由
主事件顺序协调；公司风险触发的重算保留 risk 优先级和预留容量。P6 仍只有一个提交者；
不得用多个共享可变合并器换取表面并发。
...
codes、数量和来源摘要；非 `long` 响应该字段为空数组。`long` 的 `items` 返回当前固定池
...
当前固定研究池。长期页在策略按钮下提供二级 tab：`卡脖子行业` 和 `低价潜力股`。
`卡脖子行业` 下再按行业分组切换，每个行业最多展示 5 只固定龙头股；`低价潜力股`
最多展示 26 只固定股票。两类长期分组只刷新实时价格和涨跌幅，不因行情或评分自动换股。
摘要显示推荐数、可执行数、过滤数、行情源、最高评分、模型复核数和
数据状态；每行可见关键分数、动作、
