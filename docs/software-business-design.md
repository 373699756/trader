...
候选定向特征覆盖请求代码 100%，受支持板块且无身份缺失限制的证券身份覆盖 100%，不少于
20 个历史交易日且 20 日成交额摘要有限为正的历史覆盖至少 99%。覆盖计数和比例是批次内存事实，
以加法状态投影公开；任一门禁失败返回受控 `not_ready`，不发布、不进入 DeepSeek、不冻结，
并保留此前已发布的最近有效 current。该复核不读取网络、文件或数据库，也不进入 HTTP 查询。
...
每个评分共享输入批次在确定三板候选代码后必须幂等调度证券主数据、交易日历和候选参考字段
刷新；三条评分策略复用同一次调度，不得各发一轮。该调度显式携带两组代码：免费证券身份使用
本轮规范全市场快照的完整代码集，估值、财务、研究和历史增强只使用有界候选集，禁止因持久化
身份而对全市场触发候选级慢请求。参考刷新在独立来源 lane 异步执行，首次冷启动批次允许按缺失
规则降级，完成后的数据必须进入后续批次并持久化；调度失败不得阻塞本地决策或清空最近有效结果。
免费全市场行情已提供的板块、交易所和上市日期必须覆盖本轮全市场代码并在单个 SQLite 事务中
批量写入 V2 数据平面，不能因候选轮转或 Tushare 积分不足而丢弃；上市交易日数统一复用组合根
注入的生产交易日历计算。相同来源和相同规范载荷的证券主数据幂等跳过写入，重启时恢复后继续
参与合并。
...
行情来源和行情时间；行情覆盖按当前页面完整
名单展示具备有效价格、涨跌幅、来源和来源时间的股票数/名单总数，并分别显示行情与证券身份缺失数；
推荐漏斗是候选、已评分、正式推荐、过滤数、观察数和最高评分的唯一摘要入口，不得与行情覆盖重复；Long
的推荐漏斗明确显示不适用，行情覆盖仍按完整固定观察名单统计；模型预算展示已用、
...
必须继续可见，但正式推荐固定为 0，观察明确标为“观察草稿”，不得把未发布条目伪装成 current 名单。
缺失核心值显示“不可用”或 `—`，不得伪造为 0。Header 运行条不再重复年龄、预算和冻结信息，
...
证券主数据、有效历史、过滤通过/观察/拒绝、完成评分、DeepSeek 可审、可执行/观察动作及最终两池
入选数量；同级 `summary` 以交易日绑定并只公开请求总数、有效行情数、行情缺失数、证券身份缺失数、最新行情来源/
源时间和最高最终分，不得公开股票代码或逐股值。状态还必须安全投影内存中的全市场/候选行情年龄、
来源健康和历史预热聚合，但丢弃逐股 missing/conflict、外部错误文本和载荷。每个策略同时公开一个
