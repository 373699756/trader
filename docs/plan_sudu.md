...
状态：非权威执行计划，已于 2026-07-22 按原子批次实施。
...
- `PublishedSnapshotIndex` 和 P1-P6 热读路径已经按本计划形成活动实现；实施前的
...
本计划未拆成多个独立“继续”批次；它是权威设计第 7.2 节 P1-P6 章节的内部施工
...
- 256 MiB 可以在后续有测量证据时调整，但本章节先证明没有泄漏、无界队列或重复全量副本；
  增大内存不是替代数据生命周期治理的手段。
- 该计划已实施；活动代码使用列式变更集合、P6 热索引和 schema-v2 SSE patch，实际门禁
  结果记录在对应批次 `CHANGELOG.md`，本文不形成第二套验收事实。
