...
- [ ] 每组候选和本地权重精确合计1.0。
- [ ] today、tomorrow、d25三策略与三板共九组候选和本地评分策略全部存在。
- [ ] 三板横截面完全隔离，不借用其他板分位。
...
- [ ] 候选分49.99/50.00、核心缺失30%边界和每板119/120/121只处理正确，且不足不补满。
- [ ] d25路径不存在双乘数。
- [ ] 固定融合向量仍得到83.40。
- [ ] today主执行69.99/70.00和降级执行75.99/76.00边界正确。
- [ ] tomorrow 77.99/78.00和d25 75.99/76.00边界正确。
- [ ] `top_n=0/1/10/18` 时单板 `ceil(top_n * 60%)` 边界正确。
- [ ] 主板竞争组第3/4只、创业板和科创板第2/3只边界正确。
- [ ] 减持1/2/3级、终止未实施、解禁边界和单来源失败保持null全部正确。
- [ ] 七项本地风险边界和5/4/3/3/4/3/3扣分正确，同一事实不重复且总扣分截断为25。
- [ ] 一个板块失败时不发布偏置的新TopK。
...
- [ ] today已按v16三板评分启用；long、冻结时间、SSE和旧快照兼容行为不变。
