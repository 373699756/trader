...
`production_authority=false` 和 `automatic_model_update=false`。该授权允许显式 V2/V3 工程评分，不代表历史收益
验证或默认生产切换。V2/V3 激活都是三头原子启动门：三个固定工件组全部校验成功才可接线；旧包内 V2 Tomorrow 模型和
...
日常启动只需执行 `./run.sh`，默认读取 `config/strategy.json` 的全局 `scoring_profile=v1`。
`./run.sh --profile v2` 与 `./run.sh --profile v3` 启动都必须分别读取项目
...
| 日常 | `./run.sh --profile v2` / `--profile v3` | 以显式 V2/V3 启动；`check` 也可选档位，但 `download_history` 不接受评分档位 |
...
用户显式 `--profile v3` 才能加载符合新配对合同的工件；默认 V1、V2、
冻结、DeepSeek 预算和下载续传行为不变。
