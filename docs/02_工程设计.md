...
档位声明的预测能力另通过 `application/ports/model_scoring.py` 的有类型端口和 `ModelScoringRouter` 注入，
...
| `FeatureComputationPlan` | `application/market_data`；按 catalog 依赖把 manifest 解析为确定性分组计算计划 | catalog 与向量 manifest -> 有序计算阶段和必需事实身份 | 不拥有公式，不跨过冻结截止发布迟到结果 |
