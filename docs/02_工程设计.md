...
`check`、`download_history`、`train-tomorrow`、`train-v3`、`install-history-automation` 和
...
明确引用的月份，每次至多打开一个月库；单股接口只接受最多 61 个有序实际交易日，训练扫描按日期/代码流式
读取，并为每只代码仅保留连续 61 行。训练只在当前 snapshot 通过验证后创建 `.sample-workspace.*` 一次性样本库
...
当日及后续最多 60 个特征依赖日，任一视图无法读取则 `data_incomplete`。纯压实后 bundle 的旧 snapshot hash
...
| 离线训练 | `./run.sh train-v3` | 一次校验 active snapshot、一次顺序历史扫描写一个多目标临时 SQLite，再按 Today、Tomorrow、D25 顺序拟合到期 head；三头不并行，2 线程、2048 MiB 上限不变，不能用三次独立扫描代替 |
...
`train-tomorrow` 与 `train-v3` 的交互式 stderr 使用有类型进度的紧凑边界投影，不输出内部 JSON：资源策略、输入分片校验、
...
顺序扫描必须复用已验证分片。完整训练按月份、日期、代码顺序单遍读取，只为每只股票保留最长 61 日滚动状态，
禁止恢复“全部股票 × 全部月份”的逐股小查询，也禁止以可观测性修复掩盖重复全量校验。

该底层阶段只由 `./run.sh check` 内部调用，不是公开 `run.sh` 命令。用户离线工作流仍只有一个历史下载入口，
训练则明确分为仅 Tomorrow 的 `train-tomorrow` 与一次扫描三头的 `train-v3`，不得要求再执行第三条研究状态命令。

当前 `check` 组合命令、历史自动化、`train-tomorrow` 及 `train-v3` 均由 `trader-cli` 单一编排器
