...
研究交接不在本文维护，统一见 [工程实施](03_工程实施.md)。

`download_history` 默认 `--mode snapshot`，保持原封存归档创建/续传语义；显式 `--mode update` 先消费与
`history-plan` 同一有类型缺口计划，再把缺失字段写入父归档旁的独立增量分片。旧 92 个 SQLite 分片和父 manifest 原字节不变。
增量按“代码、交易日、字段族”保存 checkpoint 与内容 hash：同一 key 同内容幂等，
不同内容失败关闭，父归档绝不写入。raw 请求已返回 isST 时不得重复请求；只有单独缺 isST 才使用
`date,code,tradestatus,isST` 请求。行业只接受真实 `updateDate`；不能提供 `effective_at`/`published_at` 的
资格、硬过滤和风险事实继续记为缺失，不得用当前快照回填历史。

增量先写 `.increment-staging` SQLite，使用 FULL synchronous、WAL checkpoint、完整性检查和整文件 SHA-256，
再移动为以 increment manifest hash 命名的不可变 generation，最后通过临时文件、flush、fsync 和原子替换切换
`active-manifest.json`。active manifest 绑定 `parent_manifest_hash`、`increment_manifest_hash`、
`parent_manifest_file_hash`、`active_data_hash`、`source_cutoff`、交易日历、来源身份、逐字段覆盖和缺失原因，并固定
`point_in_time_parity=false`、`production_authority=false`。中断或供应商失败只保留可重试 staging，损坏、
父 hash 漂移或同 key 冲突均不得切换 active；已有 active 仍是上一有效读取面。父加增量的有类型只读视图对旧 key
读父分片、新 key 读增量分片；两层同时存在时仅接受相同规范内容，不允许静默覆盖。
活动归档上下文必须保存滚动窗口内精确、有序、唯一的交易日元组，并校验其最后一日等于动态 `source_cutoff`、
其规范 hash 等于 active manifest 的 `calendar_hash`；股票代码总体只从同一上下文的分片映射投影。当前稳定
`baostock_daily_core` 类型允许显式注入该动态截止日，日线、ST、行业和覆盖日期均由所属归档上下文校验，禁止再
读取进程级固定截止常量。旧 `score_baostock_daily_core_v2` 截止日仍固定为 2026-08-31，只允许解码既有父工件；
不得重写父归档或生成新的旧身份。活动归档向训练边界提供绑定 `active_data_hash`、动态截止日、完整字段表和
滚动日数的冻结输入描述；实际逐行训练消费与三件工件整组发布由独立训练重建批次完成。
供应商对明确请求的 qfq 仍返回未复权身份时，不得改标为 qfq；该 key 保留
`supplier_adjustment_unavailable` failed checkpoint，其余合法 key 正常封存。只有这种已证明的确定性
缺失允许发布 `completed_with_failures` active 读取面并在字段覆盖中继续显式缺失；网络、超时、
未知载荷或冲突仍不得切换 active。
...
| 离线研究 | `./run.sh download_history [--runtime-dir <路径>] --sessions 1..2000 [--mode snapshot\|update]` | 显式安装 `[research]` extra 后，snapshot 创建/续传封存 BaoStock 归档；update 只补计划缺口并原子发布父加增量 active manifest；默认写入 Git 忽略的 `data/history/`，也可显式指定其它目录 |
...
当前工作区的 2026-09-10 运行证据是：旧布局 22 个根级 SQLite 分片、5453 只总体中 268 只完成、5185 只
未完成、training-ready 0，且 `catalog.sqlite3`、父 `manifest.json`、`active-manifest.json` 与训练
`active-bundle.json` 均不存在。状态必须投影为 `completed_with_failures/incomplete_codes`，V3 loader 必须返回
模型不可用，不能把旧根级模型/报告或任意 258 只子集当作活动工件。中断任务产生的空新分区应回退或隔离，避免
遮蔽仍可续传的旧 checkpoint；这项运行现状不放宽上述完整 manifest 和单一活动指针契约。
...
`history-plan` profile 只读校验封存 BaoStock manifest 与 SQLite 分片，查询交易日历后输出活动窗口、逐字段族
复用/缺失计数和新增请求估算；逐股日期明细只能写到显式仓库外路径。该 profile 不下载日线、不打开 SQLite
写连接、不改写父 manifest 或训练工件，历史资格、硬过滤、风险和缺少真实生效时间的行业事实必须继续报告缺失。
其计划构造器也是 `download_history --mode update` 的唯一缺口所有者；诊断 adapter 只负责交易日历查询和
白名单 JSON 投影，不复制父分片扫描逻辑。
默认 `live` 合并运行 Web 与来源探针，`full` 再追加浏览器与性能。总体 `failed` 表示至少一个检查未能执行或门禁失败；
`degraded` 表示全部检查完成但存在受控降级；两者都必须保留各子检查状态和发现，不能用首个错误掩盖
其他边界。`web/history/history-plan/tencent/tushare/research/browser/performance` 精确 profile 复用相同权威实现；统一入口不得
...
`tomorrow_training_input` 是冻结日线输入的稳定语义契约名，绑定父 manifest hash、字段、单位与来源身份；
