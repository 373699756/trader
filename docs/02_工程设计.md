...
结果结算用例、离线研究用例及 profile 证据端口位于 `training/evaluation/application`；离线历史研究和结果评价值对象位于
`training/evaluation/domain`。V2/V3 双头工件由
`training/infra/artifacts` 统一校验、按档位定位、发布和推理，不反向依赖离线研究对象。
跨业务使用的规范化字段、规范 JSON 和不可变发布 primitive 由 `infra/serialization` 单一拥有。
`recommendation/infra/deepseek/candidate_feature_evidence.py` 归属 DeepSeek 证据渲染；`training/evaluation/application/__init__.py`、`training/evaluation/domain/__init__.py` 和
...
- `recommendation/domain`：按 `market`、`candidate`、`scoring`、`risk`、`evidence`、`selection`、
  ...
  离线研究与结果评价由 `training/evaluation/domain` 拥有。领域包不得读取配置、
...
V2/V3 双头都采用已经由用户明确授权的日线工程代理：Tomorrow/D25 的运行锚点是 14:50，训练锚点
...
| --- | --- | --- | --- |
| `CanonicalOutcomeEvaluator` | `training/evaluation/domain`；唯一计算进场、退出、复权、可交易性、基准、成本和多 horizon 结算 | 有类型锚点、交易日路径、价格路径、基准人口与成本假设 -> 不可变 outcome | 不读数据库/网络/时钟，不修改决策、冻结或评分 |
...
| `PointInTimeDataQualificationReport` | `training/evaluation/domain`；独立聚合日线、历史行业和历史分钟三门资格 | 有类型归档状态与来源能力证据 -> 不可变合格/不足报告 | 不下载全市场、不构造样本、不开留出、不授权训练或生产 |
| `PointInTimeDatasetBuilder` | `training/evaluation/application`；复用生产纯领域选择和统一 outcome，按日重建 Tomorrow 14:50 总体 | 三门资格、点时 facts、统一特征 manifest、日期切分与选择策略 -> 不可变日分片及父 manifest | 资格不足时不读来源；不读取终端留出，不接生产组合根 |
...
| `CandidateRecallLedger` | `training/evaluation/domain` 与 `training/evaluation/application`；绑定点时数据行与完整后续漏斗，记录首次拒绝边界和历史 oracle TopK recall | 点时数据集、行 hash、评分/风险/动作/集中度追踪及累计延迟证据 -> 不可变日级与微汇总归因报告 | 不读取终端留出，不把未来收益送回线上候选，不放宽生产过滤 |
| `CandidateCapacityAndRankingStudy` | `training/evaluation/domain` 与 `training/evaluation/application`；在开发集比较预注册容量族与单一排序候选，并在确认集冻结复核 | 合格点时人口、候选召回账本、容量/排序规范和性能证据 -> 唯一候选或失败关闭终态 | 不读取终端留出，不看确认结果换赢家，不写生产配置，不把 Tomorrow 结论外推到其它策略 |
| `LimitedFactorFamilyResearch` | `training/evaluation/domain` 与 `training/evaluation/application`；一次只评价一个预注册参数族和一个预选候选 | 点时数据集、候选召回、固定六项 alpha 对照、日级配对证据 -> Holm/多成本/风险非劣终态报告 | 父证据不足时零读取，不看结果换候选，不打开终端留出或接生产 |
...
| `SelectionUtility` | `training/evaluation/domain`；比较 alpha、风险、成本和约束的隔离挑战者 | 有类型 alpha/risk/cost -> 研究排序与机会成本 | `SelectionUtility` 首先只属于隔离研究与 Shadow；未经授权不得接生产组合根 |
| `ShadowMonitor` | `training/evaluation/application`；聚合全候选漂移、延迟、召回、预测与成熟结果 | 只读生产证据与隔离 outcome -> 有界诊断状态 | 不评分、不发推荐、不写冻结、不自动晋级 |

`RiskCostUncertaintyResearchBuilder` 位于 `training/evaluation/application`，只接受有类型父资格和只读 evidence port。
