...
评分 profile 逐股复核：V1/V2 的 Tomorrow 以及 V3 的 Today/Tomorrow/D25 模型头在线推理至少需要 61 个
...
可审计。该复核不读取网络、文件或数据库，也不进入 HTTP 查询。V1/V2 使用已封存模型，不要求用户下载
离线训练历史；V3 离线训练最多消费 2000 个交易所开市日且完整训练至少要求 1250 个共同完整交易日，不能把
这个离线窗口误写成盘中评分窗口。
...
`application/research`；离线历史研究工件位于 `domain/research`，生产侧通过
...
CLI `--profile` 只形成不写回文件的进程级覆盖。公开 `tomorrow_model` 状态 JSON 在多头状态切换章节前保持不变。
...
V3 三头采用已经由用户明确授权的日线工程代理：Today 的运行锚点是 11:20，Tomorrow/D25 是 14:50，训练锚点
均为 15:00 收盘代理；因此三头固定 `historical_data_insufficient`、`point_in_time_parity=false`、
`production_authority=false` 和 `automatic_model_update=false`。该授权允许显式 V3 工程评分，不代表历史收益
验证或默认生产切换。完整 V3 激活是三头原子启动门：三个固定工件组全部校验成功才可接线；实施完成前现有
Tomorrow-only V3 仍是过渡中的活动能力，不得被文档或状态宣称为完整三头。公共风险、融合、动作、排名和
冻结链不得因此复制策略分支。
...
约束的有限参数研究 → V3 训练与开发/确认 → 相同 calculator 的增量盘中推理 → 独立风险、成本、不确定性与
DeepSeek 策略确认 → 完整选择链冻结后一次性终端留出 → Shadow。V3 模型工件只能接收已经通过确认门的 alpha；数据质量只形成资格，市场/板块/行业/流动性默认
...
`check`、`download_history`、`train-tomorrow`、`install-history-automation` 和
...
`./run.sh --profile v2` 显式选择 V2；完整三头切换完成后的 `./run.sh --profile v3` 启动必须分别读取项目
...
`schema_version`、业务身份、文件路径或机器信息；`source_commit` 仅作为训练源码审计身份，不参与训练合同 hash。
`training-input.json` 不参与算分，
但必须先通过 codec。活动提交标记、任一文件缺少、损坏或
配对不一致时在组合根失败关闭并给出稳定原因，
三组中任一组失败都使显式 V3 启动失败，不自动回退 V1/V2，也不只启用部分 head。该覆盖只影响本次进程并重新生成
...
| 离线训练 | `./run.sh train-v3` | 一次校验 active snapshot、一次顺序历史扫描写一个多目标临时 SQLite，再按 Today、Tomorrow、D25 顺序拟合到期 head；三头不并行，2 线程、2048 MiB 上限不变。该命令在统一入口章节交付前不可用，不能用三次独立扫描代替 |
...
执行资源合同变化时为 `training_contract_due`，普通文档提交和审计用 `source_commit` 变化不触发重训；输入不完整或活动基线领先当前输入时为
...
当前工作机尚未执行形成可训练 active snapshot 的真实新链下载；代码现已能建立新控制库、稳定年月月库和
active snapshot，训练入口也只接受该读取面。V3 loader 不会把旧训练根级模型/报告当作活动工件；完整三头 loader
在 Today/D25 工件和运行接线章节交付前保持未启用。这项运行现状不放宽完整 snapshot、来源 identity、
`label_cutoff` 和每 head 单一活动提交标记契约。
...
一次 active snapshot 及来源身份，逐股读取不重复对全部分片做整文件 hash；样本扫描只缓冲一个交易日并在共享暴露
上下文残差化后直接写最终表，不创建 `raw_samples`；`sample_split_dates` 一次聚合行业计数，每行业单次读取训练、早停和
校准矩阵，validation 只计数，训练结束删除工作库。历史分片可混合保存
稳定 spec 与只读旧 spec 身份；状态和训练只允许先按已存身份校验，再投影稳定语义身份，不迁移或改写旧字节。

`train-tomorrow` 的交互式 stderr 使用有类型进度的紧凑边界投影，不输出内部 JSON：资源策略、输入分片校验、
...
只由命令结果投影输出一次最终 `Tomorrow训练` 行，训练内部阶段不得重复输出相同完成行。训练固定最多两个计算线程，
...
和一个 Tomorrow 训练入口，不得要求再执行第三条研究状态命令。

当前 `check` 组合命令、历史自动化及 `train-tomorrow` 均由 `trader-cli` 单一编排器
