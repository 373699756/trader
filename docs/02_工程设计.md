...
Web 推荐漏斗必须逐层显示总体、发行资格、动态过滤、策略历史、模型输入、候选分、板内限额、行情请求、
候选特征、行情资格、证券资料、候选历史、过滤三种终态、完整评分、模型复核、观察/正式门槛、动作三种终态
以及正式/观察入选数；任一节点未知显示 `—`，不得再用三个汇总数字掩盖中间断点。当前状态与已保留推荐快照
并存时，漏斗优先展示当前交易日的 `input_quality`，正式记录仍由发布状态和推荐表单独表达。
