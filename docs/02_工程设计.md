...
| `FeatureComputationPlan` | `application/market_data`；根据输入变更集合形成有界依赖失效与批量计算计划 | 事实修订与 catalog -> 有序计算批次和逐阶段耗时 | 不拥有公式，不跨过冻结截止发布迟到结果 |
...
增量计算只在源事实 revision 或 catalog 依赖发生变化时失效相关特征组；同一输入身份必须产生同一向量和
hash。计算计划必须有总候选上限、每组耗时、取消边界、latest-wins 与冻结 deadline，并在错误时保留最近
有效快照和明确降级。性能优化不得改变 V1/V2/V3 黄金向量、固定融合、风险只扣一次、Top6 或冻结语义。
...
隔离研究按 `CanonicalOutcomeEvaluator -> CandidateRecallLedger -> 有限参数族消融 -> ShadowMonitor` 顺序消费
证据。`ShadowMonitor` 可以比较 local-only、结构化 facts veto 与固定融合，但只能输出聚合漂移、校准、成本后
收益和尾部风险报告；`SelectionUtility` 不能成为活动生产依赖。历史分钟来源必须在使用前证明 11:20/14:50
点时可得性与内容身份；能力不足时以稳定 blocker 失败关闭，不得用 15:00 收盘代理、未来采集或线上结果回灌。
