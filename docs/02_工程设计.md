...
验证或默认生产切换。完整 V3 激活是三头原子启动门：三个固定工件组全部校验成功才可接线；显式 V3 现已只接受
完整三头，旧 Tomorrow-only V3 工件不再构成可启动档位。公共风险、融合、动作、排名和
冻结链不得因此复制策略分支。
...
`./run.sh --profile v2` 显式选择 V2；`./run.sh --profile v3` 启动必须分别读取项目
