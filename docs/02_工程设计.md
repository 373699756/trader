...
同样不兼容 `data/history/baostock-daily/sessions-*`、父加增量 manifest 或旧参数；新链只读取
`data/history/baostock/control.sqlite3` 与 `partitions/YYYY/MM/<sha256>.sqlite3`。仓库级一次性运维脚本
`scripts/convert_baostock_history.py` 是唯一例外：它只在用户显式执行时旁路读取已封存旧历史并生成新目录，
不进入活动产品、启动、恢复、Web、`check`、训练或 `download_history` 调用链。
...
`baostock_partition_archive.py` 提供分区归档门面，`baostock_history_legacy.py` 只在下载锁内把根级旧 checkpoint
迁入有界分区并保留 recovery 副本。状态读取在 manifest 形成前同时识别当前分区和根级旧 checkpoint，已完成
代码覆盖同代码的陈旧失败；迁移不由 Web、状态查询或资格审计触发。公开类型和调用方式保持稳定，不增加
聚合 `utils` 模块。
...
发布状态卡之后紧跟“评分最高”卡，按当前快照全部已评分候选的最终评分降序、本地评分降序、代码升序
稳定展示最多三只股票的“分数 - 股票代码 - 股票名称”；未就绪时使用同一快照观察草稿中的 `top_scores`。
若没有已评分候选但评分诊断仍有最高最终分，显示该最高分并明确当前无达到观察门槛的股票；Long、缺失
快照或无有效最终分时显示无可用评分数据。
...
默认只读源为 `data/history/baostock-daily/sessions-2000`，目标为 `data/history/baostock`。脚本逐月串行、每批
256 行、每个 SQLite 连接缓存 8 MiB、SQLite `threads=1` 且关闭 mmap，每批和哈希块主动让步 5 ms，并在
POSIX 将自身 nice 值增加 10；磁盘必须至少容纳源分片总大小并额外保留 2048 MiB。源校验、逐月转换和补缺下载
都向标准错误输出阶段、当前月份/请求、百分比、已处理数量和耗时；完整月份记录在独立 staging 进度库中，
中断后可续传，最终控制库只由 `SQLiteHistoryControlRepository` 写入，月库只遵循阶段 C schema，全部 hash/
完整性通过后才原子发布目标目录，源目录不修改。默认仅扫描并下载可由 BaoStock 历史日线证明的
`raw/qfq/is_st` 空洞，以单个受控 SDK 子进程、至少 2 秒请求间隔、单次 60 秒硬超时和最多 2 次重试执行；
`--offline` 可禁止联网但会在摘要保留 `remaining_downloadable_gaps`。行业缺失以及资格、硬过滤和风险事实仍须
有真实 `effective_at`/`published_at`，不得由转换脚本用当前状态回填。该一次性桥接不进入公开命令的供应商
调用链；其已发布 snapshot 可由阶段 D 日常缺口、5 日回读和 active snapshot 维护继续使用。
...
`partitions/YYYY/MM/<sha256>.sqlite3`，先旁路构建、完整性/内容 hash 校验，再原子发布。内容寻址
（content-addressed）文件保证旧
snapshot 在同月发生 revision 后仍能按原 hash 重放；全部目标月完成后才切换控制库 active 指针。每行按
...
当前项和稳定错误码，不得被外层 `loading_context` 覆盖。供应商等待期间至少每 5 秒 flush 一次心跳。单次供应商调用从开始到
...
当前工作机尚未执行形成可训练 active snapshot 的真实新链下载；代码现已能建立新控制库、内容寻址月库和
...
旧 `history-plan` profile 在清理阶段前只可只读识别旧归档，不再拥有下载计划或公开命令；阶段 G 必须随旧父加
增量实现删除。新的缺口计划只能由阶段 B 控制面和阶段 C 月分片的有类型读取面拥有，诊断 adapter 只做白名单
JSON 投影。
默认 `live` 合并运行 Web 与来源探针，`full` 再追加浏览器与性能。总体 `failed` 表示至少一个检查未能执行或门禁失败；
`degraded` 表示全部检查完成但存在受控降级；两者都必须保留各子检查状态和发现，不能用首个错误掩盖
其他边界。`web/history/history-plan/history-daily-capability/tencent/tushare/research/browser/performance` 精确 profile 复用相同权威实现；统一入口不得
...
旧 BaoStock 父加增量组件、参数化下载入口、generation manifest 和对应兼容读取均不再属于活动合同；
`download_history`、训练、Web、HTTP 与调度器不得创建或扩展旧目录。工作机已有封存旧归档只能由
`scripts/convert_baostock_history.py` 显式只读转换，不能成为双读 fallback。阶段 G 最终切换时删除已经没有
消费者的旧源码、codec 和测试；在此之前也不得把它们重新接入任何活动入口。
