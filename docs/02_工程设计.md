...
新 release 不读取旧运行目录、旧数据库、旧快照或旧 schema，也不提供迁移导入命令。历史下载同样不兼容
`data/history/baostock-daily/sessions-*`、父加增量 manifest 或旧参数；用户已清空历史目录，新链只创建
`data/history/baostock/control.sqlite3` 与 `partitions/YYYY/MM.sqlite3`。
...
当前工作区的 2026-09-10 运行证据是：用户已清空 `data/history`，不存在旧分片、控制库、月库或 active
snapshot；零参数下载返回 `history_sync_pending` 且目录仍为空。V3 loader 必须继续返回模型不可用，
