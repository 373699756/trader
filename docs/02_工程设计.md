...
收益保证。评分档位只允许 V1/V2/V3；其模型身份、历史结论、人工授权和禁止自动更新边界分别以
...
评分 profile 逐股复核：V1 的 Tomorrow 与当前活动 V2/V3 三头在线推理至少需要 61 个有效 qfq 交易日及各自
manifest 必需字段；目标 V2 在线推理最多读取 251 个有效 qfq 交易日以支持 120/250 日长期趋势和市场状态，目标
V3 在线推理仍至少需要 61 个有效 qfq 交易日。没有模型头的策略使用各自登记窗口和字段。覆盖计数和比例是批次内存事实并以加法状态公开，但不再因低于统一百分比而否决已有
...
可审计。该复核不读取网络、文件或数据库，也不进入 HTTP 查询。V1 使用包内封存模型；V2/V3 启动时只加载
...
候选定向行情只请求三策略活动窗口的稳定去重并集；返回后用同一领域规划器在原板内总体上复核，失效项沿
...
5. Today、Tomorrow、D25 均由相同领域所有者按 0/50/100 不利/中性/有利锚点计算证据质量
...
记录。活动组合根必须接入 tomorrow、today、d25 的统一核心、原生 worker、正式记录仓储、
...
`DecisionView` 必须对 today、tomorrow、d25 和 long 使用同一 envelope；评分策略
...
统一数据平面、决策核心、运行时、today、tomorrow、d25、long、统一 `/api/*`、
...
`training/evaluation/domain`。V2/V3 三头工件由
...
共同调用的别名模块或 Today 包装层。三个短周期策略的统一证据质量分都由公共领域评分器按各自板块权重生成。
...
`domain/recommendation` 的对应 owner 持有，应用边界不得复制公式或引入第二条生产链。
...
策略能力，不判断 V1/V2/V3 算法，也不接受 long。模型不可用时未声明模型头的策略继续按正式规则评分，已声明
模型头的损坏不得静默切回其它模型。旧通用 today/tomorrow/d25 评分器、权重配置
...
生产评分档位基础设施位于 `infra/scoring`；V1 的 JSON codec、授权 hash、线性 Tomorrow predictor 和状态证据由
`profiles/v1` 独立拥有。`training/infra/artifacts` 是 V2/V3 三头合同、codec、locator、repository、predictor 和 profile
组装的唯一中立 owner；实现框架共用，工件所有权分档。目标 locator 必须由显式评分档位选择
`data/train/v2/{today,tomorrow,d25}` 或 `data/train/v3/{today,tomorrow,d25}`，codec 校验 model ID、feature manifest
...
V2/V3 三头都采用已经由用户明确授权的日线工程代理：Today 的运行锚点是 11:20，Tomorrow/D25 是 14:50，训练锚点
均为 15:00 收盘代理；因此三头固定 `historical_data_insufficient`、`point_in_time_parity=false`、
`production_authority=false` 和 `automatic_model_update=false`。该授权允许 V2/V3 工程评分并由用户明确选择 V2 为默认档位，
但不把当前日线代理写成已通过历史收益验证。每档激活都是三头原子启动门：所选档位三个固定工件组全部校验成功才可接线；旧包内 V2 Tomorrow 模型和
Tomorrow-only V3 工件不再构成可启动档位。公共风险、融合、动作、排名和
冻结链不得因此复制策略分支。
...
约束的有限参数研究 → 分档三头训练与开发/确认 → 相同 calculator 的增量盘中推理 → 独立风险、成本、不确定性与
...
优化都不得改变 V1/V2/V3 黄金向量、固定融合、风险只扣一次、Top6 或冻结语义。
...
| today/tomorrow/d25 纯本地评分 lane | 各策略单 worker | 每策略一个 running 加一个 pending | 不在评分 worker 中等待模型或抓网络 |
...
活动窗口合并为稳定去重的物理请求，最坏并集为 1080 只，但任一策略的逻辑候选仍为每板最多 120 只。
...
候选池按 `strategy + board` 预选执行，每策略每板最多 120 只、三板合计最多 360 只；三策略窗口完全不重合时
物理报价并集最多 1080 只。运行配置中的 `candidate_pool_size=120` 是单策略单板上限，候选报价缓存容量为
1080，不得为满足性能而静默缩减某一策略窗口。
...
三策略窗口的腾讯候选报价先形成稳定去重并集，完全不重合时最多 1080 只；允许按供应商批次上限形成有界
...
固定 P95 上限为：5500 行标准化 250ms、两源合并 600ms、统一快照可读 900ms、1080 行最坏候选并集
报价投影 100ms、5500 行完整人口的三策略候选规划 5s、单板单策略评分 250ms、三板三策略墙钟 1000ms、
...
20/50/100bp 参数化运行，但活动监控仓储只保存 20bp 主口径。Today/Tomorrow 只结算 T+1，D25 固定结算
...
| --- | --- | --- | --- |
| `GET /` | 无 | 统一桌面工作台 | 无 |
| `GET /api/decisions/<strategy>/current` | `strategy=today|tomorrow|d25|long`，可选 `If-None-Match` | `decision_view`；当前决策、可选 draft、overlay、coverage、ETag | 只读 |
| `GET /api/decisions/<strategy>/history?date=YYYY-MM-DD` | 荐股策略和日期 | 精确日期的正式 `decision_view`；long 返回 `history_not_supported` | 只读 |
| `GET /api/decisions/<strategy>/dates` | 荐股策略 | 按交易日倒序的正式记录日期；long 返回 `history_not_supported` | 只读 |
...
`setup_type` 与 `downside`；任何由所选档位声明模型头的 Today/Tomorrow/D25 项另外公开
...
Today、Tomorrow、D25 的新决策均从 `input_versions.score_scale` 公开
...
旧 Tomorrow 冻结记录继续按旧模型相对分展示，不得标为统一评分或原地迁移；旧 Today/D25 记录没有模型身份时
保持原样，不按当前 V3 head 回填。
...
三档桌面分辨率下评分输入区高度固定，长链在区内换行或滚动，不得产生页面级横向溢出。股票列表摘要固定在四策略
...
Web 中三个短线策略的独立可见标签固定为“今”“明”“2-5”；策略解释、阶段名称和冻结语义仍使用完整文字，
不得因缩短标签改变 API 策略键、评分、日期或冻结边界。
页面不再设置独立“统一评分最高”卡；推荐漏斗内明确今、明、2–5 日共享 0–100 证据质量标尺，并按当前快照全部已评分候选的最终评分降序、本地评分降序、代码升序
...
估算成本、预测成本后净超额和模型分歧；当前 V1/V2/V3 都没有逐股亏损概率输出，页面不得用历史总体严重亏损率
...
`no_positive_net_utility`。模型预测超额收益仍按同批横截面形成 0–100 相对诊断，不能覆盖三个短周期策略
...
完整多头切换后，`scoring_profile` 状态按 `profile_id` 和 `heads.today|tomorrow|d25` 白名单投影；每个 head 的
...
启动脚本无参数时不传入隐藏档位覆盖；`--profile v1|v2|v3` 只在用户显式指定时传入当前进程。
当前 release 的默认 V2 与 `./run.sh --profile v3` 都读取项目
`data/train/today-v3/`、`data/train/tomorrow-v3/`、`data/train/d25-v3/` 中的 `active-bundle.json`，再校验
...
三组中任一组失败都使所选 V2/V3 启动失败，不自动回退 V1 或旧 V2，也不只启用部分 head。该覆盖只影响本次进程并重新生成
...
目标训练布局为 `data/train/v2/{today,tomorrow,d25}/` 与 `data/train/v3/{today,tomorrow,d25}/`。两档目录现在都提交
各自真实生成的四件固定工件和活动 bundle hash；V2 工件使用 V2 特征合同，不能由 V3 文件复制或伪造。训练工件已纳入
仓库交付，但这不等于运行 loader 自动切换：loader、due 状态、locator、codec、CLI、性能入口和三头原子加载仍须在
独立 cutover 批次中完成一致性验证。切换前不把目标目录误认为生产来源；切换完成后再删除旧目录和临时兼容逻辑，使每档只有一个真相源。

启动脚本公开命令分为：

| 类别 | 命令 | 运行边界 |
| --- | --- | --- |
| 日常 | `./run.sh` | 以配置中的默认 V2 加载当前活动三头工件，启动本地 Web 看板和生产调度 |
| 日常 | `./run.sh --profile v1|v2|v3` | 显式覆盖当前进程档位；`check` 也可选档位，但 `download` 不接受评分档位 |
...
| 当前离线训练结果 | `data/train/{today-v3,tomorrow-v3,d25-v3}/` | 当前 release 的唯一活动三头；每个目录稳态只保留 `training-input.json`、`report.json`、`model.json`、`active-bundle.json` 四个固定文件。后续原子切换完成即删除 |
| 目标离线训练结果 | `data/train/{v2,v3}/{today,tomorrow,d25}/` | 每档每头保留相同四件配对工件，不创建 hash 或 generation 目录；V2 与 V3 分别拥有 manifest/model/bundle hash，只允许共享父 history snapshot。V2 三头全部有效前不得把空目录接入当前启动 |
...
| 离线训练 | `./run.sh train-v2` | 使用 V2 独立合同读取 251 日窗口并计算 120/250 日长期特征，一次扫描顺序训练三头，写入 `data/train/v2/{today,tomorrow,d25}` |
| 离线训练 | `./run.sh train-v3` | 使用 V3 独立 61 日合同，一次校验 active snapshot、一次顺序历史扫描写一个多目标临时 SQLite，再按 Today、Tomorrow、D25 顺序拟合到期 head，写入 `data/train/v3/{today,tomorrow,d25}` |
...
`daily_close_engineering_proxy`：训练锚点 `15:00_close_proxy`；Today 运行锚点 `11:20`，Tomorrow/D25 运行锚点
`14:50`，三个 head 的点时一致性均为假。
当前默认 V2 和显式 `--profile v3` 都加载同一组符合配对合同的旧扁平工件；目标切换后各自加载分档三头，
V1 仍可作显式对照。
档位切换不改变冻结、DeepSeek 预算和下载续传行为。

训练入口只接受控制库声明的 active snapshot 和稳定年月月库读取面，不会把旧训练根级模型/报告当作活动工件。
完整三头 loader 已接通固定 Today/Tomorrow/D25 工件组；这不放宽完整 snapshot、来源 identity、
...
训练明确分为 V2 三头 `train-v2` 与 V3 三头 `train-v3`，不得要求再执行研究状态命令。
