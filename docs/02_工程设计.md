...
阶段 A 已切换该合同，阶段 B 已交付控制面；阶段 C/D 月分片同步尚未闭合期间，合法零参数调用只投影
`history_maintenance_status`，固定 `state=blocked`、`reason=history_monthly_archive_pending`、归档根
...
| 离线研究 | `./run.sh download_history` | 唯一零参数维护入口；阶段 A 当前失败关闭且不写目录，阶段 D 后自动选择初次 2000 日、缺口续传、5 日回读或 `already_current` |
...
snapshot；零参数下载返回 `history_monthly_archive_pending` 且目录仍为空。V3 loader 必须继续返回模型不可用，
