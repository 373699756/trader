...
`infra/scoring/head_bundles` 统一校验、按档位定位、发布和推理，不反向依赖离线研究对象。
...
`download/infra` 提供；`application/training` 的 V2/V3 用例只依赖注入的 `TrainingRunnerPort` 和只读
...
`profiles/v1` 独立拥有。`head_bundles` 是 V2/V3 三头合同、codec、locator、repository、predictor 和 profile
