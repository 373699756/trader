...
约束的有限参数研究 → V3 训练/确认/一次性终端留出 → 相同 calculator 的盘中推理 → 独立风险、成本与固定融合
→ Shadow。V3 模型工件只能接收已经通过确认门的 alpha；数据质量只形成资格，市场/板块/行业/流动性默认
形成暴露，风险和不确定性进入 `RiskDecision`，成本与容量进入 `ExecutionCost`，未来 outcome 只作标签和评价。
任何模块不得把这些职责重新汇总成一列无语义总分。

当前 `FeatureComputationPlan` 只负责稳定依赖解析、分组顺序、必需 facts 和计划 hash；后续
...
数据集 -> CandidateRecallLedger -> 有限参数族消融 -> V3 训练/确认/一次性终端留出 -> 增量同源计算 ->
风险/成本/不确定性与 DeepSeek 消融 -> ShadowMonitor` 顺序消费证据。执行状态和逐章 blocker 只由
