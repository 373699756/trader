...
评分 profile 逐股复核：V1 的 Tomorrow 以及 V2/V3 共享的 Today/Tomorrow/D25 模型头在线推理至少需要 61 个
有效 qfq 交易日及各自 manifest 必需字段；没有模型头的策略使用各自登记窗口和字段。覆盖计数和比例是批次内存事实并以加法状态公开，但不再因低于统一百分比而否决已有
...
可审计。该复核不读取网络、文件或数据库，也不进入 HTTP 查询。V1 使用包内封存模型；V2/V3 启动时只加载已有共享 bundle，不现场训练。
共享 bundle 的离线训练最多消费 2000 个交易所开市日且完整训练至少要求 1250 个共同完整交易日，不能把
这个离线窗口误写成盘中评分窗口。
...
`application/research`；离线历史研究工件位于 `domain/research`。V2/V3 共用的三头工件由
`infra/scoring/head_bundles` 统一校验、定位、发布和推理，不反向依赖离线研究对象。
...
`profiles/v1` 独立拥有。V2/V3 不拥有各自的模型文件或 predictor；`head_bundles` 是三头合同、codec、locator、
repository、predictor 和 profile 组装的唯一中立 owner。选中 V2 或 V3 时只向共享 predictor 注入运行档位身份；
工件 model ID/hash、特征、输入 snapshot 与预测数值必须相同。统一档位选择器不得复制各档实现。
...
V2/V3 共享三头采用已经由用户明确授权的日线工程代理：Today 的运行锚点是 11:20，Tomorrow/D25 是 14:50，训练锚点
...
但不把当前日线代理写成已通过历史收益验证。V2/V3 激活都是三头原子启动门：三个固定工件组全部校验成功才可接线；旧包内 V2 Tomorrow 模型和
...
约束的有限参数研究 → 共享三头训练与开发/确认 → 相同 calculator 的增量盘中推理 → 独立风险、成本、不确定性与
DeepSeek 策略确认 → 完整选择链冻结后一次性终端留出 → Shadow。共享模型工件只能接收已经通过确认门的 alpha；数据质量只形成资格，市场/板块/行业/流动性默认
...
默认 V2 与 `./run.sh --profile v3` 都必须读取项目
...
| 类别 | 命令 | 运行边界 |
| --- | --- | --- |
| 日常 | `./run.sh` | 以配置中的默认 V2 加载共享三头工件，启动本地 Web 看板和生产调度 |
...
| 离线训练结果 | `data/train/{today-v3,tomorrow-v3,d25-v3}/` | 每个目录稳态只保留 `training-input.json`、`report.json`、`model.json`、`active-bundle.json` 四个固定文件，不创建 hash 或 generation 目录。提交标记只保存三件工件 hash 和自身 hash，因此整组可复制到另一台 PC 使用。训练日志、中间证据、checkpoint、staging 和回退目录均必须忽略。成功训练先在隐藏临时目录生成并校验三件 JSON，再以隐藏回退副本和持久化发布日志依次替换，最后替换 `active-bundle.json`；中断时恢复上一完整组或完成已验证的新组 |
...
默认 V2 和显式 `--profile v3` 都加载同一组符合配对合同的工件；V1 仍可作显式对照。
