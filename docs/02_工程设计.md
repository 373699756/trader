...
`application/research`；离线历史研究工件位于 `domain/research`。V2/V3 三头工件由
...
| --- | --- | --- | --- |
| `CanonicalOutcomeEvaluator` | `domain/outcome`；唯一计算进场、退出、复权、可交易性、基准、成本和多 horizon 结算 | 有类型锚点、交易日路径、价格路径、基准人口与成本假设 -> 不可变 outcome | 不读数据库/网络/时钟，不修改决策、冻结或评分 |
...
| `PointInTimeDataQualificationReport` | `domain/research`；独立聚合日线、历史行业和历史分钟三门资格 | 有类型归档状态与来源能力证据 -> 不可变合格/不足报告 | 不下载全市场、不构造样本、不开留出、不授权训练或生产 |
| `PointInTimeDatasetBuilder` | `application/research`；复用生产纯领域选择和统一 outcome，按日重建 Tomorrow 14:50 总体 | 三门资格、点时 facts、统一特征 manifest、日期切分与选择策略 -> 不可变日分片及父 manifest | 资格不足时不读来源；不读取终端留出，不接生产组合根 |
| `PointInTimeDatasetArtifactArchive` | `infra/research`；逐日分片并原子封存点时数据集 | 有类型数据集报告 -> 白名单 JSON、逐日 hash 与根 manifest | 不覆盖冲突工件，不内联全历史为单一大 JSON，不授权训练或生产 |
| `CandidateRecallLedger` | `domain/research` 与 `application/research`；绑定点时数据行与完整后续漏斗，记录首次拒绝边界和历史 oracle TopK recall | 点时数据集、行 hash、评分/风险/动作/集中度追踪及累计延迟证据 -> 不可变日级与微汇总归因报告 | 不读取终端留出，不把未来收益送回线上候选，不放宽生产过滤 |
| `CandidateCapacityAndRankingStudy` | `domain/research` 与 `application/research`；在开发集比较预注册容量族与单一排序候选，并在确认集冻结复核 | 合格点时人口、候选召回账本、容量/排序规范和性能证据 -> 唯一候选或失败关闭终态 | 不读取终端留出，不看确认结果换赢家，不写生产配置，不把 Tomorrow 结论外推到其它策略 |
| `LimitedFactorFamilyResearch` | `domain/research` 与 `application/research`；一次只评价一个预注册参数族和一个预选候选 | 点时数据集、候选召回、固定六项 alpha 对照、日级配对证据 -> Holm/多成本/风险非劣终态报告 | 父证据不足时零读取，不看结果换候选，不打开终端留出或接生产 |
...
| `SelectionUtility` | `domain/research`；比较 alpha、风险、成本和约束的隔离挑战者 | 有类型 alpha/risk/cost -> 研究排序与机会成本 | `SelectionUtility` 首先只属于隔离研究与 Shadow；未经授权不得接生产组合根 |
| `ShadowMonitor` | `application/research`；聚合全候选漂移、延迟、召回、预测与成熟结果 | 只读生产证据与隔离 outcome -> 有界诊断状态 | 不评分、不发推荐、不写冻结、不自动晋级 |

`RiskCostUncertaintyResearchBuilder` 位于 `application/research`，只接受有类型父资格和只读 evidence port。
...
BaoStock 当前行业适配器与 `infra/research` 历史归档、下载和训练职责隔离：它只查询当天全市场
