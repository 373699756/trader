...
active data snapshot、来源 identity、来源 commit、训练输入、特征 manifest、切分、训练合同、
...
| 离线训练 | `./run.sh train-tomorrow [--runtime-dir <路径>]` | 阶段 E 将切换为只接受已发布且完整校验的 active snapshot；stderr 只投影阶段与已处理/总代码数，不接受部分 checkpoint、阶段或 `run_id`，永不自动 promotion 或修改默认档位 |
...
当前工作区在阶段 D 交付时仍有封存旧父/增量归档，且尚未执行真实新链下载；代码现已能建立新控制库、内容
寻址月库和 active snapshot。V3 loader 在阶段 E 切换训练输入前必须继续按现有门禁返回模型不可用，
