...
重复当前计数作为存活心跳；结束摘要覆盖完成、无需训练、已有当前工件、阻塞、拒绝和取消。资源预检只显示
`峰值 RSS 目标 4096 MiB`，不采样实时 RSS；正式内存验收由 4096 MiB 峰值门禁记录。训练固定最多三个计算线程，
BLAS、OpenMP 和 LightGBM 使用同一上限并降低进程调度优先级。
训练持有历史维护锁后，每个月分片只允许执行一次整文件 hash、`quick_check` 和行数验证；同一读取器内后续计数和
