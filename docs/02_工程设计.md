...
发布状态卡之后紧跟“统一评分最高”卡，明确今、明、2–5 日共享 0–100 证据质量标尺，并按当前快照全部已评分候选的最终评分降序、本地评分降序、代码升序
...
`supply_funnel` 还必须直接公开 `issuer_eligible_population`、`dynamic_filter_eligible`、
`strategy_history_eligible`、`model_input_eligible`、`candidate_score_eligible`、
`candidate_limit_selected` 和 `candidate_quote_eligible`；这些阶段字段由类型化候选规划结果逐项投影，
不得用非互斥原因计数相减或由最终评分数倒推。共享物理候选请求的去重并集只进入行情健康，不得覆盖任何
策略自己的阶段分母。候选行情年龄只统计最近一次候选周期实际刷新的代码，缓存中为后继补位或其它周期保留的
旧报价不得污染当前 P50/P95；缓存条目数仍报告真实有界占用。
Web 推荐漏斗主线按总体、发行资格、动态过滤、策略历史、模型输入、候选分、板内限额、行情请求、候选特征、
行情资格、证券资料、候选历史、完整评分、正式门槛、可执行和正式入选逐节点显示数量；观察门槛、观察动作和
观察入选组成独立支线。过滤通过/仅观察/拒绝、模型可复核及动作不可用是同阶段分支统计，不得伪装成连续过滤节点。
任一未知节点显示 `—`，尤其不得把 JSON `null` 转成业务 0；当前状态与已保留推荐快照并存时优先展示当前交易日
...
报告还必须读取顶层受控降级原因、全市场人口数、完整 `supply_funnel` 节点及活动评分档位的三个 head computation。observer 消费失败
