...
评分 profile 逐股复核：Tomorrow V1/V2/V3 在线推理至少需要 61 个有效 qfq 交易日及模型必需字段，其他策略使用各自
...
Tomorrow 的预测能力另通过 `application/ports/model_scoring.py` 的有类型端口和 `ModelScoringRouter` 注入，
...
`setup_type` 与 `downside`；Tomorrow V1/V2/V3 另外公开
...
Tomorrow 的 GET 与 SSE 完整替换另从 `input_versions.score_model` 公开同一模型版本，页面不得以
策略标签或静态文案替代；旧正式记录没有模型身份时只隐藏版本字段，不猜测当前模型。缺少统一标尺身份的
旧 Tomorrow 冻结记录继续按旧模型相对分展示，不得标为统一评分或原地迁移。
...
数据完整度始终按真实覆盖展示。当前可配置 Tomorrow 模型允许展示评分版本、模型相对信号分、预测超额、
...
`tomorrow_model.computation` 只允许包含候选数、请求数、缓存命中数、物理预测批次数、本轮计算/复用组、
...
`./run.sh --profile v2` 显式选择 V2；`./run.sh --profile v3` 启动时先读取项目
`data/train/tomorrow-v3/active-bundle.json`，再校验并读取同目录固定的 `model.json`。该文件承载 V3
实际评分参数；同目录固定的 `report.json` 和 `training-input.json` 都是加载所需的配对证据。三件工件必须共享
...
| 离线训练结果 | `data/train/tomorrow-v3/` | 稳态只保留 `training-input.json`、`report.json`、`model.json`、`active-bundle.json` 四个固定文件，不创建 hash 或 generation 目录。提交标记只保存三件工件 hash 和自身 hash，因此整组可直接复制到另一台 PC 使用。这四个生产文件是 `data/train/` 中唯一允许进入 Git 的白名单，训练日志、中间证据、checkpoint、staging 和回退目录均必须忽略。成功训练先在隐藏临时目录生成并校验三件 JSON，再以隐藏回退副本和持久化发布日志依次替换，最后替换 `active-bundle.json`；中断时新进程失败关闭，下一次训练恢复上一完整组或完成已验证的新组 |
...
`daily_close_engineering_proxy`：训练锚点 `15:00_close_proxy`、运行锚点 `14:50_point_in_time` 且点时一致性为假。
用户显式 `--profile v3` 才能加载符合新配对合同的工件；默认 V1、V2、
冻结、DeepSeek 预算和下载续传行为不变。

当前工作机尚未执行形成可训练 active snapshot 的真实新链下载；代码现已能建立新控制库、稳定年月月库和
active snapshot，训练入口也只接受该读取面。V3 loader 不会把旧训练根级模型/报告当作活动工件；这项运行现状
不放宽完整 snapshot、来源 identity、`label_cutoff` 和单一活动指针契约。
