...
当前 `FeatureComputationPlan` 只负责稳定依赖解析、分组顺序和必需 facts；它是进程内派生计划，不生成无人消费的
独立 content hash。后续
`incremental_feature_computation` 章节才允许基于源事实 revision 失效相关特征组，并增加总候选上限、每组
耗时、取消边界、latest-wins 与冻结 deadline。该后续实现错误时必须保留最近有效快照和明确降级；任何性能
