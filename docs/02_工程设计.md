...
开发工作计划、交付状态和历史证据统一见[工程实施](03_工程实施.md)与 `CHANGELOG.md`；本文只用规范语气
...
`data/history/baostock/control.sqlite3` 与 `partitions/YYYY/MM.sqlite3`。仓库级一次性运维脚本
...
`CHANGELOG.md` 与 `docs/reports/`；开发计划、阻塞和完成状态只由 `03_工程实施.md` 维护。
...
阶段 A 已切换该合同，阶段 B 已交付控制面，阶段 C 已交付月分片读取面；阶段 D 同步尚未闭合期间，合法零参数调用只投影
`history_maintenance_status`，固定 `state=blocked`、`reason=history_sync_pending`、归档根
`data/history/baostock`、`selected_baseline_source=baostock`、`efficient_daily_source=null` 和
`automatic_training=false`，并以 `active_snapshot_hash/data_cutoff/label_cutoff=null`、
`matured_label_days_since_training=0`、`training_due=false/data_incomplete` 表达空输入，返回非零且不联网、不写
文件。不得临时恢复旧下载链绕过 blocker。后续同步只能填充同一有类型状态对象，JSON 由 CLI adapter 白名单
投影；内部调用者不得读取 JSON 字典。
...
内核锁并把竞争稳定映射为 `HistoryMaintenanceAlreadyRunningError`，阶段 D/F 接通命令时再由边界投影为
`already_running`。阶段 B 不联网、不创建真实归档，也不提前实现阶段 C/D。
...
只提供可验证的读取/缓存能力，不创建生产月库、不发布 active snapshot，也不调用供应商；这些属于阶段 D。
...
有真实 `effective_at`/`published_at`，不得由转换脚本用当前状态回填。该一次性桥接不会解除公开
`download_history` 的 `history_sync_pending`；阶段 D 仍负责日常缺口、5 日回读和 active snapshot 维护。
...
`partitions/YYYY/MM.sqlite3`，先旁路构建、完整性/内容 hash 校验，再原子发布。每行按
`(trade_date, code, revision_id)` 唯一，重复内容幂等，不同内容写新 revision；活动指针不因中断、供应商失败、
磁盘不足、月库或控制库损坏而倒退。初次建立滚动 2000 个完整交易日，日更补缺口并回读最近 5 日；公司行动只
...
active data、父 manifest、increment manifest、来源 commit、训练输入、特征 manifest、切分、训练合同、
...
| 离线研究 | `./run.sh download_history` | 唯一零参数维护入口；阶段 A–C 已完成但阶段 D 同步前仍失败关闭且不写目录，阶段 D 后自动选择初次 2000 日、缺口续传、5 日回读或 `already_current` |
...
| 离线训练 | `./run.sh train-tomorrow [--runtime-dir <路径>]` | 只接受已发布且完整校验的父加增量 active archive；stderr 只投影阶段与已处理/总代码数，不接受部分 checkpoint、阶段或 `run_id`，永不自动 promotion 或修改默认档位 |
...
当前工作区的 2026-09-10 运行证据是：封存旧父/增量归档仍在，但不存在新控制库、月库或 active snapshot；
零参数下载返回 `history_sync_pending`。V3 loader 必须继续返回模型不可用，
不能把旧训练根级模型/报告当作活动工件；这项运行现状不放宽完整 snapshot 和单一活动指针契约。
...
一次 active/父/增量身份，逐股读取不重复对全部分片做整文件 hash；样本使用一次性 SQLite 工作库按代码写入、
...
以下旧 BaoStock 父加增量组件在阶段 A 后已经失去公开写入口，仅用于说明尚待阶段 D/G 删除的当前内部代码，
不是可调用合同、兼容承诺或新归档读取面。旧 `baostock_runtime_progress` 的阶段包括
...
`remaining_codes`、`completed_codes`、`training_ready_codes`、`remaining_training_codes`、`failed_codes`、
`expected_records`、`downloaded_records`、`active_workers`、
...
`checkpoint_loading`。行业历史事实缺失时，日线 checkpoint 仍视为已下载；有至少一段可证行业区间但缺
训练事实的代码只进入 `date,code,tradestatus,isST` 四列 raw 补全队列，不进入 qfq 或完整日线请求；
成功后只清理该代码所在分片的失败状态，不得逐股重复解码全部分片上下文。完全没有可证行业日期时以
`historical_industry_incomplete` 标记降级。归档身份为 `baostock_daily_core`，分片路径为
...
父归档完成后的增量目录固定为 `increments/<increment-manifest-hash>/`，内含不可变 `manifest.json` 和
`shards/increment-<parent-partition>.sqlite3`；根级 `active-manifest.json` 是唯一活动指针。发布前必须校验父
manifest 文件 hash、父内容 hash、增量记录/checkpoint 逻辑 hash、每个增量分片整文件 hash 和行数；重复 update
必须跳过已完成 checkpoint，不能重复请求 raw/qfq/isST 或生成重复记录。
日线 manifest 的逻辑记录身份只绑定 raw/qfq 日线 batch，不再把训练事实就绪误作日线发布前提；历史行业及
其它点时资格事实必须写入独立后续数据集，不得修改已经封存分片的字节、日线内容 hash 或日线来源身份。
下载收尾先从轻量 checkpoint 索引确认代码终态，再按稳定分片顺序一次只校验一个 SQLite 分片；校验过程用
SQLite 游标逐行复核日期、载荷 hash 和批次 hash，只向覆盖审计传递紧凑有类型覆盖证据；
不得同时构造全部分片的日线领域对象。新整文件 manifest 的 catalog 登记前对每个待封存分片执行整文件
SHA-256；封存完成后的任何同文件写入都必须使 manifest 校验失败，行业及其它点时事实只能进入独立后续数据集。
状态投影不属于内容消费信任边界，只做上述有界元数据检查；下载恢复和训练读取仍必须对分片整文件 hash
完成一次校验。一次训练输入打开先构造一个有类型的共享 context 身份，对混合稳定/旧 spec 各计算一次兼容
hash，后续各分片和逐股读取复用该身份并只校验目标 checkpoint，不得按分片或股票重算全市场 context hash。
历史上已由退役的“日线局部 Hash”生成的 manifest 只允许旧日线局部 Hash 只读识别：逐分片校验
旧 Hash 命中时仅隔离旧 manifest、catalog 和派生摘要，保留日线 SQLite 并进入重新整文件封存前的一次性
训练事实迁移窗口；旧 Hash 不命中的真实损坏分片才单独隔离并重下。已完成日线但缺训练事实的代码只补 raw
逐日 isST 事实，不重复下载 qfq，
也不重写日线 batch。供应商已给出但只覆盖部分日期的真实行业生效区间可与 isST 一同封存；
行业生效区间未覆盖的早期日期排除出训练，不得用当前行业回填。这只使真实可证的日线样本可用于
非生产训练，不代替历史行业、资格、硬过滤和风险事实的后续点时资格门。
BaoStock 返回代码变更前历史时必须校验响应 `adjustflag`，不能把 qfq 请求实际返回的 raw 行接纳为 qfq；
当前确认的唯一日期化别名是 `001914` 在 2019-06-06 前使用历史代码 `000043` 的 qfq，边界后使用当前代码。
含可修复代码的质量失败只按所属分片隔离和重下。完整 manifest 即使保留 `null_rows_present`，也仅在全部
代码训练 checkpoint 齐全、没有覆盖失败代码、重复、越界或未来行且每股达到训练人口覆盖门时，允许
非生产训练读取完整 cell；null cell 必须跳过，归档质量状态、点时资格与生产权限仍保持失败关闭。
命令行输出根据 `completed_codes / universe_count` 派生两位小数的完成进度百分比；股票池尚未建立时固定显示 `0.00%`，不改变内部进度对象或最终 JSON schema。
单 worker 不可用或重启失败是本次运行级故障，终态必须保留重启的具体稳定失败码，不得把队列中尚未尝试的股票批量持久化为下载失败；续传初始化必须清理旧实现误写的 `worker_unavailable` 标记，保留待下载队列和已落盘日线。
运行时固定最多 1 个进程，单次供应商调用墙钟上限 60 秒，最多重试 2 次，每次查询至少间隔 2 秒；
60 秒只约束单次供应商调用，不约束包含多次正常调用的完整阶段或单股任务。上述旧组件不得再由
`download_history`、训练、Web、HTTP 或调度器创建数据；阶段 D/G 必须连同旧测试、codec、父增量读取与
`data/history/baostock-daily/sessions-*` 路径一起删除。
