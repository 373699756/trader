...
开发工作计划与交付状态统一见[工程实施](03_工程实施.md)。
...
[工程实施](03_工程实施.md) 为准；依赖、构建和入口以根目录 `pyproject.toml` 为唯一权威；协作流程以根目录
`AGENTS.md` 为准。
...
要求立即启用已封存但历史拒绝的 Tomorrow 候选工件，并将人工日线代理与该工件做成配置切换。活动产品
从 2026-08-30 起统一称为评分生产档位 V1/V2；既有 `tomorrow_shadow_baseline`、
`tomorrow_historical` 只作为历史研究身份保留审计证据，不再把阶段编号暴露为现行命名。
两种人工发布的身份、失败证据、代理差异、监控和禁止自动更新边界以荐股策略文档第 15.1.17–15.1.19
节为准，不得把越权发布或日线 proxy 描述成原历史研究门禁通过。
...
`check`、`research-status`、`download_history` 和 `train-tomorrow`。旧 H0 历史归档、回测和筛选命令已经退役。组合内的
...
| 日常 | `./run.sh research-status` | 只读投影归档、已下载和 training-ready 状态；不访问网络、不启动下载或训练 |
...
无最终 manifest 时，`research-status` 必须从 checkpoint 索引只读投影真实的分片、股票池、已下载和
...
[评分逻辑](01_评分逻辑.md) 与[工程实施](03_工程实施.md) 引用和解释，不在本文复制。
...
日线分片完整性 hash 只覆盖日线上下文、日线记录、批次和成功 checkpoint；它以流式 SHA-256 校验这些
日线所有权内容，追加行业和训练事实不得改变日线 manifest，任何日线载荷或身份漂移仍必须失败关闭。
正式封存先读取不含 `daily_cells.payload_json` 的轻量 checkpoint 索引确认代码终态，再按稳定路径一次只解码一个 SQLite 分片，
顺序完成覆盖审计、日线 hash 和 catalog 组装；不得在收尾前把全部分片日线载荷同时载入内存。
供应商物理坏行数只保留为质量证据：解析失败的物理行必须被丢弃并在对应逻辑日期表现为显式缺失，
不得伪造 OHLCV，也不得越过逻辑覆盖率门禁；最终资格由全体、逐板、全窗口老股的逻辑覆盖率和其它完整性异常裁决。
全部股票已有日线 batch 但尚未封存 manifest 时，只读状态使用 `history_manifest_unavailable`；只有股票池中
仍存在未形成日线 batch 的代码时才使用 `incomplete_codes`，两者均不得冒充行业或训练事实已就绪。
