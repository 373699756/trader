...
BaoStock 离线基础设施在 `infra/research` 内按当前职责拆分：`baostock_session.py` 统一匿名登录、稳定错误映射和
...
历史维护和训练命令也通过独立应用边界编排：`application/history` 的下载、缺口更新和只读状态用例只依赖
`HistorySupplierPort`、`HistoryArchivePort` 与 `HistoryStatusPort`，原子月分片、snapshot 校验和维护锁由
`infra/research` 提供；`application/training` 的 V2/V3 用例只依赖注入的 `TrainingRunnerPort` 和只读
