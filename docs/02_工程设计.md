...
阶段 A 已切换该合同；阶段 B 控制面和阶段 C/D 月分片同步尚未闭合期间，合法零参数调用只投影
`history_maintenance_status`，固定 `state=blocked`、`reason=history_control_plane_pending`、归档根
...
文件。不得临时恢复旧下载链绕过 blocker。后续控制面只能填充同一有类型状态对象，JSON 由 CLI adapter 白名单
...
snapshot；零参数下载返回 `history_control_plane_pending` 且目录仍为空。V3 loader 必须继续返回模型不可用，
