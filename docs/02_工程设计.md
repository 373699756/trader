...
`data/train/tomorrow-v3/model.json`。该文件是 V3 实际评分唯一使用的训练模型；同目录
`report.json` 和 `training-input.json` 只作离线审计，不参与算分。缺少或损坏模型时在组合根失败关闭并给出稳定原因，
...
| 离线训练结果 | `data/train/tomorrow-v3/` | 成功训练把 `model.json`、`report.json`、`training-input.json` 直接写入该目录；输入 hash 只保留在 JSON 审计字段，不创建 hash/run 子目录；不得写入历史下载分片、WAL、缓存或供应商响应 |
...
固定为 `historical_unavailable` 与 `manual_user_override`。用户显式 `--profile v3` 才能加载它；默认 V1、V2、
