...
`history_control_repository.py`、`history_month_partition.py` 与 `history_month_archive.py` 分别拥有控制库、稳定年月
...
| `PointInTimeDatasetArtifactStore` | `infra/research`；逐日分片并原子封存点时数据集 | 有类型数据集报告 -> 白名单 JSON、逐日 hash 与根 manifest | 不覆盖冲突工件，不内联全历史为单一大 JSON，不授权训练或生产 |
...
阶段 C 由 `HistoryMonthlyRevision`、严格 `history_month_codec`、`SQLiteHistoryMonthPartitionRepository` 和
`SQLiteHistoryMonthlyArchive` 组成。月库规范表以
...
通过 `scripts/repack_baostock_history.py` 的显式 `build` 在 `data/historyless/baostock` 串行执行
...
日志处于未完成或已验证待收尾状态时，普通下载和训练必须返回 `history_repack_activation_pending`；只有显式内存门
