...
5. 计算 `local_score = clamp(base_score - local_risk_penalty, 0, 100)`，风险只扣一次；
...
`setup_type` 与 `downside`；Tomorrow V1/V2 另外公开
...
发布状态卡之后紧跟“策略内评分最高”卡，明确分数只在当前策略内比较，并按当前快照全部已评分候选的最终评分降序、本地评分降序、代码升序
稳定展示最多三只股票的“分数 - 股票代码 - 股票名称”；未就绪时使用同一快照观察草稿中的 `top_scores`。
若没有可持久化的非正式明细但选择诊断保留正数已评分聚合和最高最终分，显示该最高分并明确当前无达到观察门槛的股票；
Tomorrow `no_positive_net_utility` 必须改称最高相对信号分并标明未通过成本门。若旧冻结记录的已评分数为 0，
...
Tomorrow 至少形成一条模型评分条目，且全部条目的预测成本后净超额均不大于 0 时，空结果使用
`no_positive_net_utility`。模型预测超额收益仍按同批横截面形成 0–100 相对评分，成本不得把评分统一清零；
每只非正净效用股票以 `model_net_utility_non_positive` 独立执行门禁阻止复核、观察和正式推荐。页面明确显示
“模型预测成本后净超额均未转正，保持空仓”，同时用同一决策的已评分数量和最高相对信号分证明评分已经完成，
并可展示聚合原因；该专项空结果不得拼接普通评分空结果的距正式线或观察线/正式线计数，不能误报成评分尚未完成、
模型复核缺失或达到可执行分数线，也不能把相对评分当成已通过成本门禁。
