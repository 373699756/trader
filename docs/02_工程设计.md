...
来自已验证 snapshot 在当前训练股票池和实际读取日期范围内的最新 revision 精确计数，被过滤行仍计入已检查量，有效样本另行显示；
横截面和行业阶段分别以原始样本行与实际行业数为分母。阶段边界立即输出，推进至多每 30 秒一行，60 秒无推进时
重复当前计数作为存活心跳；结束摘要覆盖完成、无需训练、已有当前工件、阻塞、拒绝和取消。交互日志不显示 RSS，
正式内存验收仍由 2048 MiB 峰值门禁记录。训练固定两个计算线程、LightGBM 单线程并降低进程调度优先级。
训练持有历史维护锁后，每个月分片只允许执行一次整文件 hash、`quick_check` 和行数验证；同一读取器内后续计数和
逐股查询必须复用已验证分片，禁止以可观测性修复掩盖重复全量校验。
