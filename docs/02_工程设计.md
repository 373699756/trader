...
`download_history` 调用链，活动产品仍不兼容读取旧数据。
...
`check`、`download_history`、`train-tomorrow`、`train-v2`、`train-v3`、`install-history-automation` 和
`uninstall-history-automation`。旧 H0 历史归档、回测和筛选命令已经退役。组合内的
底层 `trader-cli` 阶段继续保留用于测试、自动化和故障定位，但不再逐项暴露为 `run.sh` 命令。

`download_history` 是零参数历史维护命令；Linux/macOS、PowerShell 和底层 CLI 都不接受 `--runtime-dir`、
...
`install-history-automation` 必须先列出当前用户范围内将写入的绝对任务路径和调度器命令，再询问确认；它验证固定
项目目录、绝对配置、虚拟环境 Python 以及已安装的 Trader/BaoStock 依赖，不请求管理员权限，也不联网安装或升级。
任务模板固定绝对工作目录、解释器和配置，只设置 `PYTHONUNBUFFERED=1`，不得复制 Token 或其它运行密钥。
`uninstall-history-automation` 使用同一渲染计划反向注销任务，仅删除内容仍与托管模板完全一致的文件；用户修改过的
文件必须失败关闭，禁止覆盖或删除。计划任务的 stdout 是一次最终摘要；同步进度和最终结果同时写入
...
不再重复到期内容。
该链固定 `automatic_model_update=false`，禁止调用 `train-tomorrow`、修改 profile、重启服务、启用模型或写生产冻结。
...
目标训练布局为 `data/train/v2/{today,tomorrow,d25}/` 与 `data/train/v3/{today,tomorrow,d25}/`。当前已把旧扁平
三头逐字节同步到 V3 目标目录，V2 三个目录保持空占位；同步副本不是第二个活动来源。后续切换必须在同一高风险
批次完成档位感知的训练、due 状态、locator、codec、CLI、性能入口和三头原子加载，并先生成通过完整性与收益门的
V2 三头。切换前 loader 不读取空 V2 或嵌套 V3 目录；切换后删除旧扁平目录及临时兼容逻辑，使每档只有一个真相源。
...
| 日常 | `./run.sh --profile v1|v2|v3` | 显式覆盖当前进程档位；`check` 也可选档位，但 `download_history` 不接受评分档位 |
| 日常 | `./run.sh check` | 依次校验配置、只读投影研究状态并运行所选档位的离线性能门禁 |
| 运维 | `./run.sh install-history-automation` | 显示并确认当前用户的 15:10/20:30 自动同步任务；只验证既有依赖，不静默安装 |
| 运维 | `./run.sh uninstall-history-automation` | 注销用户级任务并仅删除未被改写的托管模板，不需要管理员权限 |
| 离线研究 | `./run.sh download_history` | 唯一零参数维护入口；自动选择初次 2000 日、缺口续传、5 日回读或 `already_current`，失败和取消不改变上一 active snapshot |
...
| 离线训练 | `./run.sh train-tomorrow` | 零参数读取固定项目数据根；不接受 `--runtime-dir`、`--profile`、部分 checkpoint、阶段或 `run_id`。只在 initial/cadence/revision/training-contract due 时训练，永不自动 promotion 或修改默认档位 |
...
`train-tomorrow` 的应用层节奏决策只比较不可变完整输入和已经通过整组 codec 的活动 bundle。当前完整输入的
...
`train-tomorrow`、`train-v2` 与 `train-v3` 的交互式 stderr 使用有类型进度的紧凑边界投影，不输出内部 JSON：资源策略、输入分片校验、
...
训练明确分为 V3 Tomorrow 兼容入口 `train-tomorrow`、V2 三头 `train-v2` 与 V3 三头 `train-v3`，不得要求再执行研究状态命令。

当前 `check` 组合命令、历史自动化、`train-tomorrow`、`train-v2` 及 `train-v3` 均由 `trader-cli` 单一编排器
