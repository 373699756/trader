...
`./run.sh --profile v2` 显式选择 V2；`./run.sh --profile v3` 启动时直接读取项目
`data/train/tomorrow-v3/model.json`。该文件承载 V3 实际评分参数；同目录 `report.json` 是加载所需的配对证据，
必须与模型共享来源 commit、训练输入、特征 manifest、切分、训练合同、模型载荷和报告 hash；
`training-input.json` 只作离线审计，不参与算分。缺少、损坏或配对不一致时在组合根失败关闭并给出稳定原因，
...
| 离线训练结果 | `data/train/tomorrow-v3/` | 成功训练把 `model.json`、`report.json`、`training-input.json` 直接写入该目录；模型与报告必须构成来源/输入/特征/切分/训练合同/模型载荷 hash 一致的逻辑对，输入 hash 只保留在 JSON 审计字段，不创建 hash/run 子目录；不得写入历史下载分片、WAL、缓存或供应商响应 |
| 离线训练 | `./run.sh train-tomorrow [--runtime-dir <路径>] [--allow-partial-history]` | 默认只接受完整 manifest；显式试跑参数可从哈希封存的 training-ready checkpoint 训练，stderr 只投影阶段与已处理/总代码数；两种模式均不接受阶段或 `run_id`，永不自动 promotion 或修改默认档位 |
...
training-ready 股票数，不得显示为 `not_started/0`，也不得解码全量日线载荷。显式部分训练把输入股票集合、
逐股 batch/facts hash 和父上下文绑定为不可变 `partial_checkpoint` 快照；模型 codec 必须保留该范围，V3 运行证据
固定为 `historical_data_insufficient` 与 `manual_user_override`。日线完整训练同样只能标记
...
验证仍固定在下载恢复、正式封存和训练输入打开三个信任边界，任何不匹配继续失败关闭。历史分片可混合保存
