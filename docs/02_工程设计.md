...
同步编排和供应商子进程共用不可变 `HistorySyncProgress` 事件；CLI 仅在最终外部边界将事件白名单投影到
stderr，最终 `history_maintenance_status` JSON 继续独占 stdout。进度覆盖初始化、上下文、登录、日历、证券总体、
行业、raw/qfq、股票完成数、月分片封存和 snapshot 发布，包含当前项、完成量、百分比、attempt、调用耗时和总
耗时；供应商等待期间至少每 5 秒 flush 一次心跳。单次供应商调用从开始到结果行解析共用一个 45 秒 deadline，
