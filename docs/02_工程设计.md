...
阶段 C 由 `HistoryMonthlyRevision`、严格 `history_month_codec`、`SQLiteHistoryMonthPartitionRepository`、
`SQLiteHistoryMonthlyArchive` 和 `SQLiteHistoryTrainingCache` 组成。月库规范表以
...
读取，并为每只代码仅保留连续 61 行。训练缓存以 `(trade_date, code)` 聚簇且绑定 snapshot hash，供后续按日
横截面和按代码顺序拟合使用。月库封存必须完成 WAL checkpoint、SQLite quick check、行数与整文件 SHA-256
...
| 离线训练 | `./run.sh train-tomorrow` | 零参数读取固定项目数据根；不接受 `--runtime-dir`、`--profile`、部分 checkpoint、阶段或 `run_id`。只在 initial/cadence/revision due 时训练，永不自动 promotion 或修改默认档位 |
...
一次 active snapshot 及来源身份，逐股读取不重复对全部分片做整文件 hash；样本使用一次性 SQLite 工作库按代码写入、
按交易日做横截面残差化、按行业和切分读取拟合，训练结束删除工作库。历史分片可混合保存
...
横截面和行业阶段分别以原始样本行与实际行业数为分母。阶段边界立即输出，推进至多每 30 秒一行，60 秒无推进时
...
`峰值 RSS 上限 2048 MiB`，不采样实时 RSS；正式内存验收由 2048 MiB 峰值门禁记录。成功、无需训练、阻塞或拒绝
