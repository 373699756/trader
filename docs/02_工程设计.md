...
`data/train/tomorrow-v3/active-bundle.json`，再定位其内容寻址 generation 中的 `model.json`。该文件承载 V3
实际评分参数；同 generation 的 `report.json` 和 `training-input.json` 都是加载所需的配对证据。三者必须共享
active data snapshot、来源 identity、成熟标签截止日 `label_cutoff`、来源 commit、训练输入、特征 manifest、切分、训练合同、
模型载荷和报告 hash；`training-input.json` 不参与算分，但必须先通过 codec。活动指针、任一文件缺少、损坏或
...
| 离线训练结果 | `data/train/tomorrow-v3/` | 成功训练先在独立临时目录生成三件 JSON，经完整 codec/配对校验后把目录原子移动为 `generations/<bundle-hash>/`，最后原子替换 `active-bundle.json`；旧 generation 可保留供恢复，运行时只消费活动指针，任一步失败都不改变上一活动组 |
...
`initial_training_required`、`cadence_due`、`input_revision_due`；输入不完整或活动基线领先当前输入时为
...
`峰值 RSS 上限 2048 MiB`，不采样实时 RSS；正式内存验收由 2048 MiB 峰值门禁记录。训练固定最多两个计算线程，
