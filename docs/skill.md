...
- 进程生命周期：`RuntimeSupervisor` 已统一管理实时调度器、DeepSeek 调度器、验证线程、推荐刷新线程、实时行情刷新线程和快照写入线程；但历史因子、舆情和推荐池行情刷新等一次性线程仍未全部纳入可停止的统一所有权。
...
- 后台资源统一收口：为历史因子、舆情和推荐池行情刷新建立可观测、可停止、可等待的线程所有者，并由 `RuntimeSupervisor` 统一停止。
