...
`docs/software-business-design.md` 和 `docs/recommendation-strategy.md` 为准。
  ...
  当前 `system.start()` 会同步进入 `RuntimeSupervisor.start()`；
  `RuntimeSupervisor.start()` 在启动 scheduler 和 Web server 之前，依次执行全部
  initializer。当前 initializer 包含正式记录恢复、参考数据恢复、
  `pipeline.initialize()` 和预算恢复：

  - `src/trader/application/runtime.py:66-80`
  - `src/trader/bootstrap.py:222-240`

  `pipeline.initialize()` 会在启动线程中刷新交易日历。无本地缓存或缓存过期时，
  `ChinaTradingCalendar` 可能执行外部网络请求；因此网络慢或不可用时，端口尚未监听，
  Web 也无法及时返回 `calendar_unavailable`。这与“日历失败不阻塞 Web”的既定契约不一致。

  历史预热虽然由首轮行情异步触发，但当前配置下批次超时为：

  `12 × (4 × ceil(30 / 5) + 1) = 300 秒`

  该上限过大，可能长期占用历史 source lane，并拖慢首轮历史覆盖和推荐就绪；它不得进入
  Web 启动关键路径，也不得影响首轮行情任务。

  ### 11.2 P0：建立启动耗时基线

  增加结构化启动阶段日志和耗时指标，至少覆盖：

  - 进程入口、`build_system()`、`system.start()`；
  - 每个 initializer 的开始、结束、失败和耗时；
  - `pipeline.initialize()`、线程启动、Web 开始监听；
  - 首次 HTTP 可用、首次行情可用、首次可解释预览和首次正式推荐可用。

  测试矩阵必须区分冷启动、热启动、大运行目录、无日历缓存、日历网络慢、日历网络失败和
  历史数据不可用。基线完成前不得用单一总耗时推断根因。
  ...
  - 研究数据恢复；
  - 非关键证据恢复；
  - 冷启动补算和 catch-up freeze。

  建议将 `pipeline.initialize()` 拆分为：

  - `initialize_local_state()`：只做本地恢复，不访问网络；
  - `schedule_deferred_initialization()`：由 scheduler 或后台初始化 worker 执行外部刷新、
    日历判断和补算。

  `RuntimeSupervisor.start()` 不得在持有生命周期锁时执行可能阻塞的外部 I/O；初始化失败或
  超时必须转换为状态和退避重试，不得阻塞 Web 启动。

  ### 11.4 P0：交易日历 fail-closed 且不阻塞 Web

  - 优先使用仍在有效期内的本地日历缓存；
  - 无缓存或刷新失败时，启动 Web，状态为 `calendar_unavailable`；
  - 在日历可靠前禁止抓行情、评分、冻结和 fallback；
  - 后台按 `30/60/120/300` 秒退避刷新；
  - HTTP 只读取已发布状态，不触发日历网络请求。

  外部日历请求必须有独立的短超时，不能继承启动总期限，也不能无限等待。

  ### 11.5 P1：限制历史预热对首轮可用性的影响

  - 将历史预热单批超时改为可配置的较小上限，建议 12–20 秒；
  - 首批只覆盖有限候选集，后续按批次异步推进；
  - 历史预热使用独立容量，不得挤占首轮行情、候选和 TopK 所需 worker；
  - 失败只更新 `history_warming`、重试计数和下一次退避时间；
  - 预热失败不得被视为服务启动失败，也不得触发同步重试风暴。

  ### 11.6 P1：启动状态和业务门禁

  Web 启动后应能明确返回 `starting`、`calendar_unavailable`、`history_warming` 或
  `ready`。状态未达到业务资格时：

  - 不发布评分结果；
  - 不执行冻结；
  - 不生成收盘 fallback；
  - 保留可诊断的失败原因、耗时和下次重试时间。

  这不改变既有“正式记录、预算、证据、overlay 和结算记录可恢复，历史/候选/观察池/review
  等临时状态重启后重新预热”的持久化边界。
  ...
  - 无日历缓存且网络不可用时，Web 在固定时间内可访问；
  - 日历请求超时不阻塞 `system.start()`；
  - 大量本地恢复不阻塞端口监听；
  - 首次行情和可解释预览在 15 秒内出现；
  - 日历未就绪时不会抓行情、评分、冻结或生成 fallback；
  - 历史预热超时后服务仍可查询状态并按退避重试；
  - 冷启动、热启动、午间启动和 15:00 后启动矩阵全部通过；
  - 启动性能修复不得破坏既有 30 秒共享关闭期限和第二次信号强制退出语义。

  验收命令仍包括 `make format-check`、`make lint`、`make type-check`、`make test`、
  `make package` 和 `git diff --check`；另需保留离线 fixture 验收，禁止用外部网络状态作为
  唯一通过条件。
