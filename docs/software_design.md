...
- 自动收益回填默认开启，从14:30开始按600秒间隔仅补齐历史未成熟结果，不重复覆盖当日信号；发现 `needs_backfill`、待回填或旧口径 outcome 时会按 current baseline 自动回填，页面按钮只保留 dry-run 预览和手动重试。
- 自动快照默认开启，15:00后使用收盘锚点保存一次；明日优先和2-5日持有均按下一交易日开盘价验证执行。
...
PYTHONPATH=. .venv/bin/pytest -q \
  ...
  tests/scoring/test_point_in_time_execution.py

# 执行/组合
PYTHONPATH=. .venv/bin/pytest -q \
  tests/scoring/test_backtest_exit.py \
  tests/scoring/test_portfolio_risk.py \
  tests/scoring/test_portfolio_baseline.py
