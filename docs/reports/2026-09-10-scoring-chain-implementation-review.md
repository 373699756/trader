...
| 对应章节 | 状态 | 真实缺口 | 后续入口 |
| --- | --- | --- | --- |
| 第 5–8 节 | 部分实现 | 候选资格顺序和单一所有权已实现，但当前性能绝对预算、精确 1080 最坏路径及真实服务六检查点未闭合 | `docs/03_工程实施.md` 第 2 节 |
| 第 5–8 节 | 未实现 | 候选/本地组件内部仍有硬编码权重，设置校验器保存重复固定数值，尚未实现所有生产权重由配置单一拥有 | 工程实施第 3 节 |
...
| 第 3、11、12 节 | 外部证据阻塞 | 缺可证明查询/生效时间的历史行业、11:20/14:50 分钟锚点，以及完整资格、硬过滤和风险事实；因此真实点时数据集、收益结论和终端留出保持关闭 | 工程实施第 6 节 |
| 第 11–12 节 | 部分实现 | `CandidateRecallLedger` 和有限参数族已实现，但指定容量族、五方案、NDCG/分组/性能完整报告及真实确认结果不存在 | 工程实施第 7 节 |
| 第 11–12 节 | 部分实现 | `FeatureSpecCatalog`、`CanonicalOutcomeEvaluator`、风险/成本/不确定性及多类 shadow 研究对象已经实现；评分逻辑点名的统一 `ShadowMonitor` 尚未实现 | 工程实施第 8 节；不得用 fixture 代替 |
| 第 12 节 | 阻塞 | 终端留出尚未打开，Shadow 收益/尾部风险门未执行，人工生产启用也未获授权 | 工程实施第 8–9 节 |
| 第 6、12 节可选路线 | 未授权 | Today 与 D25 仍按已授权规则头运行；各自模型头没有独立点时数据、训练、确认、终端留出和授权 | 工程实施第 10 节 |
| 第 10、11 节 | 明确不建模 | V1/V2 没有逐股严重亏损概率，状态公开 `loss_probability_status=not_modeled`，页面不得用总体统计伪造 | 若未来建模，须经过第 12 节完整门禁 |

因此，后续不能笼统表述为“继续补整条评分链”。应按工程实施中的依赖顺序先闭合候选验收与权重配置单一来源，
