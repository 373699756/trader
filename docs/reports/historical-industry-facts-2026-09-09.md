...
- 最终报告 schema 实物复跑：未验证（用户明确要求暂不恢复正式归档；重试以 `daily_archive_invalid` 退出）

本报告记录初次完整审计证据；最终代码的聚合计数预计不变，但新的报告内容 hash 未经实物复跑确认，不能写成
已通过。本报告不表示行业永远不可取得，也不授权点时数据集、V3 训练、终端留出或生产切换。
...
`daily_archive_invalid` 退出。按用户明确要求暂不恢复正式归档；未来恢复父 manifest hash
`a3bd8aa73b42b99c7a646d3a67a4fce8451401e67ac45596c20e17812d93acff` 对应的原归档后，必须从同一命令重新
运行并更新内容 hash，不得重下载或改写日线。只有未来来源同时满足
