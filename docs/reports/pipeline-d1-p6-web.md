...
| patch 投影 | `src/trader/application/projection_patch.py` | 当前 patch 使用 `schema_version=2`；需 A 冻结是否改为 `patch_schema_version=2` |
...
未提交；公共工作树内 D 只新增 docs/reports/pipeline-d1-p6-web.md 交接报告。

owned_paths_changed
docs/reports/pipeline-d1-p6-web.md
...
src/trader/application/projection_patch.py
...
| `.venv/bin/python -m pytest tests/contract/test_project_records.py tests/contract/test_pipeline_contract_base.py -q` | 通过，6 passed |
...
| `.venv/bin/mypy src/trader/application/published_snapshots.py src/trader/application/projection_patch.py src/trader/web/sse.py` | 通过 |
| `git diff --check` | 通过 |
| `make format-check` | 失败，非 D 文件 `src/trader/infra/deepseek/evidence_router.py`、`src/trader/infra/market_data/columnar.py`、`tests/component/test_pipeline_deepseek_c2.py` 需要格式化 |
| `make lint` | 失败，非 D 文件 `src/trader/infra/deepseek/schema.py`、`tests/component/test_pipeline_deepseek_c2.py`、`tests/unit/test_v17_columnar_changes.py` 存在 lint 错误 |
...
| D 本次修改 | `src/trader/application/projection_patch.py`、`src/trader/application/publisher.py`、`tests/unit/application/test_publisher.py`、本报告 |
...
`docs/reports/pipeline-g3-gate-review.md`、`tests/component/test_pipeline_deepseek_c3.py`、
`tests/fixtures/market_data/pipeline_b3/`。
...
| `node --check src/trader/web/static/dashboard.js` | 通过 |
| `.venv/bin/python -m ruff check src/trader/application/projection_patch.py src/trader/application/publisher.py tests/unit/application/test_publisher.py` | 通过 |
| `.venv/bin/python -m mypy src/trader/application/projection_patch.py src/trader/application/publisher.py tests/unit/application/test_publisher.py` | 通过 |
| `make type-check` | 通过，162 source files |
| `git diff --check` | 通过 |
| `make format-check` | 失败，非 D 并发文件 `tests/component/test_pipeline_deepseek_c3.py` 需格式化 |
| `make lint` | 失败，非 D 并发文件 `src/trader/infra/deepseek/reviewer_requests.py` import 顺序 |
| `make test` | 失败，非 D 并发用例 `tests/component/test_pipeline_deepseek_c3.py::test_c3_same_stock_three_strategies_share_one_raw_facts_request` 把 `mappingproxy` 当作 `dict` 断言 |
...
| 命令/检查 | 结果 |
| --- | --- |
| `pytest tests/performance/test_pipeline_d4_web.py -s` | D4 当前/驻留延迟与传输门限通过，数值见 11.2-11.3 |
...
tests/performance/test_pipeline_d4_web.py
tests/performance/pipeline_d4_browser_fixture.py
tests/unit/application/test_published_snapshots.py
tests/unit/application/test_publisher.py
docs/reports/pipeline-d1-p6-web.md
CHANGELOG.md
