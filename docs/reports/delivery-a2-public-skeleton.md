...
新增 `src/trader/application/ports/delivery.py`，集中定义唯一公共 schema/version、identity、
event 和校验函数：

- 总契约：`delivery_contract_base_v1`。
...
新增 `src/trader/application/delivery_test_doubles.py`：
...
新增 `tests/contract/test_delivery_a2_public_skeleton.py`，覆盖：
...
`docs/reports/delivery-g1-contract-base.md` 中 A2 -> B2 -> C2 -> D2 -> G2 顺序执行。

## 7. 验证

- `PYTHONPATH=src .venv/bin/python -m pytest -q tests/contract/test_delivery_a2_public_skeleton.py tests/contract/test_v2_architecture.py tests/unit/test_v2_settings.py`
  通过：62 项。
...
- 全局 `make format-check` 被非 A2 的 `src/trader/infra/deepseek/evidence_router.py`、
  `src/trader/infra/deepseek/schema.py`、`tests/component/test_delivery_deepseek_c2.py` 格式阻断。
- 全局 `make lint` 被非 A2 的 `src/trader/infra/deepseek/schema.py` import 排序和
  `tests/component/test_delivery_deepseek_c2.py` 未使用导入阻断。
