...
Score-R5 工程能力已完成；V2 工程发布章节全部闭合。Score-R5-Run 运行证据连续性修复、
Score-R0-Rerun 替代窗口预注册与身份贯通及 Score-H0 可下载历史回测基础已完成。下一研究章节为
Score-R6；历史参数筛选不再等待 `score_p0_v2` 的未来 40 日，但任何生产晋级仍须取得独立的真实
前向证据，不得用重建历史生成 `promotion_eligible`。
