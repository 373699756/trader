...
def test_scheduler_restores_state_when_thread_start_fails():
    class FailingThread:
        def start(self):
    ...
    scheduler = DeepSeekPrecomputeScheduler(thread_factory=lambda **_kwargs: FailingThread())

    with (
        patch.object(config, "ENABLE_DEEPSEEK_FEATURES", True),
...
def test_scheduler_claims_each_pre_1430_slot_once_across_rechecks(tmp_path):
    ...
    with (
        patch.object(config, "DEEPSEEK_SCHEDULER_DB_PATH", str(tmp_path / "scheduler.sqlite3")),
        patch.object(
            config,
...
def test_scheduler_never_auto_runs_at_or_after_on_demand_window(tmp_path):
    ...
    with (
        ...
        patch.object(config, "DEEPSEEK_ON_DEMAND_START", "14:30"),
        patch.object(
            scheduler,
            "_execute",
        ) as execute,
...
def test_scheduler_persists_successful_execution_status(tmp_path):
    scheduler = DeepSeekPrecomputeScheduler()
    slot_key = "2026-07-14T09:30"
    with patch.object(config, "DEEPSEEK_SCHEDULER_DB_PATH", str(tmp_path / "scheduler.sqlite3")):
        assert scheduler._claim(slot_key, datetime(2026, 7, 14, 9, 30)) is True
        with patch(
            "stock_analyzer.deepseek_scheduler.subprocess.run",
            return_value=subprocess.CompletedProcess(args=[], returncode=0, stdout="complete", stderr=""),
        ):
            scheduler._execute(slot_key)
        last_run = scheduler._last_run()

    assert last_run["status"] == "ok"
    assert last_run["exit_code"] == 0
    assert last_run["completed_at"]
    assert last_run["message"] == "complete"


def test_scheduler_persists_failed_execution_status(tmp_path):
    scheduler = DeepSeekPrecomputeScheduler()
    slot_key = "2026-07-14T09:30"
    with patch.object(config, "DEEPSEEK_SCHEDULER_DB_PATH", str(tmp_path / "scheduler.sqlite3")):
        assert scheduler._claim(slot_key, datetime(2026, 7, 14, 9, 30)) is True
        with patch(
            "stock_analyzer.deepseek_scheduler.subprocess.run",
            side_effect=subprocess.TimeoutExpired(cmd="deepseek-precompute", timeout=60),
        ):
            scheduler._execute(slot_key)
        last_run = scheduler._last_run()
    ...
    assert last_run["completed_at"]
    assert "timed out" in last_run["message"]
...
def test_app_factory_starts_only_the_internal_deepseek_scheduler():
    with (
        ...
        patch.object(config, "DEEPSEEK_INTERNAL_SCHEDULER_ENABLED", True),
        patch("stock_analyzer.deepseek_scheduler.start_deepseek_scheduler") as start_scheduler,
    ):
