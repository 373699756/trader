...
- `tests/fixtures/deepseek/delivery_c1/report_to_a.md`
...
- `delivery_contract_base_v1`、`p4_p5_high_value_review_manifest_v1` 和 `deepseek_v4_review_facts_v1` 由 A 作为唯一公共 owner 冻结。
...
- C2 must report process peak RSS for maximum Flash batch, Pro batch, schema repair and response validation as required by `docs/plan_delivery.md`.
...
- `budget.py`、`budget_batch_ledger.py`、`budget_reporting.py`、`budget_support.py`: SQLite 原子预算、批次、恢复和状态汇总。
...
- Full repo quality gates are known not globally green on A1 base per `docs/reports/delivery-a1-baseline.md`; C1 single-domain tests pass.
