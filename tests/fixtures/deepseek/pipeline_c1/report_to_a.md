...
- `tests/fixtures/deepseek/pipeline_c1/report_to_a.md`
...
- `pipeline_contracts_v1`、`p4_p5_high_value_review_manifest_v1` 和 `deepseek_v4_review_facts_v1` 由 A 作为唯一公共 owner 冻结。
...
- C2 must report process peak RSS for maximum Flash batch, Pro batch, schema repair and response validation as required by `docs/plan_pipeline.md`.
...
- `reviewer_selection.py`: deadline、priority、高价值启发、emergency 原因、candidate 去重。
- `budget.py`、`budget_batch_ledger.py`、`budget_reporting.py`、`budget_audit.py`: SQLite 原子预算、批次、恢复和状态汇总。
...
- `DeepSeekBudgetLedger` 用 SQLite `BEGIN IMMEDIATE` 做原子预留。
...
- Full repo quality gates are known not globally green on A1 base per `docs/reports/pipeline-a1-baseline.md`; C1 single-domain tests pass.
