...
def _measure(config_path: Path, fixture_path: Path) -> dict[str, Any]:
    ...
    pipeline, pipeline_retained = _exercise_pipeline_pressure()
    retained = (market.retained, deepseek_retained, pipeline_retained)
    gc.collect()
    stages["combined_rss_bytes"] = _rss_bytes()
    peak = _peak_rss_bytes()
    uss = _uss_bytes()
    cache_bytes = _cache_estimated_bytes(cache.status())
    logical_bytes = cache_bytes + market.polars_bytes + market.snapshot_bytes + pipeline["logical_bytes"]
    ...
    return {
        ...
        "workload": {
            ...
            "p6_resident_days": pipeline["resident_days"],
            "p6_rejects_older_dates": pipeline["rejects_older_dates"],
            "slow_clients_opened": pipeline["slow_clients_opened"],
        },
        "market": {
...
def _fill_cache_pools(cache: BoundedLruCache[object], policy: Any) -> dict[str, dict[str, object]]:
    datasets = {
        ...
        "p6_projection": ("published_recommendation_view", 4),
    }
...
def _exercise_pipeline_pressure() -> tuple[dict[str, Any], tuple[object, ...]]:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the fixed pipeline A4 integrated memory acceptance suite")
