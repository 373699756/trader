...
from trader.training.domain.evaluation.statistics import (
...
def test_r5_bootstrap_uses_preregistered_seed_and_is_deterministic() -> None:
    expected = int.from_bytes(
        hashlib.sha256(b"historical_research_baseline|20260811|continuous_entry|5").digest()[:8],
        "big",
        signed=False,
    )
...
def test_r5_bootstrap_seed_is_namespaced_by_the_new_research_identity() -> None:
    expected = int.from_bytes(
        hashlib.sha256(b"preregistered_research|20260820|continuous_entry|5").digest()[:8],
        "big",
        signed=False,
    )

    assert bootstrap_seed("continuous_entry", 5, spec=PREREGISTERED_RESEARCH_SPEC) == expected
    assert bootstrap_seed("continuous_entry", 5, spec=PREREGISTERED_RESEARCH_SPEC) != bootstrap_seed(
        "continuous_entry", 5
    )
