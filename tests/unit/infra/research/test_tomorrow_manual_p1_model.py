...
def test_manual_p1_fit_is_deterministic_bounded_and_provenance_bound() -> None:
    ...
    first = fit_manual_p1_model(iter(_rows()), SCORE_H0_V1_SPEC, manifest)
    second = fit_manual_p1_model(iter(_rows()), SCORE_H0_V1_SPEC, manifest)
    payload = sealed_production_artifact_payload(first)

    assert first == second
    assert first.feature_ids == P1_FEATURE_IDS
...
def test_manual_p1_fit_rejects_a_manifest_from_another_archive() -> None:
    manifest = HistoricalArchiveManifest("other_research", "", "a" * 64, "b" * 64, ())

    with pytest.raises(ValueError, match="exact H0 manifest"):
        fit_manual_p1_model(iter(_rows()), SCORE_H0_V1_SPEC, manifest)
