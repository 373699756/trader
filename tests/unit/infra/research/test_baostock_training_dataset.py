...
def test_dataset_terminal_binds_daily_and_effective_fact_failures_without_split(tmp_path) -> None:
    ...
    dataset = build_baostock_training_dataset_manifest(_daily_manifest(), facts, ())
    store = BaoStockTrainingDatasetArtifactStore(tmp_path)

    assert dataset.status == "historical_data_insufficient"
    assert dataset.split is None
    assert dataset.label_contract == BaoStockTrainingLabelContract()
    assert "historical_industry_effective_at_unavailable" in dataset.failure_reasons
    assert store.write(dataset) == dataset
    assert store.write(dataset) == dataset

    path = tmp_path / "tomorrow-training-dataset.json"
    payload = json.loads(path.read_text(encoding="utf-8"))
    payload["status"] = "dataset_ready"
    path.write_text(json.dumps(payload), encoding="utf-8")
    with pytest.raises(BaoStockTrainingDatasetArtifactConflictError, match="schema or hash"):
        store.verify()
...
def test_ready_dataset_round_trip_binds_split_label_and_rejects_conflicting_identity(tmp_path) -> None:
    ...
    split = build_baostock_training_split(dates, parent_manifest_hash=digest)
    manifest = BaoStockTrainingDatasetManifest(digest, "c" * 64, split.label_contract, "dataset_ready", split, ())
    store = BaoStockTrainingDatasetArtifactStore(tmp_path)
    ...
    with pytest.raises(ValueError, match="formula"):
        BaoStockTrainingLabelContract(formula="different")

    conflict = BaoStockTrainingDatasetManifest(digest, "d" * 64, split.label_contract, "dataset_ready", split, ())
    with pytest.raises(BaoStockTrainingDatasetArtifactConflictError, match="identity conflict"):
