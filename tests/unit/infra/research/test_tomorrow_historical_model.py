...
from trader.training.application.tomorrow_historical_screening import TomorrowHistoricalRow
...
def test_p2_ensemble_is_deterministic_and_keeps_the_official_validation_out_of_fitting() -> None:
    trainer = TomorrowHistoricalEnsembleTrainer()
    training = _rows(date(2025, 1, 2), 10)
    validation = _rows(date(2026, 1, 2), 2)

    first = trainer.fit(training, validation, TOMORROW_HISTORICAL_SPEC.candidate)
    second = trainer.fit(training, validation, TOMORROW_HISTORICAL_SPEC.candidate)
