...
def _rows() -> tuple[TomorrowHistoricalRow, ...]:
    return tuple(
        TomorrowHistoricalRow(
            trade_date=HISTORICAL_SCREENING_SPEC.training_start + timedelta(days=index),
...
def test_manual_v1_fit_is_deterministic_bounded_and_provenance_bound() -> None:
    ...
    first = fit_manual_v1_model(iter(_rows()), HISTORICAL_SCREENING_SPEC, manifest)
    second = fit_manual_v1_model(iter(_rows()), HISTORICAL_SCREENING_SPEC, manifest)
