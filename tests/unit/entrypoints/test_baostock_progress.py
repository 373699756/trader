...
def test_cli_progress_writer_flushes_stage_totals_and_database_locations() -> None:
    stream = io.StringIO()
    writer = _BaoStockProgressWriter(Path("/var/lib/trader/history"), stream=stream, monotonic=lambda: 12.5)
    ...
    assert payload == {
        "active_workers": 1,
        "checkpoint_database_pattern": "/var/lib/trader/history/baostock-daily/shard-*.sqlite3",
        "completed_codes": 13,
        "downloaded_records": 23117,
        "elapsed_seconds": 0.0,
        "expected_records": 9250000,
        "failed_codes": 2,
        "final_database": "/var/lib/trader/history/baostock-daily/score-baostock-daily-core-v2.sqlite3",
        "last_failure_reason": "supplier_query_failed_blacklisted",
        "phase": "downloading",
