...
def test_latest_wins_keeps_running_item_and_only_the_newest_pending_item() -> None:
    ...
    worker = LatestWinsWorker("test-latest", process, order_key=lambda value: value)
    assert worker.start()
    ...
    assert not any(thread.name == "test-latest" for thread in threading.enumerate())
...
def test_latest_wins_stop_uses_shared_deadline_and_cancels_pending() -> None:
    ...
    worker = LatestWinsWorker("test-deadline", block, order_key=lambda value: value)
    worker.start()
    worker.offer(1)
    assert entered.wait(timeout=1.0)
    worker.offer(2)
