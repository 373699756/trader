...
from trader.application.recommendation.scored_projection import build_scored_hybrid, build_scored_local
...
def test_today_observe_phase_never_emits_executable_action(application_feature_factory) -> None:
    policy = _recommendation_policy(load_strategy_settings(PROJECT_ROOT / "config" / "strategy.json"))
    features = _features(application_feature_factory)
    projection = build_scored_local(_native_input(features, phase="today_observe"), policy, sequence=1)
