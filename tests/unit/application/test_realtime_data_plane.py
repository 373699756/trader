...
from trader.domain.market.epochs import (
    DataPlaneCoverage as EpochCoverage,
)
    ...
    conflicting_pack = DailyFeaturePack(
        ...
        rows=(
            DailyFeatureRow(
                ...
                history_sessions=61,
                data_as_of=date(2026, 7, 27),
                security_master_version="master-1",
...
def test_candidate_history_below_99_percent_and_invalid_empty_do_not_replace_last_valid_epoch() -> None:
    ...
    with pytest.raises(ValueError, match="core-history coverage must be at least 99%"):
        _coverage_pack(codes, history_covered=98)
...
def _coverage_pack(codes: tuple[str, ...], *, history_covered: int) -> DailyFeaturePack:
    ...
    return DailyFeaturePack(
        ...
        coverage=EpochCoverage(
            potential_executable_codes=codes,
            security_master_codes=codes,
            candidate_codes=codes,
            candidate_history_codes=codes[:history_covered],
        ),
    )
...
def _coverage_market(pack: DailyFeaturePack, codes: tuple[str, ...]) -> MarketEpoch:
    ...
    return MarketEpoch(
        ...
        daily_feature_pack_version=pack.version,
        quotes=quotes,
        field_values={quote.code: market_field_values(quote) for quote in quotes},
...
def _coverage_candidate(market: MarketEpoch, codes: tuple[str, ...]) -> CandidateQuoteEpoch:
    ...
    return CandidateQuoteEpoch(
        ...
        market_epoch_version=market.version,
        quotes=quotes,
        field_values={quote.code: candidate_field_values(quote) for quote in quotes},
