...
from trader.application.research.challenger_replay_report import ChallengerReplaySelection
from trader.application.research.challengers import ChallengerReplayer
...
def _inputs():
    ...
    extraction = HistoricalExtractor(_Port(replace(summary, candidates=candidates)), _Evaluator()).extract()
    baseline = HistoricalBaselineReplayer(_R4BaselineEvaluator()).replay(extraction)
...
def test_r4_replays_all_five_variants_as_same_day_same_stock_pairs() -> None:
    ...
    report = ChallengerReplayer(_ChallengerEvaluator()).replay(extraction, baseline)
    ...
    assert report.content_hash == ChallengerReplayer(_ChallengerEvaluator()).replay(extraction, baseline).content_hash
...
def test_r4_rejects_hybrid_facts_that_were_not_recorded_in_r2() -> None:
    ...
    with pytest.raises(ValueError, match="existing facts"):
        ChallengerReplayer(_ManufacturedFactsEvaluator()).replay(extraction, baseline)
