...
class _Evidence:
    def __init__(self, rows: tuple[TomorrowHistoricalRow, ...], *, coverage: float = 0.98) -> None:
        self._rows = rows
...
def _rows() -> tuple[TomorrowHistoricalRow, ...]:
    rows: list[TomorrowHistoricalRow] = []
...
def test_p2_screen_fits_the_only_candidate_once_and_evaluates_the_frozen_validation() -> None:
    ...
    execution = TomorrowHistoricalScreeningService(_Evidence(_rows()), trainer).execute(TOMORROW_HISTORICAL_SPEC)
...
def test_p2_screen_rejects_incomplete_h0_before_training_and_does_not_try_an_alternative() -> None:
    ...
    execution = TomorrowHistoricalScreeningService(_Evidence(_rows(), coverage=0.94), trainer).execute(
        TOMORROW_HISTORICAL_SPEC
    )
...
def test_p2_screen_records_a_legal_empty_portfolio_as_a_gate_rejection() -> None:
    execution = TomorrowHistoricalScreeningService(_Evidence(_rows()), _EmptyTrainer()).execute(
        TOMORROW_HISTORICAL_SPEC
    )
