...
def _source_row(
    ...
    *,
    ...
    multiplier = {"v1": 0.8, "v2": -5.0, "daily_close_ensemble": 1.2}[profile_id]
...
def _complete_batches(start_day: int, days: int) -> tuple[TomorrowJointProfileBatch, ...]:
    return tuple(
        ...
        for profile_id in ("v1", "v2", "daily_close_ensemble")
    )
...
def test_alignment_uses_strict_common_intersection_and_reports_coverage_loss() -> None:
    ...
    c3_rows = tuple(_source_row("daily_close_ensemble", 0, code) for code in (1, 2, 4))

    dataset = align_tomorrow_joint_batches(
        (_batch("v1", v1_rows), _batch("v2", v2_rows), _batch("daily_close_ensemble", c3_rows))
    )
    ...
    assert by_profile["daily_close_ensemble"].missing_from_union == 1
    assert by_profile["daily_close_ensemble"].dropped_outside_common == 1
...
def test_alignment_rejects_common_row_metadata_mismatch(mismatch: str) -> None:
    ...
    daily_close_ensemble = _source_row(
        "daily_close_ensemble", 0, 1, actual=0.99 if mismatch == "mature_label" else None
    )

    with pytest.raises(ValueError, match="metadata and mature labels"):
        align_tomorrow_joint_batches(
            (_batch("v1", (v1,)), _batch("v2", (v2,)), _batch("daily_close_ensemble", (daily_close_ensemble,)))
        )
...
def test_research_fit_and_prediction_keep_one_non_production_joint_output() -> None:
    ...
    model = next(item for item in result.candidate_family.candidates if item.candidate_id == "v1_daily_close_ensemble")
