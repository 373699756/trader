...
from trader.application.recommendation.scored_freezing import DecisionRuntimeIdentity
...
def _coordinator(
    ...
    clock: _Clock,
) -> TodayFreezeCoordinator:
    return TodayFreezeCoordinator(
        index,
        repository,
        clock,
...
def test_boundary_read_failure_still_closes_and_seals_the_retry_candidate(tmp_path: Path) -> None:
    ...
    current = _today(_at(11, 19, 59))
    assert index.publish(current, expected_version=None).accepted
    coordinator = TodayFreezeCoordinator(
        index,
        FailFirstLoad(),
        _Clock(_at(11, 20)),
        runtime_identity=DecisionRuntimeIdentity("config-current", "strategy-current", "fusion-current"),
