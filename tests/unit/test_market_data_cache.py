...
def _policy(*, capacity: int = 2, group_bytes: int = 100_000) -> CachePolicy:
    return CachePolicy(
        schema_version=6,
        policy_version="cache_policy_unsupported",
        datasets={
        ...
        estimator_version="canonical_json_utf8",
    )
...
def test_cache_uses_monotonic_ttl_without_changing_business_source_time() -> None:
    ...
    cache.put(identity, {"rows": 1}, data_version="quotes-fixture", source_time=source_time)
...
def test_negative_refresh_cache_preserves_value_without_reestimating_on_recovery(monkeypatch) -> None:
    ...
    cache.put(identity, {"close": 1.0}, data_version="history-current", source_time=source_time)

    cache.put_negative(identity, error_code="timeout")
    suppressed = cache.get(identity)

    assert suppressed.value == {"close": 1.0}
    assert suppressed.data_version == "history-current"
...
def test_cache_lru_capacity_is_deterministic_and_observable() -> None:
    ...
    cache.put(first, {"close": 1.0}, data_version="initial", source_time=NOW)
    cache.put(second, {"close": 2.0}, data_version="revision", source_time=NOW)
    assert cache.get(first).value == {"close": 1.0}
    cache.put(third, {"close": 3.0}, data_version="incoming", source_time=NOW)
...
def test_cache_evicts_business_degraded_value_before_healthy_lru_value() -> None:
    ...
    cache.put(
        degraded,
        {"close": 1.0},
        data_version="degraded",
        source_time=NOW - timedelta(seconds=121),
    )
    cache.put(healthy, {"close": 2.0}, data_version="healthy", source_time=NOW)
    assert cache.get(degraded).state == "degraded"

    cache.put(incoming, {"close": 3.0}, data_version="incoming", source_time=NOW)
...
def test_cache_status_uses_insert_time_scope_counters_without_scanning_entries() -> None:
    ...
    cache = BoundedLruCache(_policy())
    identity = _identity("600001")
    cache.put(identity, {"close": 1.0}, data_version="initial", source_time=NOW)
...
def test_cache_insert_below_capacity_does_not_scan_existing_entries() -> None:
    ...
    assert cache.put(second, {"close": 2.0}, data_version="revision", source_time=NOW) is True
