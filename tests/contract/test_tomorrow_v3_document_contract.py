...
def test_v3_owns_three_independent_strategy_heads_without_stacking() -> None:
    ...
    required_contract = (
        ...
        "Ridge/LightGBM",
        "预测绝不跨策略复用",
        "D25 使用与 Today 相同的五个趋势特征，仍只有一个生产 head",
...
def test_remaining_v3_delivery_has_isolated_owners_and_explicit_commands() -> None:
    ...
    assert "状态：`blocked_by_shared_samples`" in strategy
    assert "状态：`blocked_by_real_training`" in strategy
...
def test_trained_v3_profile_loads_only_the_fixed_active_bundle_files() -> None:
    ...
    assert "只完整验证一次 snapshot、月分片 hash 和来源 identity" in strategy
    ...
    assert "heads: Mapping[Strategy, HeadRuntime]" in model_port
    assert "profile_id: ScoringProfileId" in model_port
