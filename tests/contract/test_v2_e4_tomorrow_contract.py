...
def test_v2_e4_is_complete_and_e5_is_the_next_engineering_section() -> None:
...
def test_tomorrow_freeze_control_seals_before_the_boundary_scoring_offer() -> None:
    ...
    first_control = source.index("pipeline._tomorrow_v2_control.on_clock(now)")
    scoring = source.index("snapshots = list(_score_strategies_on_workers")
    second_control = source.index("pipeline._tomorrow_v2_control.on_clock(now)", first_control + 1)
