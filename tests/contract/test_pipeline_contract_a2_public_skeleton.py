...
from trader.application.pipeline_contract_doubles import P4ConsumerStub, ProjectionOverlayProducerStub, ReviewInputStub
from trader.application.ports.pipeline_contracts import (
    CONTRACT_VERSION,
...
def test_a2_pipeline_schema_versions_and_resync_reasons_are_single_owner() -> None:
    versions = pipeline_schema_versions()
...
def test_a2_review_manifest_rejects_long_requests_and_invalid_identity() -> None:
    ...
    with pytest.raises(PipelineContractError, match="long review input collection"):
        HighValueReviewManifest(
            schema_version=P4_P5_SCHEMA_VERSION,
    ...
    with pytest.raises(PipelineContractError, match="feature_snapshot_identity"):
        HighValueReviewInput(
            **{
...
def test_a2_deepseek_v4_facts_are_evidence_bounded_and_score_free() -> None:
    ...
    with pytest.raises(PipelineContractError, match="outside the manifest"):
        DeepSeekV4Facts(
            **{
                **facts.__dict__,
...
def test_a2_projection_overlay_stub_preserves_cas_and_overlay_boundaries() -> None:
    ...
    with pytest.raises(PipelineContractError, match="upsert and remove"):
        ProjectionEvent(
            **{
                **projection.__dict__,
