...
from trader.application.delivery_test_doubles import P4ConsumerStub, ProjectionOverlayProducerStub, ReviewInputStub
from trader.application.ports.delivery import (
    CONTRACT_VERSION,
...
def test_a2_review_manifest_rejects_long_requests_and_invalid_identity() -> None:
    ...
    with pytest.raises(DeliveryContractError, match="long review input collection"):
        HighValueReviewManifest(
            schema_version=P4_P5_SCHEMA_VERSION,
    ...
    with pytest.raises(DeliveryContractError, match="feature_snapshot_identity"):
        HighValueReviewInput(
            **{
...
def test_a2_deepseek_v4_facts_are_evidence_bounded_and_score_free() -> None:
    ...
    with pytest.raises(DeliveryContractError, match="outside the manifest"):
        DeepSeekV4Facts(
            **{
                **facts.__dict__,
...
def test_a2_projection_overlay_stub_preserves_cas_and_overlay_boundaries() -> None:
    ...
    with pytest.raises(DeliveryContractError, match="upsert and remove"):
        ProjectionEvent(
            **{
                **projection.__dict__,
