...
def test_production_tomorrow_runtime_has_no_baseline_or_cutover_dependency() -> None:
    production_sources = (
        PROJECT_ROOT / "src" / "trader" / "application" / "tomorrow_v2_projection.py",
        PROJECT_ROOT / "src" / "trader" / "application" / "tomorrow_v2_runtime.py",
        PROJECT_ROOT / "src" / "trader" / "application" / "tomorrow_v2_freezing.py",
    )
    ...
    assert "TomorrowDecisionFreeze" not in source
...
def test_system_start_failure_stops_tomorrow_v2_runtime_with_shutdown_budget() -> None:
    history_pool = Mock()
    ...
    assert history_deadline is tomorrow_runtime.stop.call_args.kwargs["deadline"]
