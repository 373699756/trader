...
class BlockingScoringInputs(DataRefresh):
    ...
    def refresh_task(self, request) -> RefreshOutcome:
        ...
        if request.task is PipelineTask.CANDIDATE_QUOTES:
            ...
            raise DataRefreshUnavailableError("controlled_input_failure")
        return outcome
...
class Decisions:
    ...
    def research_intent(self, current: ScoredDecision) -> ResearchIntent:
        codes = tuple(item.code for item in current.items)
        return ResearchIntent(current.strategy, current.trade_date, codes, codes)
...
def test_scheduler_atomically_publishes_complete_quotes_for_local_and_hybrid() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            index=index,
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-overlay-observer"),
            freezes=Freezes(),
...
def test_slow_hybrid_review_does_not_block_a_newer_local_score() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            index=index,
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-async-review-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_scheduler_refreshes_frozen_today_overlay_without_mutating_formal_decision() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=index,
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-frozen-overlay-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_frozen_cadence_targets_formal_codes_and_refreshes_overlay_without_rescoring() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=index,
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-frozen-cadence-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_scheduler_recovers_overlay_issue_after_later_success() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=index,
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-overlay-recovery-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
    ...
    second = replace(first, observed_at=second_at, input_version="quotes-retry")
...
def test_scheduler_publishes_local_before_research_and_defers_first_review_until_risk_rescore() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=reviews,
            index=index,
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-research-order-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
...
def test_periodic_tick_does_not_score_until_a_scoring_input_finishes() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=index,
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-input-trigger-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_unchanged_candidate_refresh_does_not_trigger_another_score() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=UnifiedDecisionIndex(),
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-unchanged-input-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_afternoon_checkpoint_is_dispatched_for_both_scored_strategies() -> None:
    ...
    class CurrentRequiredFreezes(Freezes):
        ...
        def capture_checkpoint(self, strategy: Strategy, at: datetime) -> None:
            ...
            if index.snapshot(strategy).current is None:
                raise FreezeUnavailableError("no_eligible_decision")
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=index,
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-checkpoint-observer"),
            freezes=freezes,
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_current_fixture_runs_without_the_legacy_pipeline_through_shutdown() -> None:
    ...
    observer = AsyncDecisionObserver(
        ...
        capacity=16,
        thread_name="test-fixture-observer",
    )
    index = UnifiedDecisionIndex()
    ...
    assert status.config_version == "runtime-current"
...
def test_after_close_cold_start_recovers_missing_scored_strategies_and_long() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=reviews,
            index=index,
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-after-close-observer"),
            freezes=freezes,
            settlement=settlement,
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_midday_cold_start_recovers_only_missing_non_today_outputs_once_without_review() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=reviews,
            index=index,
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-midday-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_midday_empty_observation_draft_is_a_completed_recovery_not_a_retry_loop() -> None:
    ...
    class DraftOnlyDecisions(Decisions):
        ...
        def build_local(self, request: CycleRequest):
            if request.strategy is Strategy.TOMORROW:
                ...
                raise DecisionUnavailableError("security_master_coverage_incomplete")
            return super().build_local(request)
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=UnifiedDecisionIndex(),
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-midday-draft-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_midday_recovery_retries_when_refresh_failed_before_any_output() -> None:
    ...
    class FailingOnceData(DataRefresh):
        ...
        def refresh(self, request: CycleRequest) -> None:
            ...
            if request.strategy is Strategy.TOMORROW and not self.failed:
                self.failed = True
                raise DataRefreshUnavailableError("source_unavailable")
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=index,
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-midday-retry-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_midday_recovery_does_not_queue_duplicate_while_lane_is_active() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=UnifiedDecisionIndex(),
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-midday-active-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
    ...
    runtime.submit_due(midday + timedelta(seconds=30))
    tomorrow_lane = next(lane for lane in runtime.status().lanes if lane.name == "trader-tomorrow")
    release.set()
    assert runtime.wait_idle(2.0)
    runtime.stop(ShutdownDeadline.start(2.0))
...
def test_after_close_prefers_existing_same_day_current_without_rebuilding() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=index,
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-after-close-current-observer"),
            freezes=freezes,
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_after_close_formal_records_only_refresh_selected_overlays_without_full_market_recovery() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=index,
            observer=AsyncDecisionObserver((), capacity=4, thread_name="test-after-close-formal-observer"),
            freezes=Freezes(),
            settlement=settlement,
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_tomorrow_lane_progresses_while_today_lane_is_blocked() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=index,
            observer=AsyncDecisionObserver((), capacity=1, thread_name="test-reserved-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_newer_same_strategy_cycle_does_not_starve_first_current_publication() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=index,
            observer=AsyncDecisionObserver((), capacity=2, thread_name="test-superseded-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
    ...
    first = CycleRequest(Strategy.TOMORROW, NOW.date(), NOW, "afternoon", 1, "input-initial", False, NOW)
    second = replace(
        first,
        observed_at=NOW + timedelta(seconds=1),
        sequence=3,
...
def test_first_completed_current_can_freeze_while_newer_cycle_is_still_scoring() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=index,
            observer=AsyncDecisionObserver((), capacity=2, thread_name="test-freeze-progress-observer"),
            freezes=Freezes(index),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=published.append,
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            observer=AsyncDecisionObserver(
                (blocked_audit,),
                capacity=1,
                thread_name="test-full-audit-observer",
            ),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
...
def test_refresh_failure_retains_last_valid_decision_without_cascading_build_failure() -> None:
    class FailingData(DataRefresh):
        ...
        def refresh(self, request: CycleRequest) -> None:
            super().refresh(request)
            if self.fail:
                raise DataRefreshUnavailableError("source unavailable")
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=index,
            observer=AsyncDecisionObserver((), capacity=1, thread_name="test-failure-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
    ...
    assert payload["runtime_version"] == "runtime-current"
    ...
    runtime.submit_cycle(replace(request, sequence=5, input_version="input-latest", allow_review=False))
    assert runtime.wait_idle(1.0)
    recovered = runtime.status()
...
def test_successful_hybrid_publish_recovers_decision_path_issues() -> None:
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=UnifiedDecisionIndex(),
            observer=AsyncDecisionObserver((), capacity=1, thread_name="test-hybrid-recovery-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_runtime_error_history_is_bounded_and_repeated_failures_are_coalesced() -> None:
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=UnifiedDecisionIndex(),
            observer=AsyncDecisionObserver((), capacity=1, thread_name="test-error-history-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_successful_publish_does_not_recover_an_unrelated_freeze_failure() -> None:
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=UnifiedDecisionIndex(),
            observer=AsyncDecisionObserver((), capacity=1, thread_name="test-freeze-issue-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_afternoon_schedule_skips_today_after_its_freeze_boundary() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=SharedReviews(),
            index=UnifiedDecisionIndex(),
            observer=AsyncDecisionObserver((), capacity=1, thread_name="test-afternoon-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
...
def test_review_deadline_prevents_a_late_model_upgrade() -> None:
    ...
    runtime = SchedulerRuntime(
        RuntimeDependencies(
            ...
            reviews=reviews,
            index=index,
            observer=AsyncDecisionObserver((), capacity=1, thread_name="test-deadline-observer"),
            freezes=Freezes(),
            settlement=Settlement(),
            research_factory=noop_research_factory,
            publish_decision=lambda _event: None,
    ...
    request = CycleRequest(Strategy.TOMORROW, NOW.date(), NOW, "afternoon", 1, "input-initial", True, NOW)
    runtime.start()
    runtime.submit_cycle(request)
    assert runtime.wait_idle(1.0)
    status = runtime.status()
