...
def test_budget_rejects_all_long_reservations(tmp_path, emergency: bool) -> None:
    ...
    assert (result.allowed, result.reason) == (False, "unknown_strategy")
    assert budget.summary(NOW.date().isoformat())["used"] == 0
...
def test_budget_rejects_long_bucket_even_for_non_long_strategy(tmp_path) -> None:
    ...
    assert (result.allowed, result.reason) == (False, "unknown_bucket")
    assert budget.summary(NOW.date().isoformat())["used"] == 0
...
def _settings() -> DeepSeekSettings:
    return DeepSeekSettings(
        ...
        strategy_limits={"today": 68, "tomorrow": 45, "d25": 35, "shared_preheat": 15, "emergency": 5},
        stage_targets={
            "today_main": 0,
...
def _budget(path, *, hard_limit: int) -> DeepSeekBudgetLedger:
