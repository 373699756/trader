...
def test_legacy_v1_event_is_readable_but_cannot_be_written_by_current_store(tmp_path) -> None:
    ...
    observation = V2DecisionObservation(event, legacy_audit)
    store = SQLiteV2ResearchTraceStore(tmp_path, use_legacy_layout=True)
    store.initialize()
    ...
    assert SQLiteV2ResearchTraceStore(tmp_path).get(event.decision_version) == observation


def test_v2_point_in_time_population_survives_restart_and_obeys_cutoff(tmp_path) -> None:
    before = replace(
        decision(sequence=1),
        observed_at=datetime(2026, 8, 11, 14, 49, tzinfo=ZoneInfo("Asia/Shanghai")),
    )
    before_event = build_v2_decision_committed(before)
    before_audit = _v2_audit(before_event.decision_version, before_event.decision_hash, before.observed_at)
    before_observation = V2DecisionObservation(before_event, before_audit)
    late = replace(
        before,
        sequence=2,
        observed_at=datetime(2026, 8, 11, 14, 51, tzinfo=ZoneInfo("Asia/Shanghai")),
    )
    late_event = build_v2_decision_committed(late)
    late_observation = V2DecisionObservation(
        late_event,
        _v2_audit(late_event.decision_version, late_event.decision_hash, late.observed_at),
    )
...
def _audit(decision_version: str, decision_hash: str) -> V2CommittedResearchAudit:
    return _v2_audit(
        decision_version,
        decision_hash,
        datetime(2026, 8, 11, 10, 0, tzinfo=ZoneInfo("Asia/Shanghai")),
    )


def _legacy_audit(decision_version: str, decision_hash: str) -> V2CommittedResearchAudit:
...
def _v2_audit(decision_version: str, decision_hash: str, observed_at: datetime) -> V2CommittedResearchAudit:
    legacy = _legacy_audit(decision_version, decision_hash)
    population = (
        V2ResearchPopulationAudit(
            code="600001",
            board="main",
            ...
            has_delisting_name=False,
            structured_risk_values=tuple(
                (name, None)
                for name in (
    ...
    return V2CommittedResearchAudit(
        decision_version=legacy.decision_version,
        decision_hash=legacy.decision_hash,
        input_version=legacy.input_version,
        hard_filter_aggregates=legacy.hard_filter_aggregates,
        passed_candidates=legacy.passed_candidates,
        production_local=legacy.production_local,
        research_shadow=legacy.research_shadow,
        shadow_mode=legacy.shadow_mode,
