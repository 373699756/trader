...
- P2-0 只冻结了可执行研究契约，没有运行或读取 H0/P2 历史结果，因此尚无 95% 覆盖、300 条有效配对、
  ...
  P2-3 人工晋级前活动生产保持不变。
- H0 不含真实历史 ST/行业、14:50 分钟输入、披露/接收时点风险或 DeepSeek facts；这些字段未被中性值
  伪造，但也意味着历史筛选不能证明真实每行业最多 2 只或盘中硬过滤效果。该训练/生产差异必须在 P2-2
  前向报告中显式评估。pandas/NumPy ABI 问题只修复了本地验证环境，未改变 `pyproject.toml`；干净构建
  由当前依赖解析获得兼容版本，长期仍应由常规依赖门禁观察间接依赖漂移。
