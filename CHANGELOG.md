...
- 历史日线主链路固定为官方 Tushare SDK/API：120 积分只允许一次批量代码的 `daily` 未复权数据，明确完成手到股、千元到元的单位归一；证券主数据、交易日历、复权因子、`pro_bar`、日度估值和财务指标必须等积分达到 2000 后才可启用。实时全市场继续并行采集，Sina 备源改为 5 worker 有界分页以在 20 秒截止内交付完整 5,000+ 行，腾讯继续承担候选和 TopK 定向复核。
...
- 官方权限文档列出的 A 股 `daily` 最低积分为 120，但当前 Token 的真实接口响应仍拒绝该请求并被脱敏归类为 `quota_or_rate_limit`；因此本机暂以只读历史种子降级，覆盖仅限旧库已有代码，其他代码继续等待 Tushare 权限恢复或远端历史源成功。12:10 属于契约规定的 11:20-13:00 暂停评分窗口，且服务在 today 11:20 冻结后才重启，所以现场不能补造当日 today 快照；tomorrow/d25 将在 13:00-14:50 活动窗口按修复链路形成，冻结后迟到结果仍不得回写。
