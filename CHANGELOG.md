...
- 用户要求再次 Review 零参数历史下载整体方案，并把每日自动更新、每 20 个成熟标签日如何自动训练或提醒、
  ...
  小时，“每日触发”不能等同于“每日高效全市场重拉”；同时 `01_评分逻辑.md` 固定
  `automatic_model_update=false` 并禁止自动训练，因此当前不能让定时下载暗中启动一至两小时训练。Changed:
  `04_策略回溯.md` 的目标计划新增日更来源能力门、15:10 主触发与 20:30 幂等检查、Linux/macOS/Windows
  休眠补跑、单进程互斥、磁盘预检、控制库重建、SIGTERM 续传和通知降级；20 日改为从活动模型成功训练的
  `label_cutoff` 统计不同成熟交易日，initial、cadence、历史修订和数据不完整使用独立原因，只有成功 bundle
  清零。当前选定“每日自动同步、到期只提醒”：控制台、任务日志和桌面每天最多提醒一次，`check` 只读同一
  状态；第二次触发在首轮成功/运行时分别 `already_current`/`already_running`，只有未执行、失败、中断或未完整
  发布才续传缺口。自动训练与自动生产启用继续关闭。Added: 把整个目标拆为来源实证、控制面、月分片、零参数同步、
  训练节奏、三平台自动化和最终切换 A–G 七个依赖阶段，并为每阶段写明退出条件和大任务唯一一次完整门禁；
  `03_工程实施.md` 的当前 V3 工件任务同步把阶段 A 设为下一实施切片，不再把旧 `--mode update` 当成目标前置。
  Verification: 新文档契约先在缺失自动化所有者、due 语义和执行阶段的旧计划上失败，更新后运行文档契约、
  Ruff、格式和 `git diff --check`。Residual Risks: 本批仍只交付计划；尚未安装系统任务、接入批量日更来源、
  改造 SQLite、运行自动同步或重训。日更来源能力必须以真实 raw/qfq 与复权语义实证，若没有候选来源通过，
  高效日更保持阻塞而不是回退到伪数据；无人值守训练若将来需要，必须由用户另行明确授权并同步修改两份权威
  文档，且仍不得自动启用模型。`Regression-Key: zero-argument-history-snapshot-training-alignment`。

- 用户要求完全重构 `./run.sh download_history` 的后续方案，只保留零参数入口，并追问每日数据更新、重训周期、
  2000 日窗口是否合理、SQLite 分片查询，以及上市不足 2000 日的股票如何对齐。现状判断：当前参数化父归档加
  增量归档结构不是目标形态；单一巨型库和三个板块大库都会扩大故障域，且板块分片不符合训练的日期截面主访问
  路径；2000 日适合作为滚动研究归档上限，不应等同于固定拟合窗口或每股必备行数；BaoStock 逐股限速也意味着
  单个月重拉耗时仍可能由请求数主导。Changed: 在 `04_策略回溯.md` 形成待实施的完整目标合同：历史维护命令
  拒绝全部尾随参数，目录收敛为控制库和 `YYYY/MM.sqlite3` 月库；月库以 `(trade_date, code, revision_id)` 聚簇、以
  `(code, trade_date)` 和日期/板块覆盖索引兼顾全量训练与单股查询，训练按月顺序扫描并维护 61 日滚动状态，
  避免“全部股票 × 全部月份”小查询。全市场交易日历成为唯一日期主轴，证券只在上市至退市有效区间贡献样本，
  不为上市前日期补行；同一交易日不得跨训练、确认和留出集合。归档滚动保留 2000 个完整交易日，750/1000/1250
  候选拟合跨度经 walk-forward 选择，约 250 日确认、约 250 日终端留出并设 embargo；数据可每日同步，但默认每
  新增 20 个成熟标签日才提示 `training_due`，下载不自动训练，训练失败继续使用上一活动模型。Verification:
  新文档契约先对旧的板块分片、缺失日期对齐和未声明日期主存储方案失败，更新后通过；Ruff、文档定向契约和
  `git diff --check` 按低风险文档门禁执行。Residual Risks: 本批只确定实施计划，没有修改下载器、训练器、运行
  loader 或本地历史数据；月库原子发布、损坏旁路重建、真实 2000 日下载、跨月查询性能、完整训练资源和 V3
  重启加载仍须在后续高风险实施批次实证，不得把计划写入视为功能已经可用。
  `Regression-Key: zero-argument-history-snapshot-training-alignment`。
...
- 用户要求 Review `docs/01_评分逻辑.md`，聚焦数据采集、过滤、评分、冻结和展示的完整荐股链，并逐环节
  ...
  活动 `base_score` 已由所选 V1/V2/V3 模型给出，文档仍把旧 Tomorrow 规则组件权重并列为活动本地评分，
  也容易误读。现将文档从 1552 行收敛为端到端 13 章：逐层固定输入、输出、过滤原因、候选权重、Today/D25
  规则评分、Tomorrow 模型评分、成本执行门、本地风险、DeepSeek、68/32 融合、动作、TopK、冻结恢复和展示；
  明确 `not_ready`、降级、业务空集与成本空仓的差异。新增 13 环节优化矩阵，分别评估实时性、稳定性和收益
  空间，并把完整点时数据/收益闭环、过滤与风险消融、V3/成本感知选择、DeepSeek/门槛/集中度验证排成
  P0–P2 顺序；全部标记为研究建议，未经历史 point-in-time 终端留出不得改变生产策略或宣称收益提高。
  软件发布、迁移、owner、命令状态和详细历史工件仍由 `02_工程设计.md`、`03_工程实施.md` 与 Changelog
  维护。本批不修改代码、配置、运行数据或实际荐股行为。Verification: 18 个直接相关契约文件共 65 项测试
  通过，新增文档聚焦/顺序/优化覆盖/固定不变量契约通过；文档链接、受影响测试 Ruff 和
  `git diff --check` 通过。全量测试、打包、仓库外安装、性能和浏览器门禁不适用，因为本批只重构策略说明
  和直接文档契约。Residual Risks: 当前尚无证据证明 V1/V2 能稳定提高未来收益；V3 完整点时历史、行业
  有效期、终端留出和生产授权仍未闭合；表中所有优化须按优先级另立高风险实现与验证批次。
  `Regression-Key: focus-end-to-end-scoring-strategy-review`。
...
- 用户追问“成本后净效用不大于 0 为什么与评分有关”，并要求移除该耦合且不让成本继续影响荐股评分。
  根因确认是 Tomorrow 生产评分把交易可执行性错误塞入分数刻度：先按成本后净效用排名，再把所有非正值
  统一清零，导致数百只已完成模型推理的股票同时显示 0 分，前三名只是无意义的代码排序；上一批只让
  运行诊断对此报错，没有修复生产评分。现将两项职责拆开：V1/V2/V3 的 Tomorrow 模型信号分只按同批
  预测超额收益稳定映射为 0–100，成本正负不再改写评分；20–40bp Amihud 估算成本、73/78 门槛、风险、
  固定 68/32 融合和冻结规则均保持。预测成本后净效用不大于 0 的股票携带独立
  `model_net_utility_non_positive` 门禁，在本地选择、DeepSeek 复核与最终动作三处失败关闭，因此不会因
  恢复相对评分而进入观察或正式推荐。Web 保留最高评分和股票身份，同时明确“成本后净超额均未转正，
  保持空仓”，不再宣称固定成本把信号分变成 0；统一诊断继续把有已评分候选但最高分不大于 0 视为错误。
  Verification: 先以旧实现复现模型、应用和页面三端失败；受影响评分、选择、融合、决策记录、Web 与
  两份权威文档定向测试、JavaScript 状态契约、`make format-check`、`make lint`、`make type-check` 和完整
  `make test` 均通过；无隔离 sdist/wheel 构建及提权后的 Firefox 刷新门禁通过。隔离 `make package` 复跑受
  PyPI SSL/代理中断阻塞，离线性能的等价、分配及相对预算通过，但三个既有绝对墙钟预算在当前宿主超限，
  均未冒充通过。Residual Risks: 用户桌面服务当前不可达，需在新提交重启后补运行态确认；旧冻结记录保持
  不可变，仍可能按旧分数显示，但不会被新 release 回写。
  `Regression-Key: tomorrow-zero-score-cost-aware-cash-v1`。

- 用户要求把三类短线标签从“今早/明日/2-5日”统一缩短为“今/明/2-5”，并反馈三页每天长期停在
  “等待本轮评分完成”、没有荐股数据。现场从运行态、输入漏斗、供应商探针到 Web/API 逐层核对后确认：
  证券主数据和全市场行情可用，API 与浏览器 release 身份一致；但 360 只候选中有 358 只报价年龄约 8 分钟，
  三策略每轮都只剩 2 只可评分。根因是候选来源 lane 在共享实时数据池内再提交三个腾讯分片，池饱和时嵌套
  fan-out 退化为串行并超过 3 秒硬截止；旧适配器又在任一分片失败时丢弃其它成功分片，外层也没有为规范化、
  合并和提交预留 deadline，导致该故障持续自我复现。现由组合根拥有独立四 worker 候选定向报价池并纳入有界
  启停；腾讯最多三个分片恢复真实并发，任一成功分片立即进入统一快照，只有全失败才保留整批旧值；3 秒预算
  中为本地规范化和原子提交预留 0.2 秒，三分片可使用 2.8 秒网络预算。Web 模板与唯一标签函数统一输出
  “今/明/2-5”，状态、空态和冻结摘要
  复用同一映射，不改变策略键、评分、风险、融合、DeepSeek、日期或冻结规则。
  按用户补充要求，既有统一运行诊断现在对三套短线策略逐一校验：评分阶段候选报价 P95 超过今 20 秒或明/2-5 30 秒、
  已声明 ready 但完整评分数为 0、已有评分但聚合最高分/current 最高分/Top 分数缺失或不大于 0、以及
  `top_scores` 为空或分数无效均返回失败；报告只保留年龄、计数和分数聚合，不输出股票身份。
  定向架构门禁同时暴露已推送基线中的 BaoStock 运行模块为 1202 行；本批仅移除其冗余文件头说明，恢复
  1200 行强制上限，不改变下载流程或公开类型。
  Verification: 新增分片部分成功、deadline 预留、独立池装配/生命周期、静态模板、浏览器状态，以及无评分、
  最高分为 0、Top 分数缺失和候选行情陈旧的诊断回归；完整 `make format-check`、`make lint`、
  `make type-check`、`make test`、`make package` 全部通过。无头浏览器专项通过：DOM 刷新 P95 1.048 秒、
  patch-to-paint P95 16 ms，实时页面确认新标签存在且旧标签不存在。腾讯直连诊断通过（三只聚合样本延迟
  843.7 ms）。最终重启后连续三次真实服务采样均获得候选特征/证券资料 360/360，候选报价 P95 25.179 秒，
  未再触发 30 秒陈旧门禁；2-5 完整评分 163 只、最高 71.21 并发布 1 只观察项。同一诊断对明策略最高分
  0 连续三次返回 `highest_final_score_non_positive` 错误，因此运行验收按预期失败，没有将异常伪报为通过。
  离线性能专项的结果等价性、相对预算和内存均通过；绝对耗时在本批代码与未修改基线上都可复现
  `targeted_overlay_commit` 抖动超过 100 ms，基线复测还同时出现 `market_merge` 超限，不属于本批差异。
  Residual Risks: 当前生产选用的 V1 明策略对已评候选均给出不大于成本的模型净效用，依权威评分契约只能空仓，
  但按用户验收已由诊断阻止声称正常；修改模型、固定成本或分数映射需用户明确选择评分生产档位，本批未伪造非零分。
  今策略在 11:20 后重启，按冻结契约保留既有正式记录且不追补；腾讯公开接口仍可能整体不可用，该场景继续显式降级。
  仓库外 wheel 安装已完成且 CLI 配置校验通过，但安装脚本随后的 `pip check` 被主机 `--system-site-packages`
  环境中既有 `python-debian` 缺少 `charset-normalizer` 阻断，与本 wheel 依赖无关。
  `Regression-Key: candidate-quote-shard-partial-deadline`。
