...
- P2 目前只是 `docs/fenshu.md` 中的非权威加速计划，`score_tomorrow_historical_p2_v1`、
  `score_tomorrow_shadow_p2_v1`、字段准入矩阵和约 20 日前向 collector 尚未写入权威策略契约或实现；
  当前生产评分及 P1 机器身份没有变化。H0 真实归档能否达到 95% 覆盖、是否存在历史合格候选，以及随后
  约 20 个交易日能否取得 100 条配对并通过收益/风险门禁均待后续批次验证；任一门禁失败都不会自动
  上线或通过降低标准解除。
...
- P1 批次 6 仍缺少 2027 年官方日历证明、40 个历史日、20 个前向日、至少 300/100 条配对及全部晋级
  ...
  产品等待时间；新的 P2 身份仍须在读取对应结果前完成预注册，不能复用 P1 规避审计。
