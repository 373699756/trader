...
- DeepSeek：后台预计算四策略五维决策，本地硬过滤后以 25% 权重参与综合评分；推荐和个股预测请求不等待外部 API
...
直接运行 `app.py` 时，进程内 supervisor 会统一管理实时行情、推荐池行情、历史因子、舆情、DeepSeek、自动快照与结果回填线程的生命周期；`create_app()` 本身不创建后台线程。使用其他 WSGI 入口时，需要显式调用 `create_app(start_runtime=True)`，或由外部任务平台执行上述命令，但不要同时启用两套调度。周期线程使用可中断的停止信号，一次性刷新线程由各自所有者拒绝新任务并在退出时等待回收。任务依赖 SQLite 租约防止相同槽位并发重入。`deepseek-precompute` 从交易日 09:15 共享预热开始，覆盖 09:30 观察、09:36 主执行、10:30 降级、13:00 午后三策略和 14:20 最终补审；14:48 停止 API，14:50 冻结最终推荐。缓存命中和无证据弃权不计次，所有策略与模型合计最多 188 次/交易日，并受分策略与分窗口预算约束。
