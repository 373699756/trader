...
class HistoricalFullFieldBundle:
    ...
    def __post_init__(self) -> None:
        ...
        requested = tuple(sorted(set(self.requested_codes)))
        if requested != tuple(sorted(self.requested_codes)) or any(not code for code in requested):
...
def _validate_full_field_cutoffs(
    ...
    if any(item.session_date > trade_date or item.lineage.received_at > cutoff_by_code[item.code] for item in daily):
        raise ValueError("daily bars must be available by the candidate cutoff")
    if any(
