"""Deterministic five-candidate daily-close model selection and freezing."""
...
class DailyCloseBaseModelFit:
    ...
    def __post_init__(self) -> None:
        ...
        if width < 1 or len(self.preprocessing_scales) != width or len(self.ridge_coefficients) != width:
            raise ValueError("daily-close model selection fitted model vector widths are inconsistent")
        numeric = (
        ...
        if not all(math.isfinite(value) for value in numeric) or any(
            ...
            raise ValueError("daily-close model selection fitted model parameters are invalid")
        if not self.lightgbm_model_text.strip() or self.lightgbm_best_iteration < 1 or not self.dependency_versions:
            raise ValueError("daily-close model selection fitted model dependencies are incomplete")


class DailyCloseBaseModelTrainerPort(Protocol):
...
@dataclass(frozen=True)
class DailyCloseOutOfFoldPrediction:
    candidate_id: str
    fold_index: int
    trade_date: date
    code: str
    ...
    schema_version: str = "tomorrow_daily_close_oof_prediction"
    production_authority: bool = False

    def __post_init__(self) -> None:
        if self.candidate_id not in _CANDIDATE_IDS or not 1 <= self.fold_index <= 5:
            raise ValueError("daily-close model selection OOF identity is invalid")
        if not math.isfinite(self.predicted_net_excess_return) or not all(
            ...
            raise ValueError("daily-close model selection OOF values must be finite")
        if self.schema_version != "tomorrow_daily_close_oof_prediction" or self.production_authority:
            raise ValueError("daily-close model selection OOF schema is invalid")


@dataclass(frozen=True)
class DailyCloseCandidateMetrics:
    ...
    def __post_init__(self) -> None:
        ...
        if min(self.evaluated_trade_dates, self.evaluated_rows, self.positive_fold_count) < 0:
            raise ValueError("daily-close model selection candidate metric counts are invalid")
        if not all(math.isfinite(value) for value in values):
            raise ValueError("daily-close model selection candidate metrics must be finite")
        if not 0.0 <= self.severe_loss_rate <= 1.0 or not 0.0 <= self.rank_direction_rate <= 1.0:
            raise ValueError("daily-close model selection candidate rates are invalid")


@dataclass(frozen=True)
class DailyCloseCandidateEvaluation:
    candidate_id: str
    predictions: tuple[DailyCloseOutOfFoldPrediction, ...]
    metrics: DailyCloseCandidateMetrics
    production_authority: bool = False
    ...
    def __post_init__(self) -> None:
        if self.candidate_id not in _CANDIDATE_IDS or any(
            ...
            raise ValueError("daily-close model selection candidate OOF identity is invalid")
        identities = tuple((item.fold_index, item.trade_date, item.code) for item in self.predictions)
        if len(identities) != len(set(identities)):
            raise ValueError("daily-close model selection candidate OOF identities must be unique")
        if self.production_authority:
            raise ValueError("daily-close model selection candidate OOF cannot authorize production")
        object.__setattr__(self, "content_hash", canonical_artifact_hash(self))


@dataclass(frozen=True)
class DailyCloseModelSelectionResult:
    dataset_manifest_hash: str
    temporal_split_hash: str
    candidates: tuple[DailyCloseCandidateEvaluation, ...]
    selected_candidate_id: str
    schema_version: str = "tomorrow_daily_close_model_selection_result"
    ...
    def __post_init__(self) -> None:
        ...
        if tuple(item.candidate_id for item in self.candidates) != _CANDIDATE_IDS:
            raise ValueError("daily-close model selection development requires the fixed five-candidate family")
        if self.selected_candidate_id not in _CANDIDATE_IDS:
            raise ValueError("daily-close model selection selected candidate is invalid")
        if self.schema_version != "tomorrow_daily_close_model_selection_result":
            raise ValueError("daily-close model selection development schema is invalid")
        if self.terminal_holdout_opened or self.production_authority:
            raise ValueError("daily-close model selection development cannot open holdout or production")
        object.__setattr__(self, "content_hash", canonical_artifact_hash(self))


class DailyCloseCandidateEvaluator:
    def evaluate(self, predictions: tuple[DailyCloseOutOfFoldPrediction, ...]) -> DailyCloseCandidateMetrics:
        if not predictions:
            raise ValueError("daily-close model selection evaluation requires OOF predictions")
        by_day: dict[date, list[DailyCloseOutOfFoldPrediction]] = defaultdict(list)
        ...
        return DailyCloseCandidateMetrics(
    ...
    def select(self, candidates: tuple[DailyCloseCandidateEvaluation, ...]) -> str:
        if tuple(item.candidate_id for item in candidates) != _CANDIDATE_IDS:
            raise ValueError("daily-close candidate selection requires the preregistered family")
...
class DailyCloseModelSelectionService:
    def __init__(self, fit_port: DailyCloseBaseModelTrainerPort, evaluator: DailyCloseCandidateEvaluator) -> None:
        self._fit_port = fit_port
        self._evaluator = evaluator

    def develop(self, dataset: FeatureDataset, split: DailyCloseTemporalSplit) -> DailyCloseModelSelectionResult:
        _validate_dataset_split(dataset, split)
        predictions: dict[str, list[DailyCloseOutOfFoldPrediction]] = {
            candidate_id: [] for candidate_id in _CANDIDATE_IDS
        }
        base_oof: list[DailyCloseOutOfFoldPrediction] = []
        stratum_oof: list[DailyCloseOutOfFoldPrediction] = []
        for fold in build_expanding_walk_forward(split.development_dates):
            ...
            if not training_rows or not validation_rows:
                raise ValueError("daily-close model selection fold lacks mature training or validation rows")
            ...
            if len(ridge) != len(validation_rows) or len(lightgbm) != len(validation_rows):
                raise ValueError("daily-close model selection fit port prediction width is invalid")
        ...
        return DailyCloseModelSelectionResult(
    ...
    def freeze(
        ...
        development: DailyCloseModelSelectionResult,
        *,
        ...
        if (
            ...
            raise ValueError("daily-close model selection development parents do not match freeze inputs")
        ...
        return CandidateModelArtifact(
            model_id="score_tomorrow_daily_close_model_selection",
...
def _validate_dataset_split(dataset: FeatureDataset, split: DailyCloseTemporalSplit) -> None:
    if dataset.manifest.trading_dates != split.all_dates:
        raise ValueError("daily-close model selection split must bind the complete dataset date identity")
    if set(split.point_in_time_reserved_dates).intersection(split.daily_close_dates):
        raise ValueError("daily-close model selection daily-close dates cannot open point-in-time reserve")
...
def _fit_oof_board_corrections(predictions: tuple[DailyCloseOutOfFoldPrediction, ...]) -> tuple[StratumCorrection, ...]:
...
def _oof_rows(
    ...
    rows: tuple[DailyCloseFeatureRow, ...],
    predictions: tuple[float, ...],
) -> tuple[DailyCloseOutOfFoldPrediction, ...]:
    return tuple(
        DailyCloseOutOfFoldPrediction(
            candidate_id,
            fold.index,
            row.trade_date,
            row.code,
...
def _hash(value: str, label: str) -> None:
    if _SHA256.fullmatch(value) is None:
        raise ValueError(f"daily-close model selection {label} identity must be SHA-256")


__all__ = [
    "DailyCloseBaseModelTrainerPort",
    "DailyCloseCandidateEvaluator",
    "DailyCloseModelSelectionService",
    "DailyCloseModelSelectionResult",
