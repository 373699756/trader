"""Today V2 exact-boundary freeze with permanent missed-freeze closure."""
...
class TodayV2FreezeCoordinator:
    ...
    def initialize(self) -> V2FreezeOperationResult:
        ...
        if self._index.is_closed(Strategy.TODAY, now.date()):
            self._index.discard_closed_current(Strategy.TODAY, now.date())
            return V2FreezeOperationResult("missed_freeze")
        return V2FreezeOperationResult("ready_for_freeze")
    ...
    def _close(self, trade_date: date) -> None:
        ...
        if not self._index.close_for_date(Strategy.TODAY, trade_date, boundary_at=boundary):
            raise RuntimeError("Today V2 freeze close conflicts with another trade date")
...
__all__ = ["TodayV2FreezeCoordinator"]
