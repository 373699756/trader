"""Feature assembly boundary for recommendation use cases.

Feature calculation consumes an already published data-plane snapshot.  It
does not fetch data, read training files, or write caches; callers can replace
the implementation with a deterministic test calculator.
"""
...
class ScoredSelectionNotReadyError(RuntimeError):
    """A coherent current market epoch is not available for feature assembly."""


class FeatureCalculationPort(Protocol):
    """Assemble immutable recommendation features from a coherent snapshot."""

    def calculate(self, snapshot: MarketDataPlaneSnapshot) -> tuple[FeatureSnapshot, ...]: ...


class FeatureCalculationService(FeatureCalculationPort):
    """Assemble scored features from one coherent data-plane snapshot."""
...
def assemble_scored_features(snapshot: MarketDataPlaneSnapshot) -> tuple[FeatureSnapshot, ...]:
    """Merge market, candidate and research epochs into immutable features."""
...
def _assemble_feature(context: _AssemblyContext, code: str) -> FeatureSnapshot:
    ...
    if candidate_is_current and context.candidate_epoch is not None and candidate_quote is not None:
        ...
        observed_times.extend((context.candidate_epoch.observed_at, context.candidate_epoch.received_at))
    if research is not None and context.research_epoch is not None:
        ...
        observed_times.extend((context.research_epoch.observed_at, context.research_epoch.received_at))
