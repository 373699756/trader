"""Public pipeline contracts owned by the application layer."""
...
CONTRACT_VERSION = "pipeline_contracts_v1"
...
class PipelineContractError(ValueError):
    """A public pipeline contract value failed validation."""
...
class MemoryBudgetContract:
    ...
    def __post_init__(self) -> None:
        ...
        if self.cache_logical_bytes != LOGICAL_CACHE_LIMIT_BYTES:
            raise PipelineContractError("logical cache budget must be exactly 248 MiB")
        if self.process_peak_rss_bytes != PROCESS_PEAK_RSS_LIMIT_BYTES:
            raise PipelineContractError("process peak RSS budget must be exactly 384 MiB")
        if self.process_peak_rss_bytes <= self.cache_logical_bytes:
            raise PipelineContractError("process peak RSS must be separate from and larger than logical cache")
...
class MemoryUsageSnapshot:
    ...
    def __post_init__(self) -> None:
        if self.cache_logical_bytes < 0 or self.process_peak_rss_bytes < 0:
            raise PipelineContractError("memory usage counters cannot be negative")
...
class MarketChangeSet:
    ...
    def __post_init__(self) -> None:
        ...
        if self.overlay_only and any((self.inserted_codes, self.updated_codes, self.removed_codes)):
            raise PipelineContractError("overlay-only change set cannot alter feature rows")
        if self.full_invalidation_reason is not None and not self.full_invalidation_reason.strip():
            raise PipelineContractError("full invalidation reason cannot be blank")
...
class FeatureSnapshotEnvelope:
    ...
    def __post_init__(self) -> None:
        ...
        if self.merge_epoch != self.market_change_set.merge_epoch:
            raise PipelineContractError("feature envelope and market change set merge_epoch must match")
        ...
        if codes != tuple(sorted(codes)) or len(set(codes)) != len(codes):
            raise PipelineContractError("feature snapshots must be sorted by unique code")
...
class EvidenceManifestItem:
    ...
    def __post_init__(self) -> None:
        ...
        if self.published_at > self.received_at:
            raise PipelineContractError("evidence published_at cannot be after received_at")
...
class EvidenceManifest:
    ...
    def __post_init__(self) -> None:
        ...
        if len(set(evidence_ids)) != len(evidence_ids):
            raise PipelineContractError("evidence manifest ids must be unique")
...
class HighValueReviewInput:
    ...
    def __post_init__(self) -> None:
        ...
        if not math.isfinite(self.local_score):
            raise PipelineContractError("local_score must be finite")
        if self.local_rank < 1:
            raise PipelineContractError("local_rank must be positive")
        if self.action_threshold is not None and not math.isfinite(self.action_threshold):
            raise PipelineContractError("action_threshold must be finite when present")
        if self.owner_identity.owner_strategy is not self.strategy:
            raise PipelineContractError("owner strategy must match review input strategy")
...
class HighValueReviewManifest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.strategy is Strategy.LONG and self.inputs:
            raise PipelineContractError("long review input collection must be empty")
        codes = tuple(item.candidate_code for item in self.inputs)
        if len(set(codes)) != len(codes):
            raise PipelineContractError("review input candidate codes must be unique")
        if any(item.strategy is not self.strategy or item.trade_date != self.trade_date for item in self.inputs):
            raise PipelineContractError("review input identities must match the manifest")
        if any(item.evidence_manifest_hash != self.evidence_manifest.manifest_hash for item in self.inputs):
            raise PipelineContractError("review inputs must reference the manifest hash")
...
class DeepSeekV4Facts:
    ...
    def validate_against_manifest(self, manifest: EvidenceManifest) -> None:
        ...
        if not manifest.contains_all(evidence_ids):
            raise PipelineContractError("DeepSeek facts reference evidence outside the manifest")
...
class ProjectionUpsert:
    ...
    def __post_init__(self) -> None:
        ...
        if self.rank < 1:
            raise PipelineContractError("projection rank must be positive")
        if not math.isfinite(self.score):
            raise PipelineContractError("projection score must be finite")
...
class ProjectionEvent:
    ...
    def __post_init__(self) -> None:
        ...
        if self.filtered_count < 0:
            raise PipelineContractError("filtered_count cannot be negative")
        ...
        if len(set(upsert_codes)) != len(upsert_codes):
            raise PipelineContractError("projection upsert codes must be unique")
        if set(upsert_codes).intersection(removed_codes):
            raise PipelineContractError("projection cannot upsert and remove the same code")
    ...
    def __post_init__(self) -> None:
        ...
        for field_name in ("price", "pct_change", "data_age_seconds"):
            ...
            if value is not None and not math.isfinite(value):
                raise PipelineContractError(f"{field_name} must be finite when present")
...
class OverlayEvent:
    ...
    def __post_init__(self) -> None:
        ...
        if len(set(codes)) != len(codes):
            raise PipelineContractError("overlay quote codes must be unique")
        if any(quote.source_time > self.observed_at for quote in self.quotes):
            raise PipelineContractError("overlay cannot contain future quotes")
...
def pipeline_schema_versions() -> JsonObject:
    return freeze_json_object(
        {
...
def _require_version(actual: str, expected: str, label: str) -> None:
    if actual != expected:
        raise PipelineContractError(f"{label} schema version must be {expected}")


def _require_text(value: str, label: str) -> None:
    if not value.strip():
        raise PipelineContractError(f"{label} must not be empty")
...
def _require_code(value: str, label: str) -> None:
    if len(value) != 6 or not value.isdigit():
        raise PipelineContractError(f"{label} must be a normalized six-digit stock code")


def _require_aware(value: datetime, label: str) -> None:
    if value.tzinfo is None or value.utcoffset() is None:
        raise PipelineContractError(f"{label} must be timezone-aware")


def _sorted_unique_codes(values: tuple[str, ...], label: str) -> tuple[str, ...]:
    if any(len(value) != 6 or not value.isdigit() for value in values):
        raise PipelineContractError(f"{label} must contain normalized six-digit stock codes")
    if len(set(values)) != len(values):
        raise PipelineContractError(f"{label} must be unique")
    return tuple(sorted(values))


def _sorted_unique_texts(values: tuple[str, ...], label: str) -> tuple[str, ...]:
    if any(not value.strip() for value in values):
        raise PipelineContractError(f"{label} cannot contain blank values")
    if len(set(values)) != len(values):
        raise PipelineContractError(f"{label} must be unique")
    return tuple(sorted(values))
