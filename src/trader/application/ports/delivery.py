"""delivery phase-2 public contracts owned by the application layer."""
...
CONTRACT_VERSION = "delivery_contract_base_v1"
...
class DeliveryContractError(ValueError):
    """A public delivery contract value failed validation."""
...
class MemoryBudgetContract:
    ...
    def __post_init__(self) -> None:
        ...
        if self.cache_logical_bytes != LOGICAL_CACHE_LIMIT_BYTES:
            raise DeliveryContractError("logical cache budget must be exactly 248 MiB")
        if self.process_peak_rss_bytes != PROCESS_PEAK_RSS_LIMIT_BYTES:
            raise DeliveryContractError("process peak RSS budget must be exactly 384 MiB")
        if self.process_peak_rss_bytes <= self.cache_logical_bytes:
            raise DeliveryContractError("process peak RSS must be separate from and larger than logical cache")
...
class MemoryUsageSnapshot:
    ...
    def __post_init__(self) -> None:
        if self.cache_logical_bytes < 0 or self.process_peak_rss_bytes < 0:
            raise DeliveryContractError("memory usage counters cannot be negative")
...
class MarketChangeSet:
    ...
    def __post_init__(self) -> None:
        ...
        if self.overlay_only and any((self.inserted_codes, self.updated_codes, self.removed_codes)):
            raise DeliveryContractError("overlay-only change set cannot alter feature rows")
        if self.full_invalidation_reason is not None and not self.full_invalidation_reason.strip():
            raise DeliveryContractError("full invalidation reason cannot be blank")
...
class FeatureSnapshotEnvelope:
    ...
    def __post_init__(self) -> None:
        ...
        if self.merge_epoch != self.market_change_set.merge_epoch:
            raise DeliveryContractError("feature envelope and market change set merge_epoch must match")
        ...
        if codes != tuple(sorted(codes)) or len(set(codes)) != len(codes):
            raise DeliveryContractError("feature snapshots must be sorted by unique code")
...
class EvidenceManifestItem:
    ...
    def __post_init__(self) -> None:
        ...
        if self.published_at > self.received_at:
            raise DeliveryContractError("evidence published_at cannot be after received_at")
...
class EvidenceManifest:
    ...
    def __post_init__(self) -> None:
        ...
        if len(set(evidence_ids)) != len(evidence_ids):
            raise DeliveryContractError("evidence manifest ids must be unique")
...
class HighValueReviewInput:
    ...
    def __post_init__(self) -> None:
        ...
        if not math.isfinite(self.local_score):
            raise DeliveryContractError("local_score must be finite")
        if self.local_rank < 1:
            raise DeliveryContractError("local_rank must be positive")
        if self.action_threshold is not None and not math.isfinite(self.action_threshold):
            raise DeliveryContractError("action_threshold must be finite when present")
        if self.owner_identity.owner_strategy is not self.strategy:
            raise DeliveryContractError("owner strategy must match review input strategy")
...
class HighValueReviewManifest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.strategy is Strategy.LONG and self.inputs:
            raise DeliveryContractError("long review input collection must be empty")
        codes = tuple(item.candidate_code for item in self.inputs)
        if len(set(codes)) != len(codes):
            raise DeliveryContractError("review input candidate codes must be unique")
        if any(item.strategy is not self.strategy or item.trade_date != self.trade_date for item in self.inputs):
            raise DeliveryContractError("review input identities must match the manifest")
        if any(item.evidence_manifest_hash != self.evidence_manifest.manifest_hash for item in self.inputs):
            raise DeliveryContractError("review inputs must reference the manifest hash")
...
class DeepSeekV4Facts:
    ...
    def validate_against_manifest(self, manifest: EvidenceManifest) -> None:
        ...
        if not manifest.contains_all(evidence_ids):
            raise DeliveryContractError("DeepSeek facts reference evidence outside the manifest")
...
class ProjectionUpsert:
    ...
    def __post_init__(self) -> None:
        ...
        if self.rank < 1:
            raise DeliveryContractError("projection rank must be positive")
        if not math.isfinite(self.score):
            raise DeliveryContractError("projection score must be finite")
...
class ProjectionEvent:
    ...
    def __post_init__(self) -> None:
        ...
        if self.filtered_count < 0:
            raise DeliveryContractError("filtered_count cannot be negative")
        ...
        if len(set(upsert_codes)) != len(upsert_codes):
            raise DeliveryContractError("projection upsert codes must be unique")
        if set(upsert_codes).intersection(removed_codes):
            raise DeliveryContractError("projection cannot upsert and remove the same code")
    ...
    def __post_init__(self) -> None:
        ...
        for field_name in ("price", "pct_change", "data_age_seconds"):
            ...
            if value is not None and not math.isfinite(value):
                raise DeliveryContractError(f"{field_name} must be finite when present")
...
class OverlayEvent:
    ...
    def __post_init__(self) -> None:
        ...
        if len(set(codes)) != len(codes):
            raise DeliveryContractError("overlay quote codes must be unique")
        if any(quote.source_time > self.observed_at for quote in self.quotes):
            raise DeliveryContractError("overlay cannot contain future quotes")
...
def _require_version(actual: str, expected: str, label: str) -> None:
    if actual != expected:
        raise DeliveryContractError(f"{label} schema version must be {expected}")


def _require_text(value: str, label: str) -> None:
    if not value.strip():
        raise DeliveryContractError(f"{label} must not be empty")
...
def _require_code(value: str, label: str) -> None:
    if len(value) != 6 or not value.isdigit():
        raise DeliveryContractError(f"{label} must be a normalized six-digit stock code")


def _require_aware(value: datetime, label: str) -> None:
    if value.tzinfo is None or value.utcoffset() is None:
        raise DeliveryContractError(f"{label} must be timezone-aware")


def _sorted_unique_codes(values: tuple[str, ...], label: str) -> tuple[str, ...]:
    if any(len(value) != 6 or not value.isdigit() for value in values):
        raise DeliveryContractError(f"{label} must contain normalized six-digit stock codes")
    if len(set(values)) != len(values):
        raise DeliveryContractError(f"{label} must be unique")
    return tuple(sorted(values))


def _sorted_unique_texts(values: tuple[str, ...], label: str) -> tuple[str, ...]:
    if any(not value.strip() for value in values):
        raise DeliveryContractError(f"{label} cannot contain blank values")
    if len(set(values)) != len(values):
        raise DeliveryContractError(f"{label} must be unique")
    return tuple(sorted(values))
