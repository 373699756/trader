...
class SnapshotPublisher:
    def __init__(
        ...
        *,
        ...
        self._rejected_late_drafts = 0
        self._rejected_frozen_replacements = 0
        self._rejected_older_snapshots = 0
        self._enqueue_latencies_ms: deque[float] = deque(maxlen=512)
    ...
    def publish(self, snapshot: RecommendationSnapshot) -> PublishedEvent | None:
        started = self._monotonic()
        with self._lock:
            base_snapshot = self._snapshots.get(snapshot.strategy.value)
            if base_snapshot is not None and snapshot.trade_date < base_snapshot.trade_date:
                self._rejected_older_snapshots += 1
                return None
            if base_snapshot is not None and base_snapshot.trade_date == snapshot.trade_date and base_snapshot.frozen:
                if not snapshot.frozen:
                    self._rejected_late_drafts += 1
                elif base_snapshot.snapshot_id != snapshot.snapshot_id or base_snapshot != snapshot:
                    self._rejected_frozen_replacements += 1
                return None
            ...
            today_source_times = (
                tuple(item.features.quote.source_time for item in snapshot.recommendations)
                if snapshot.strategy.value == "today"
                else ()
            )
        emitted_at = self._now()
        enqueue_ms = max(0.0, (self._monotonic() - started) * 1000.0)
        with self._lock:
            self._enqueue_latencies_ms.append(enqueue_ms)
            self._publish_latencies.append(max(0.0, (emitted_at - snapshot.published_at).total_seconds()))
            self._today_score_latencies.extend(
                max(0.0, (emitted_at - source_time).total_seconds()) for source_time in today_source_times
            )
        return event
    ...
    def publish_overlay(self, overlay: LiveOverlay) -> PublishedEvent:
        started = self._monotonic()
        with self._lock:
            event = self._new_event_locked("overlay_patch", overlay_patch(overlay))
        emitted_at = self._now()
        enqueue_ms = max(0.0, (self._monotonic() - started) * 1000.0)
        with self._lock:
            self._enqueue_latencies_ms.append(enqueue_ms)
    ...
    def status(self) -> dict[str, object]:
        with self._lock:
            return {
                ...
                "rejected_late_drafts": self._rejected_late_drafts,
                "rejected_frozen_replacements": self._rejected_frozen_replacements,
                "rejected_older_snapshots": self._rejected_older_snapshots,
                "sse_enqueue_latency": _latency_summary_ms(self._enqueue_latencies_ms, target_ms=100.0),
...
def _latency_summary_ms(values: deque[float], *, target_ms: float) -> dict[str, object]:
    if not values:
        return {
            "sample_count": 0,
            "p50_ms": None,
