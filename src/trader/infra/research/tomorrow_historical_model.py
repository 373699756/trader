"""Deterministic ridge/LightGBM trainer for the isolated Tomorrow historical screen."""
...
class TomorrowHistoricalEnsembleTrainer:
    def fit(
        self,
        ...
        artifact = TomorrowHistoricalModelArtifact(
            ...
            feature_ids=TOMORROW_HISTORICAL_ALPHA_FEATURE_IDS,
        ...
        return TomorrowHistoricalModelFit(
            artifact=artifact,
...
def _internal_validation_start(rows: tuple[TomorrowHistoricalRow, ...]) -> int:
    dates = tuple(sorted({row.trade_date for row in rows}))
    if len(dates) < 5:
        raise ValueError("Tomorrow historical training requires at least five chronological dates")
...
def _matrix(rows: Sequence[TomorrowHistoricalRow]) -> _Array:
    return _NUMPY.asarray(tuple(row.alpha_features for row in rows), dtype=_NUMPY.float64)


def _targets(rows: Sequence[TomorrowHistoricalRow]) -> _Array:
...
def _fit_lightgbm(
    ...
    train = lgb.Dataset(
        fitting_x,
        label=fitting_y,
        feature_name=list(TOMORROW_HISTORICAL_ALPHA_FEATURE_IDS),
        free_raw_data=True,
    )
...
__all__ = ["TomorrowHistoricalEnsembleTrainer"]
