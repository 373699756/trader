"""Immutable JSON artifact boundary for the Tomorrow training BaoStock dataset."""
...
class BaoStockTrainingDatasetArtifactConflictError(RuntimeError):
    """Raised when an existing dataset artifact conflicts or is corrupt."""


class BaoStockTrainingDatasetArtifactStore:
    def __init__(self, root: Path) -> None:
        self._root = root
        self._path = root / "tomorrow-training-dataset.json"

    def write(self, manifest: BaoStockTrainingDatasetManifest) -> BaoStockTrainingDatasetManifest:
        self._root.mkdir(parents=True, exist_ok=True)
        if self._path.exists():
            existing = self.verify()
            if existing.content_hash != manifest.content_hash:
                raise BaoStockTrainingDatasetArtifactConflictError("dataset artifact identity conflict")
            return existing

        payload = _encode_manifest(manifest)
        payload["content_hash"] = manifest.content_hash
        descriptor, temporary_name = tempfile.mkstemp(
            prefix=".tomorrow-training-dataset.", suffix=".tmp", dir=self._root
        )
        ...
        try:
            ...
            try:
                ...
                existing = self.verify()
                if existing.content_hash != manifest.content_hash:
                    raise BaoStockTrainingDatasetArtifactConflictError("dataset artifact identity conflict") from None
                return existing
        finally:
            temporary.unlink(missing_ok=True)
        return self.verify()

    def verify(self) -> BaoStockTrainingDatasetManifest:
        try:
            ...
            raise BaoStockTrainingDatasetArtifactConflictError("dataset artifact schema or hash is invalid") from exc


def _encode_manifest(value: BaoStockTrainingDatasetManifest) -> dict[str, object]:
    return {
...
def _decode_manifest(raw: dict[str, object]) -> BaoStockTrainingDatasetManifest:
    _fields(
        raw,
        {
            "daily_manifest_hash",
    ...
    return BaoStockTrainingDatasetManifest(
        ...
        status=cast(BaoStockTrainingDatasetStatus, _string(raw["status"])),
...
def _encode_label_contract(value: BaoStockTrainingLabelContract) -> dict[str, object]:
    return {
...
def _decode_label_contract(raw: dict[str, object]) -> BaoStockTrainingLabelContract:
    _fields(
        raw,
        {
            "formula",
    ...
    return BaoStockTrainingLabelContract(
...
def _encode_split(value: BaoStockTrainingSplit) -> dict[str, object]:
    return {
...
def _decode_split(raw: dict[str, object]) -> BaoStockTrainingSplit:
    date_fields = {
        "model_fit_dates",
...
def _object(value: object, label: str) -> dict[str, object]:
    if not isinstance(value, dict) or any(not isinstance(key, str) for key in value):
        raise TypeError(f"BaoStock training {label} is invalid")
    return cast(dict[str, object], value)


def _fields(raw: dict[str, object], expected: set[str], label: str) -> None:
    if set(raw) != expected:
        raise ValueError(f"BaoStock training {label} fields are invalid")


def _string(value: object) -> str:
    if not isinstance(value, str):
        raise TypeError("BaoStock training string field is invalid")
    return value


def _boolean(value: object) -> bool:
    if not isinstance(value, bool):
        raise TypeError("BaoStock training boolean field is invalid")
    return value


def _integer(value: object) -> int:
    if isinstance(value, bool) or not isinstance(value, int):
        raise TypeError("BaoStock training integer field is invalid")
    return value


def _strings(value: object) -> tuple[str, ...]:
    if not isinstance(value, list) or not all(isinstance(item, str) for item in value):
        raise TypeError("BaoStock training string list is invalid")
    return tuple(value)
