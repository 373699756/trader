...
from trader.training.domain.evaluation.baseline import population_spearman
from trader.training.domain.evaluation.paired_statistics import (
...
class TerminalHoldoutRow:
    ...
    def __post_init__(self) -> None:
        ...
        values: tuple[float, ...] = (
            *self.actual_net_excess_returns,
            *self.baseline_net_excess_returns,
            self.predicted_net_excess_return,
        )
        values += (
            *self.horizon_net_excess_returns,
            *self.baseline_horizon_net_excess_returns,
            self.mae_atr20,
            self.baseline_mae_atr20,
        )
        if any(not math.isfinite(value) for value in values):
            raise ValueError("terminal holdout numeric values must be finite")
        if self.baseline_horizon_net_excess_returns and len(self.horizon_net_excess_returns) != len(
            self.baseline_horizon_net_excess_returns
        ):
...
@dataclass(frozen=True)
class TerminalHoldoutEvaluation:
    strategy: TerminalStrategy
    research_identity: str
    parent_hash: str
    candidate_hash: str
    rows: tuple[TerminalHoldoutRow, ...]
...
@dataclass(frozen=True)
class TerminalHoldoutParentState:
...
def evaluate_terminal_holdout(evaluation: TerminalHoldoutEvaluation) -> TerminalHoldoutReport:
    """Run exactly one deterministic terminal evaluation over an already sealed candidate."""

    if evaluation.terminal_holdout_already_opened:
        return _closed_report(
            evaluation,
            "historical_rejected",
            ("terminal_holdout_already_opened",),
        )
    if evaluation.parent_status not in {"historical_candidate_ready", "historical_daily_close_proxy_validated"}:
        parent_result: TerminalStatus = (
            "historical_data_insufficient"
            if evaluation.parent_status == "historical_data_insufficient"
            else "historical_rejected"
        )
        return _closed_report(
            evaluation,
            parent_result,
            evaluation.parent_failure_reasons or (evaluation.parent_status,),
        )
    ordered = tuple(sorted(evaluation.rows, key=lambda item: (item.trade_date, item.code)))
    ...
    if evaluation.strategy == "d25" and any(
    ...
    if len(dates) < _MIN_DATES:
        return _closed_report(
            evaluation,
            "historical_data_insufficient",
            ("terminal_trade_dates_below_200",),
        )
    metrics = _metrics(
        ordered,
        strategy=evaluation.strategy,
        bootstrap_block_days=evaluation.bootstrap_block_days or (10 if evaluation.strategy == "d25" else 5),
    )
    failures = _gate_failures(metrics, evaluation.strategy)
    ...
    return TerminalHoldoutReport(
        evaluation.strategy,
        evaluation.research_identity,
        evaluation.parent_hash,
        evaluation.candidate_hash,
        evaluation.anchor or _anchor(evaluation.strategy),
        True,
        result_status,
        metrics,
        failures,
...
def _closed_report(
    evaluation: TerminalHoldoutEvaluation,
    status: TerminalStatus,
    reasons: tuple[str, ...],
...
def _metrics(
    ...
    paired: tuple[float, float, float] = tuple(
        _mean(
            tuple(
                accumulator.candidate_daily[day][index] - accumulator.baseline_daily[day][index]
                for day in range(len(accumulator.candidate_daily))
            )
        )
        for index in range(3)
    )  # type: ignore[assignment]
    lowers = _cost_lowers(
        accumulator.candidate_daily,
        accumulator.baseline_daily,
        strategy=strategy,
        bootstrap_block_days=bootstrap_block_days,
    )
    total_positive = math.fsum(accumulator.positive_by_code.values())
    shares = (
        sorted((value / total_positive for value in accumulator.positive_by_code.values()), reverse=True)
        if total_positive
        else []
    )
    selected_rows = sum(len(values) for values in accumulator.candidate_sets)
    baseline_rows = sum(len(values) for values in accumulator.baseline_sets)
    max_board, maximum_industry_count = _portfolio_concentration(grouped, selected_rows)
    return TerminalHoldoutMetrics(
        len(grouped),
        len(rows),
        selected_rows,
        baseline_rows,
        _cost_means(tuple(accumulator.candidate_daily)),
        _cost_means(tuple(accumulator.baseline_daily)),
        paired,
        lowers,
        _mean(tuple(accumulator.severe_values)) if accumulator.severe_values else None,
        _mean(tuple(accumulator.baseline_severe_values)) if accumulator.baseline_severe_values else None,
        _turnover(accumulator.candidate_sets),
        _turnover(accumulator.baseline_sets),
        _mean(tuple(accumulator.rank_ics)) if accumulator.rank_ics else None,
        _mean(tuple(accumulator.spreads)) if accumulator.spreads else None,
        shares[0] if shares else 0.0,
        math.fsum(shares[:5]),
        max_board,
        maximum_industry_count,
        selected_rows / len(grouped) if grouped else 0.0,
        baseline_rows / len(grouped) if grouped else 0.0,
        _horizon_means(accumulator.horizon),
        _horizon_means(accumulator.baseline_horizon),
        tuple(sorted(accumulator.state_counts.items())),
    )
...
def _return_gate_failures(metrics: TerminalHoldoutMetrics) -> tuple[str, ...]:
    ...
    if any(value <= 0.0 for value in metrics.mean_net_excess_returns[:2]) or any(
        value <= 0.0 for value in metrics.paired_net_increments[:2]
    ):
...
def _risk_gate_failures(metrics: TerminalHoldoutMetrics) -> tuple[str, ...]:
    ...
    if (
        ...
        or metrics.severe_loss_rate > metrics.baseline_severe_loss_rate
    ):
...
def _canonical_hash(value: object) -> str:
    def canonical(item: object) -> object:
        if dataclasses.is_dataclass(item):
            return {
                field.name: canonical(getattr(item, field.name)) for field in dataclasses.fields(item) if field.init
            }
