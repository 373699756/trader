...
class PreregisteredRuleCandidate:
    candidate_id: str
    strategy: str
...
def _validate_candidate_change(candidate: PreregisteredRuleCandidate) -> None:
    if _IDENTITY.fullmatch(candidate.candidate_id) is None or not candidate.strategy:
        raise ValueError("preregistered rule candidate identity is invalid")
    if candidate.change_kind not in {"control", "observe_rule", "remove_component", "cost_guard"}:
        raise ValueError("preregistered rule candidate change kind is invalid")
    if any(rule not in ABLATION_RULES for rule in candidate.disabled_rules):
        raise ValueError("preregistered rule candidate disabled rule is not preregistered")
...
def _validate_candidate_risk(candidate: PreregisteredRuleCandidate) -> None:
    if not math.isfinite(candidate.cost_rate) or candidate.cost_rate not in {0.002, 0.005, 0.01}:
        raise ValueError("preregistered rule candidate cost must be 20bp, 50bp, or 100bp")
    if candidate.severe_loss_guard is not None and (
        ...
        raise ValueError("preregistered rule candidate severe-loss guard is invalid")
    if candidate.production_authority:
        raise ValueError("preregistered rule candidates cannot authorize production")


@dataclass(frozen=True)
class RuleCandidateMetrics:
    candidate_id: str
    evaluated_rows: int
    evaluated_dates: int
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError("preregistered rule candidate metrics identity/count is invalid")
        values = (
        ...
        if any(not math.isfinite(value) for value in values):
            raise ValueError("preregistered rule candidate metrics must be finite")
        rates = (
            self.severe_loss_rate,
            self.turnover,
            self.concentration,
        ...
        if any(not 0.0 <= value <= 1.0 for value in rates):
            raise ValueError("preregistered rule candidate rates must be in [0, 1]")
        ...
        if any(value is not None and not 0.0 <= value <= 1.0 for value in recalls):
            raise ValueError("preregistered rule candidate recall must be in [0, 1]")
        if self.mean_mae_atr20 is not None and not math.isfinite(self.mean_mae_atr20):
            raise ValueError("preregistered rule candidate MAE must be finite")
        if len(self.daily_net_excess_20bp) != len(self.daily_net_excess_50bp):
            raise ValueError("preregistered rule candidate daily cost evidence must align")


@dataclass(frozen=True)
class PreregisteredRuleCandidateFamily:
    strategy: str
    ...
    def __post_init__(self) -> None:
        if not self.strategy or not self.candidates or len(self.candidates) > 8:
            raise ValueError("preregistered rule candidate family must contain one to eight candidates")
        if self.candidates[0].change_kind != "control":
            raise ValueError("preregistered rule candidate family must start with its control")
        if len({candidate.candidate_id for candidate in self.candidates}) != len(self.candidates):
            raise ValueError("preregistered rule candidate ids must be unique")
        if not re.fullmatch(r"[0-9a-f]{64}", self.source_ablation_hash) or not self.development_dates:
            raise ValueError("preregistered rule candidate family identity is invalid")
        ...
        if self.production_authority:
            raise ValueError("preregistered rule candidate family cannot authorize production")
        object.__setattr__(self, "content_hash", _hash(self))


@dataclass(frozen=True)
class RuleCandidateEvaluationReport:
    ...
    production_authority: bool = False
    schema_version: str = "rule_candidate_evaluation_report"
    content_hash: str = dataclasses.field(init=False)
    ...
    def __post_init__(self) -> None:
        ...
        if tuple(item.candidate_id for item in self.metrics) != expected:
            raise ValueError("preregistered rule candidate report must retain the complete family")
        if self.status == "candidate_family_sealed" and self.selected_candidate_id not in expected:
            raise ValueError("sealed preregistered rule family requires one registered candidate")
        if self.status != "candidate_family_sealed" and self.selected_candidate_id is not None:
            raise ValueError("rejected preregistered rule family cannot select a candidate")
        if self.production_authority or self.schema_version != "rule_candidate_evaluation_report":
            raise ValueError("preregistered rule candidate report cannot authorize production")
        object.__setattr__(self, "content_hash", _hash(self))


def preregister_rule_candidates(
    ...
    *,
    ...
    candidates: list[PreregisteredRuleCandidate] = [
        PreregisteredRuleCandidate(f"{report.strategy}_control", report.strategy, "control")
    ]
    ...
    return PreregisteredRuleCandidateFamily(
        report.strategy, tuple(candidates), report.content_hash, report.development_dates
    )
...
def evaluate_rule_candidate(
    ...
    *,
    control: RuleCandidateMetrics | None = None,
    ...
    if not rows and not evaluation_dates:
        raise ValueError("preregistered rule candidate evaluation requires rows")
    ...
    if any(row.trade_date not in dates for row in rows):
        raise ValueError("preregistered rule candidate rows fall outside the evaluation dates")
...
def evaluate_rule_candidate_family(
    ...
    if family.confirmation_locked:
        raise ValueError("preregistered rule candidate family cannot be reevaluated after confirmation is locked")
    metrics = tuple(
        evaluate_rule_candidate(candidate, rows, evaluation_dates=family.development_dates)
    ...
    return RuleCandidateEvaluationReport(
        family=family,
        metrics=metrics,
...
def choose_development_candidate(metrics: tuple[RuleCandidateMetrics, ...]) -> str | None:
...
def _candidate_passes(candidate: PreregisteredRuleCandidate, row: FilterAblationRow) -> bool:
    if (
