"""Pure consumer contract for the frozen Tomorrow training input."""
...
class DailyInputField:
    ...
    def __post_init__(self) -> None:
        if not self.name or not self.name.isidentifier() or not self.unit or len(self.unit) > 64:
            raise ValueError("Tomorrow training input field contract is invalid")
...
class FrozenDailyInputDescriptor:
    ...
    def __post_init__(self) -> None:
        if _SHA256.fullmatch(self.manifest_hash) is None:
            raise ValueError("Tomorrow training input manifest hash is invalid")
...
class TomorrowInputCompatibility:
    ...
    schema_version: str = "tomorrow_training_input"
    content_hash: str = dataclasses.field(init=False)
    ...
    def __post_init__(self) -> None:
        ...
        if any(_SHA256.fullmatch(value) is None for value in hashes):
            raise ValueError("Tomorrow training input compatibility parent identity is invalid")
        reasons = tuple(sorted(set(self.failure_reasons)))
        if any(_REASON.fullmatch(reason) is None for reason in reasons):
            raise ValueError("Tomorrow training input compatibility failure reason is invalid")
        expected_status = "compatible" if not reasons else "incompatible"
        if self.status != expected_status:
            raise ValueError("Tomorrow training input compatibility status is inconsistent")
        if self.feature_names != TOMORROW_TRAINING_ALPHA_NAMES or self.feature_units != TOMORROW_TRAINING_ALPHA_UNITS:
            raise ValueError("Tomorrow training input compatibility alpha contract is invalid")
        if (
            ...
            raise ValueError(
                "Tomorrow training input compatibility cannot train, open holdout, or authorize production"
            )
...
def evaluate_tomorrow_training_input(
    ...
    *,
    ...
    return TomorrowInputCompatibility(
        ...
        feature_names=TOMORROW_TRAINING_ALPHA_NAMES,
        feature_units=TOMORROW_TRAINING_ALPHA_UNITS,
