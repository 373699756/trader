...
usage() {
  printf '%s\n' \
    "用法: ./run.sh [serve|validate-config|research-status|research-history-download|research-backtest]" \
    "" \
    "环境变量:" \
...
case "$MODE" in
  serve|app)
    ...
    ;;
  validate-config|research-status|research-history-download|research-backtest)
